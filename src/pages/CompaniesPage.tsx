...
const CompanyObjectsSection = ({ companyId, companyArchived }: { companyId: string; companyArchived: boolean }) => {
  ...
  const saveObject = async (request: CompanyObjectRequest) => { if ((editing ? !canEdit : !canCreate) || saving) return; setSaving(true); try { if (editing) await updateCompanyObject(companyId, editing.id, request, editing.version); else await createCompanyObject(companyId, request); await queryClient.invalidateQueries({ queryKey: ['company-objects', companyId] }); await queryClient.invalidateQueries({ queryKey: ['company', companyId] }); toast.success(editing ? 'Объект обновлён' : 'Объект создан'); setModalOpen(false); setEditing(null); } catch (error) { throw error; } finally { setSaving(false); } };
  const confirmArchive = async () => { if (!archiveTarget || !canArchive || saving) return; setSaving(true); try { if (archiveTarget.status === 'ARCHIVED') await restoreCompanyObject(companyId, archiveTarget.id, archiveTarget.version); else await archiveCompanyObject(companyId, archiveTarget.id, archiveTarget.version); setArchiveTarget(null); await queryClient.invalidateQueries({ queryKey: ['company-objects', companyId] }); await queryClient.invalidateQueries({ queryKey: ['company', companyId] }); toast.success(archiveTarget.status === 'ARCHIVED' ? 'Объект восстановлен' : 'Объект архивирован'); } catch (error) { toast.error('Не удалось изменить статус объекта', getApiErrorMessage(error)); } finally { setSaving(false); } };
