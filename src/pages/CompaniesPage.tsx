...
import { Archive, ArrowLeft, Edit3, Eye, Plus, RefreshCw, RotateCcw, Search } from 'lucide-react';
...
import MembershipAccessPanel from '../components/memberships/MembershipAccessPanel';
import Modal from '../components/ui/Modal';
import { useAuth } from '../contexts/AuthContext';
import { useToast } from '../hooks/useToast';
import { getApiErrorMessage, getApiStatus } from '../services/apiHelpers';
import {
  archiveCompany, archiveCompanyObject, createCompany, createCompanyObject, getCompanies, getCompanyFieldErrors,
  getCompanyById, getCompanyObjects, restoreCompany, restoreCompanyObject, updateCompany, updateCompanyObject,
...
import { canOpenCompanyEditor, hasCompanyPermission } from '../features/companies/companyPermissions';
...
const CompaniesList = () => {
  ...
  const confirmArchive = async () => {
    ...
    setArchiving(true);
    try {
      if (archiveTarget.status === 'ARCHIVED') await restoreCompany(archiveTarget.id); else await archiveCompany(archiveTarget.id);
      setArchiveTarget(null);
      toast.success(archiveTarget.status === 'ARCHIVED' ? 'Компания восстановлена' : 'Компания архивирована');
  ...
  return <PageShell>
    <header className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
      ...
      {canCreate && <Button asChild><Link to="/staff/companies/new"><Plus className="h-4 w-4" /> Добавить компанию</Link></Button>}
    </header>
    ...
    {!companiesQuery.isLoading && !companiesQuery.isError && companies.length === 0 && (
      <section className="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
        ...
        <div className="mt-4 flex justify-center gap-3">{hasFilters ? <Button type="button" variant="secondary" onClick={resetFilters}>Сбросить фильтры</Button> : canCreate && <Button asChild><Link to="/staff/companies/new">Добавить компанию</Link></Button>}</div>
      </section>
    )}
    {!companiesQuery.isLoading && !companiesQuery.isError && companies.length > 0 && (
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1250px] table-fixed text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">{companies.map((company) => <tr key={company.id} tabIndex={0} onClick={() => navigate(`/staff/companies/${company.id}`)} onKeyDown={(event) => { if (event.key === 'Enter') navigate(`/staff/companies/${company.id}`); }} className={`cursor-pointer align-top focus:bg-eco-50 focus:outline-none ${company.status === 'ARCHIVED' ? 'bg-slate-50/70 text-slate-600' : 'hover:bg-slate-50'}`}>
              <td className="truncate px-4 py-3 font-bold text-slate-900" title={company.name}>{value(company.name)}</td><td className="px-4 py-3 font-mono text-xs">{value(company.bin)}</td><td className="truncate px-4 py-3" title={company.responsiblePerson}>{value(company.responsiblePerson)}</td><td className="px-4 py-3">{value(company.responsiblePersonPhone || company.phone)}</td><td className="truncate px-4 py-3" title={company.contactEmail || company.email}>{value(company.contactEmail || company.email)}</td><td className="truncate px-4 py-3" title={company.actualAddress || company.legalAddress}>{value(company.actualAddress || company.legalAddress)}</td><td className="px-4 py-3">{company.objectCount ?? 0}</td><td className="px-4 py-3"><StatusPill status={company.status} /></td><td className="px-4 py-3">{formatDate(company.updatedAt)}</td>
              <td className="px-4 py-3" onClick={stop}><div className="flex justify-end gap-2"><button type="button" aria-label={`Открыть ${company.name}`} onClick={() => navigate(`/staff/companies/${company.id}`)} className="inline-flex h-10 w-10 items-center justify-center rounded-full text-eco-800 ring-1 ring-eco-200 hover:bg-eco-50"><Eye className="h-4 w-4" /></button>{canEdit && company.status === 'ACTIVE' && <button type="button" aria-label={`Изменить ${company.name}`} onClick={() => navigate(`/staff/companies/${company.id}/edit`)} className="inline-flex h-10 w-10 items-center justify-center rounded-full text-eco-800 ring-1 ring-eco-200 hover:bg-eco-50"><Edit3 className="h-4 w-4" /></button>}{canArchive && company.status === 'ACTIVE' && <button type="button" aria-label={`Архивировать ${company.name}`} onClick={() => setArchiveTarget(company)} className="inline-flex h-10 w-10 items-center justify-center rounded-full text-rose-700 ring-1 ring-rose-200 hover:bg-rose-50"><Archive className="h-4 w-4" /></button>}{canArchive && company.status === 'ARCHIVED' && <button type="button" aria-label={`Восстановить ${company.name}`} onClick={() => setArchiveTarget(company)} className="inline-flex h-10 w-10 items-center justify-center rounded-full text-eco-800 ring-1 ring-eco-200 hover:bg-eco-50"><RotateCcw className="h-4 w-4" /></button>}</div></td>
            </tr>)}</tbody>
          </table>
        </div>
    ...
    <Modal open={Boolean(archiveTarget)} loading={archiving} onClose={() => !archiving && setArchiveTarget(null)} title={`${archiveTarget?.status === 'ARCHIVED' ? 'Восстановить' : 'Архивировать'} компанию «${archiveTarget?.name || ''}»?`} description={archiveTarget?.status === 'ARCHIVED' ? 'Компания снова станет доступна для работы.' : 'Компания станет недоступна для создания новых протоколов. Существующие протоколы сохранятся.'} size="sm" footer={<><Button type="button" variant="secondary" disabled={archiving} onClick={() => setArchiveTarget(null)}>Отмена</Button><Button type="button" variant={archiveTarget?.status === 'ARCHIVED' ? 'primary' : 'danger'} disabled={archiving} onClick={confirmArchive}>{archiving ? 'Выполнение…' : archiveTarget?.status === 'ARCHIVED' ? 'Восстановить' : 'Архивировать'}</Button></>}><p className="text-sm text-slate-600">Статус изменится только после успешного ответа backend.</p></Modal>
  </PageShell>;
};
...
const objectServerFieldMap: Record<string, FieldPath<CompanyObjectFormValues>> = {
  facilityName: 'name', facilityType: 'objectType', facilityAddress: 'address',
  responsiblePerson: 'contactPerson', responsiblePersonPhone: 'contactPhone',
  isPrimary: 'primary', businessActivity: 'activityType', samplingPlace: 'samplingLocation', comment: 'notes',
};
const resolveObjectServerField = (field: string): FieldPath<CompanyObjectFormValues> | undefined => {
  const path = field.replace(/\[\d+\]/g, '').split('.').filter(Boolean);
  const leaf = path[path.length - 1] || field;
  return objectServerFieldMap[field] || objectServerFieldMap[leaf]
    || (leaf in emptyObjectValues ? leaf as FieldPath<CompanyObjectFormValues> : undefined);
};
...
const CompanyObjectModal = ({ open, object, loading, onClose, onSubmit }: { open: boolean; object: CompanyObject | null; loading: boolean; onClose: () => void; onSubmit: (request: CompanyObjectRequest) => Promise<void> }) => {
  ...
  const submit: SubmitHandler<CompanyObjectFormValues> = async (values) => {
    try {
      await onSubmit(toObjectRequest(values));
      reset(values);
    } catch (error) {
      const fieldErrors = getCompanyFieldErrors(error);
      let mappedErrors = 0;
      fieldErrors.forEach(({ field, message }) => {
        const target = resolveObjectServerField(field);
        if (target) {
          mappedErrors += 1;
          setError(target, { type: 'server', message });
        }
      });
      if (!mappedErrors) setError('root.server', { type: 'server', message: getApiErrorMessage(error, 'Не удалось сохранить объект') });
    }
  };
...
const CompanyObjectsSection = ({ companyId, companyArchived }: { companyId: string; companyArchived: boolean }) => {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-bold text-slate-900">Объекты</h2>{companyArchived && <p className="mt-1 text-sm text-amber-700">Для архивной компании изменение статуса и данных объектов недоступно.</p>}</div>{canCreate && <Button type="button" onClick={() => { setEditing(null); setModalOpen(true); }}><Plus className="h-4 w-4" /> Добавить объект</Button>}</div>
    ...
    <Modal open={Boolean(archiveTarget)} loading={saving} onClose={() => !saving && setArchiveTarget(null)} title={`${archiveTarget?.status === 'ARCHIVED' ? 'Восстановить' : 'Архивировать'} объект «${archiveTarget?.name || ''}»?`} size="sm" footer={<><Button type="button" variant="secondary" disabled={saving} onClick={() => setArchiveTarget(null)}>Отмена</Button><Button type="button" variant={archiveTarget?.status === 'ARCHIVED' ? 'primary' : 'danger'} disabled={saving} onClick={confirmArchive}>{saving ? 'Выполнение…' : archiveTarget?.status === 'ARCHIVED' ? 'Восстановить' : 'Архивировать'}</Button></>}><p className="text-sm text-slate-600">Статус объекта изменится только после успешного ответа backend.</p></Modal>
  </section>;
};

const CompanyPekSection = ({ companyId, archived }: { companyId: string; archived: boolean }) => (
  <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h2 className="text-lg font-bold text-slate-900">Экология и ПЭК</h2>
        <p className="mt-1 text-sm text-slate-600">Экологические разрешения, точки мониторинга, источники выбросов, выпуски сточных вод и места накопления отходов хранятся в production-модуле ПЭК.</p>
      </div>
      <div className="flex flex-wrap gap-2">
        <Button asChild variant="secondary"><Link to={`/staff/pek/programs?companyId=${encodeURIComponent(companyId)}`}>Программы ПЭК</Link></Button>
        <Button asChild variant="secondary"><Link to={`/staff/pek/reports?companyId=${encodeURIComponent(companyId)}`}>Отчёты ПЭК</Link></Button>
        {!archived && <Button asChild><Link to={`/staff/pek/programs/new?companyId=${encodeURIComponent(companyId)}`}>Настроить ПЭК</Link></Button>}
      </div>
    </div>
  </section>
);
...
const CompanyFormPage = ({ edit = false }: { edit?: boolean }) => {
  ...
  if (edit && companyQuery.data && !canOpenCompanyEditor(user, companyQuery.data.status)) return <ErrorBox message="Архивную компанию нельзя редактировать. Сначала восстановите её в списке компаний." back={() => navigate(`/staff/companies/${companyId}`)} />;
...
const CompanyViewPage = () => {
  const { companyId } = useParams(); const navigate = useNavigate(); const toast = useToast(); const queryClient = useQueryClient(); const { user } = useAuth(); const [archiveOpen, setArchiveOpen] = useState(false); const [archiving, setArchiving] = useState(false); const [tab, setTab] = useState<'details' | 'members'>('details');
  ...
  return <PageShell><header className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between"><div><button type="button" onClick={() => navigate('/staff/companies')} className="mb-3 inline-flex min-h-10 items-center gap-2 text-sm font-bold text-eco-700"><ArrowLeft className="h-4 w-4" /> К компаниям</button><div className="flex flex-wrap items-center gap-3"><h1 className="text-2xl font-black text-slate-950 sm:text-3xl">{company.name}</h1><StatusPill status={company.status} /></div><p className="mt-2 font-mono text-sm text-slate-600">БИН {value(company.bin)}</p></div>{company.status === 'ACTIVE' && <div className="flex flex-wrap gap-2">{canArchive && <Button type="button" variant="danger" onClick={() => setArchiveOpen(true)}><Archive className="h-4 w-4" /> Архивировать</Button>}{canEdit && <Button type="button" onClick={() => navigate(`/staff/companies/${company.id}/edit`)}><Edit3 className="h-4 w-4" /> Изменить</Button>}</div>}</header><nav className="flex gap-2 rounded-xl border bg-white p-2"><button type="button" className={`rounded-lg px-4 py-2 text-sm font-bold ${tab === 'details' ? 'bg-eco-700 text-white' : 'text-slate-600'}`} onClick={() => setTab('details')}>Карточка компании</button>{hasCompanyPermission(user, 'COMPANY_VIEW') && <button type="button" className={`rounded-lg px-4 py-2 text-sm font-bold ${tab === 'members' ? 'bg-eco-700 text-white' : 'text-slate-600'}`} onClick={() => setTab('members')}>Сотрудники / Доступ</button>}</nav>{tab === 'details' ? <><CompanyDetailsBlock company={company} /><CompanyObjectsSection companyId={company.id} companyArchived={company.status === 'ARCHIVED'} /><CompanyPekSection companyId={company.id} archived={company.status === 'ARCHIVED'} /></> : <MembershipAccessPanel scope="companies" companyId={company.id} canView={hasCompanyPermission(user, 'COMPANY_VIEW')} canManage={hasCompanyPermission(user, 'COMPANY_EDIT')} />}<Modal open={archiveOpen} loading={archiving} onClose={() => !archiving && setArchiveOpen(false)} title={`Архивировать компанию «${company.name}»?`} description="Компания станет недоступна для создания новых протоколов. Существующие протоколы сохранятся." size="sm" footer={<><Button type="button" variant="secondary" disabled={archiving} onClick={() => setArchiveOpen(false)}>Отмена</Button><Button type="button" variant="danger" disabled={archiving} onClick={confirmArchive}>{archiving ? 'Архивирование…' : 'Архивировать'}</Button></>}><p className="text-sm text-slate-600">Просмотр старых данных останется доступен.</p></Modal></PageShell>;
};
