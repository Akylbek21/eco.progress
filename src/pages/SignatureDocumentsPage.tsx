...
import { Download, Eye, FileCheck2, FileSignature, MoreVertical, Upload } from 'lucide-react';
...
export default function SignatureDocumentsPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      {documents.isLoading ? <div className="flex min-h-48 items-center justify-center"><LoadingSpinner /></div> : documents.isError ? <div className="p-6 text-sm font-semibold text-rose-700">{normalizeApiError(documents.error, 'Не удалось загрузить список.').message} <button className="ml-2 underline" onClick={() => documents.refetch()}>Повторить</button></div> : !documents.data?.items.length ? <div className="p-12 text-center"><FileCheck2 className="mx-auto h-10 w-10 text-slate-300" /><p className="mt-3 font-semibold text-slate-600">Документов пока нет</p></div> : <div className="overflow-x-auto"><table className="min-w-[980px] w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr>{['Название', 'Файл', 'Дата загрузки', 'Статус', 'Дата подписи', ''].map((item, index) => <th key={`${item}-${index}`} className="px-4 py-3 font-bold">{item}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{documents.data.items.map((document) => { const busy = busyId === document.id; return <tr key={document.id}><td className="px-4 py-4 font-bold text-slate-900">{document.name}</td><td className="px-4 py-4 text-slate-600">{document.fileName}</td><td className="px-4 py-4 text-slate-600">{date(document.uploadedAt)}</td><td className="px-4 py-4"><span className={`rounded-full px-3 py-1 text-xs font-bold ${document.status === 'SIGNED' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-800'}`}>{document.status === 'SIGNED' ? 'Подписан' : 'Не подписан'}</span></td><td className="px-4 py-4 text-slate-600">{date(document.signedAt)}</td><td className="relative px-4 py-4 text-right"><details className="group relative inline-block text-left"><summary onClick={(event) => { if (busy) event.preventDefault(); }} className={`flex size-10 list-none items-center justify-center rounded-xl border border-slate-200 text-eco-900 transition [&::-webkit-details-marker]:hidden ${busy ? 'cursor-wait opacity-50' : 'cursor-pointer hover:bg-slate-50'}`} aria-label={`Действия с документом ${document.name}`}><MoreVertical size={20} /></summary><div className="absolute right-0 z-20 mt-2 w-64 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 shadow-xl"><button type="button" disabled={busy} className="flex w-full items-center gap-2 px-4 py-2.5 text-left font-semibold text-eco-800 hover:bg-slate-50 disabled:opacity-50" onClick={() => void run(document, 'open')}><Eye size={16} /> Открыть</button><button type="button" disabled={busy} className="flex w-full items-center gap-2 px-4 py-2.5 text-left font-semibold text-eco-800 hover:bg-slate-50 disabled:opacity-50" onClick={() => void run(document, 'download')}><Download size={16} /> Скачать</button>{document.status === 'UNSIGNED' && <button type="button" disabled={busy} className="flex w-full items-center gap-2 px-4 py-2.5 text-left font-semibold text-eco-800 hover:bg-slate-50 disabled:opacity-50" onClick={() => void run(document, 'sign')}><FileSignature size={16} /> Подписать ЭЦП</button>}{document.status === 'SIGNED' && <button type="button" disabled={busy} className="flex w-full items-center gap-2 px-4 py-2.5 text-left font-semibold text-eco-800 hover:bg-slate-50 disabled:opacity-50" onClick={() => void run(document, 'package')}><Download size={16} /> Скачать подписанный ZIP</button>}</div></details></td></tr>; })}</tbody></table></div>}
