...
import { Building2, ImagePlus, Pencil, Plus, RefreshCw, RotateCcw, Star, Trash2, UserMinus } from 'lucide-react';
import { useSearchParams } from 'react-router-dom';
import AuthenticatedImage from '../components/ui/AuthenticatedImage';
import Button from '../components/ui/Button';
import Modal from '../components/ui/Modal';
import LaboratoryForm from '../components/laboratories/LaboratoryForm';
import { validateLaboratoryLogo } from '../features/laboratories/components/LaboratoryLogoUploader';
import { useAuth } from '../contexts/AuthContext';
import { useToast } from '../hooks/useToast';
import {
  accreditationState, activateLaboratory, addLaboratoryEmployee, createLaboratory,
  deactivateLaboratoryEmployee, getEligibleLaboratoryEmployees, getLaboratories, getLaboratory, getLaboratoryEmployees,
  getLaboratoryLogoUrl, removeLaboratoryLogo, deactivateLaboratory, setDefaultLaboratory, updateLaboratory,
...
const accreditationLabels: Record<AccreditationStatus, string> = { VALID: 'Действует', EXPIRING: 'Скоро истекает', EXPIRED: 'Истёк', NOT_SPECIFIED: 'Не настроен' };
const employeeDefaults: LaboratoryEmployeeFormValues = { userId: null, fullName: '', position: '', email: '', phone: '', employeeNumber: '', qualification: '', role: '', canExecuteMeasurements: false, canApproveProtocols: false, canSignProtocols: false, active: true };
type EmployeeFilter = 'ACTIVE' | 'INACTIVE' | 'ALL';
type ConfirmAction = { type: 'DEFAULT' | 'DEACTIVATE_LAB' | 'ACTIVATE_LAB'; laboratory: LaboratoryListItem | LaboratoryDetails } | { type: 'DEACTIVATE'; employee: LaboratoryEmployee };
...
const LaboratorySettingsPage = () => {
  ...
  const listQuery = useQuery({ queryKey: ['laboratories', { page, size, search, status, accreditationFilter, city, defaultOnly, sort }], queryFn: ({ signal }) => getLaboratories({ page, size, search: search || undefined, status, accreditationStatus: accreditationFilter || undefined, city: city || undefined, defaultOnly, sort }, signal), placeholderData: keepPreviousData });
  ...
  const runConfirmation = async () => {
    if (!confirmAction || mutationBusy) return;
    if (confirmAction.type === 'DEACTIVATE_LAB' && confirmAction.laboratory.isDefault) {
      toast.error('Сначала назначьте другую лабораторию основной.');
    ...
    try {
      ...
      if (confirmAction.type === 'DEACTIVATE_LAB') await deactivateLaboratory(confirmAction.laboratory.id);
      if (confirmAction.type === 'ACTIVATE_LAB') await activateLaboratory(confirmAction.laboratory.id);
      ...
      toast.success({ DEFAULT: 'Лаборатория назначена основной', DEACTIVATE_LAB: 'Лаборатория деактивирована', ACTIVATE_LAB: 'Лаборатория активирована', DEACTIVATE: 'Сотрудник деактивирован' }[confirmAction.type]); setConfirmAction(null);
  ...
  const openEmployee = (employee?: LaboratoryEmployee) => {
    ...
    setEmployeeDraft(employee ? { userId: employee.userId, version: employee.version, fullName: employee.fullName, position: employee.position || '', email: employee.email || '', phone: employee.phone || '', employeeNumber: employee.employeeNumber || '', qualification: employee.qualification || '', role: employee.role || '', canExecuteMeasurements: employee.canExecuteMeasurements, canApproveProtocols: employee.canApproveProtocols, canSignProtocols: employee.canSignProtocols, active: employee.active } : { ...employeeDefaults });
  ...
  const validateLogo = (file: File) => {
    const validationError = validateLaboratoryLogo(file);
  ...
  const certificate = accreditationState(laboratory?.accreditationValidUntil);
  const stats = laboratory ? [['Сотрудники', laboratory.employeesCount], ['Протоколы', laboratory.protocolsCount], ['Средства измерений', laboratory.devicesCount]].filter(([, value]) => value !== undefined) : [];
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 md:grid-cols-3 xl:grid-cols-8">
      ...
      <select aria-label="Статус" value={status} onChange={(event) => updateUrl({ status: event.target.value, page: 0 })} className={inputClass}><option value="ACTIVE">Активные</option><option value="INACTIVE">Неактивные</option><option value="ALL">Все</option></select>
      ...
      <input aria-label="Город" value={city} onChange={(event) => updateUrl({ city: event.target.value || undefined, page: 0 })} placeholder="Город" className={inputClass} />
      <select aria-label="Основная лаборатория" value={defaultOnly ? 'true' : ''} onChange={(event) => updateUrl({ defaultOnly: event.target.value || undefined, page: 0 })} className={inputClass}><option value="">Все лаборатории</option><option value="true">Только основная</option></select>
    ...
    <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
      <aside className="space-y-3">
        {listQuery.isLoading ? Array.from({ length: 5 }).map((_, index) => <div key={index} className="h-28 animate-pulse rounded-xl bg-slate-100" />) : listQuery.data?.items.map((item) => <article key={item.id} className={`rounded-xl border bg-white p-4 ${selected === item.id ? 'border-eco-400 ring-2 ring-eco-100' : 'border-slate-200'}`}>
          <button type="button" className="w-full text-left focus:outline-none focus:ring-2 focus:ring-eco-500" onClick={() => updateUrl({ laboratoryId: item.id })}><div className="flex gap-2"><h2 className="flex-1 break-words font-black">{item.name}</h2>{item.isDefault && <span className="h-fit rounded-full bg-amber-50 px-2 py-1 text-xs font-bold text-amber-800">По умолчанию</span>}</div><p className="mt-2 break-words text-sm text-slate-600">{item.address || 'Адрес не указан'}</p><p className="mt-2 text-xs text-slate-600">Аттестат: {item.accreditationNumber || 'не указан'} · до {item.accreditationValidUntil || '—'}</p><p className="mt-1 text-xs text-slate-500">Обновлено: {item.updatedAt ? new Date(item.updatedAt).toLocaleDateString('ru-RU') : '—'}</p><div className="mt-2 flex flex-wrap gap-2"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs">{accreditationLabels[item.accreditationStatus]}</span><span className="rounded-full bg-slate-100 px-2 py-1 text-xs">Сотрудников: {item.employeesCount ?? 0}</span><span className={`rounded-full px-2 py-1 text-xs ${item.active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-700'}`}>{item.active ? 'Активная' : 'Неактивная'}</span></div></button>
        </article>)}
      ...
      <main>
        {detailsQuery.isLoading ? <div className="grid animate-pulse gap-4 rounded-xl border bg-white p-5 md:grid-cols-2">{Array.from({ length: 10 }).map((_, index) => <div key={index} className="h-16 rounded-xl bg-slate-100" />)}</div> : laboratory ? <div className="space-y-5">
          <section className="rounded-xl border bg-white p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-black">{laboratory.name}</h2>{laboratory.isDefault && <span className="rounded-full bg-amber-50 px-2 py-1 text-xs font-bold text-amber-800">По умолчанию</span>}<span className={`rounded-full px-2 py-1 text-xs font-bold ${laboratory.active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{laboratory.active ? 'Активная' : 'Неактивная'}</span></div><p className="mt-2 text-sm text-slate-600">{laboratory.address || 'Адрес не указан'}</p><p className="mt-1 text-sm font-semibold">Аттестат: {accreditationLabels[laboratory.accreditationStatus]}</p>{certificate.status === 'EXPIRING' && <p className="mt-1 text-sm text-amber-700">Срок действия аттестата истекает через {certificate.daysLeft} дней</p>}{certificate.status === 'EXPIRED' && <p className="mt-1 text-sm text-rose-700">Срок действия аттестата истёк</p>}</div><div className="flex flex-wrap gap-2">{permissions.canEdit && <Button type="button" variant="secondary" onClick={() => setEditing(laboratory)}><Pencil className="h-4 w-4" /> Изменить</Button>}{permissions.canSetDefault && !laboratory.isDefault && <Button type="button" variant="secondary" onClick={() => setConfirmAction({ type: 'DEFAULT', laboratory })}><Star className="h-4 w-4" /> Сделать основной</Button>}{permissions.canArchive && <Button type="button" variant="secondary" onClick={() => setConfirmAction({ type: 'DEACTIVATE_LAB', laboratory })}><UserMinus className="h-4 w-4" /> Деактивировать</Button>}{permissions.canRestore && <Button type="button" onClick={() => setConfirmAction({ type: 'ACTIVATE_LAB', laboratory })}><RefreshCw className="h-4 w-4" /> Активировать</Button>}</div></div></section>
          <section className="grid gap-4 rounded-xl border bg-white p-5 md:grid-cols-2"><h3 className="font-black md:col-span-2">Основные данные и аккредитация</h3>{[['Краткое название', laboratory.shortName], ['Юридическое название', laboratory.legalName], ['БИН', laboratory.bin], ['Город', laboratory.city], ['Телефон', laboratory.phone], ['Email', laboratory.email], ['Сайт', laboratory.website], ['Номер аттестата', laboratory.accreditationNumber], ['Дата выдачи', laboratory.accreditationIssuedAt], ['Дата окончания', laboratory.accreditationValidUntil], ['Кем выдан', laboratory.accreditationIssuedBy]].map(([label, value]) => <div key={label}><p className="text-xs text-slate-500">{label}</p><p className="break-words font-semibold">{value || '—'}</p></div>)}</section>
          <section className="grid gap-4 rounded-xl border bg-white p-5 md:grid-cols-2"><h3 className="font-black md:col-span-2">Руководство</h3><div><p className="text-xs text-slate-500">Директор</p><p className="font-semibold">{laboratory.directorName || employees.find((item) => item.id === laboratory.directorId)?.fullName || 'Не назначен'}</p>{laboratory.directorId && !employees.some((item) => item.id === laboratory.directorId && item.active) && <p className="mt-1 text-xs text-amber-700">Назначенный руководитель неактивен. Выберите другого сотрудника.</p>}</div><div><p className="text-xs text-slate-500">Заведующий лабораторией</p><p className="font-semibold">{laboratory.laboratoryHeadName || employees.find((item) => item.id === laboratory.laboratoryHeadId)?.fullName || 'Не назначен'}</p>{laboratory.laboratoryHeadId && !employees.some((item) => item.id === laboratory.laboratoryHeadId && item.active) && <p className="mt-1 text-xs text-amber-700">Назначенный руководитель неактивен. Выберите другого сотрудника.</p>}</div></section>
