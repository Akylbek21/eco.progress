...
const protocolStatuses = new Set<ProtocolStatus>(['DRAFT', 'READY_TO_SIGN', 'CALCULATED', 'READY', 'READY_FOR_APPROVAL', 'UNDER_REVIEW', 'RETURNED_FOR_CORRECTION', 'NEEDS_REVISION', 'APPROVED', 'SIGNED', 'REPLACED', 'CANCELLED', 'ARCHIVED']);
const visibleStatusFilters: ProtocolStatus[] = ['DRAFT', 'READY_TO_SIGN', 'SIGNED', 'ARCHIVED'];
...
const ProtocolsPage = () => {
  ...
  const remove = async () => { if (!deleteTarget || busyId) return; setBusyId(deleteTarget.id); try { await protocolService.deleteProtocol(deleteTarget.id, Number(deleteTarget.version)); setDeleteTarget(null); toast.success('Протокол удалён'); await queryClient.invalidateQueries({ queryKey: ['protocols'] }); } catch (error) { toast.error(parseApiError(error, 'Не удалось удалить протокол').message); } finally { setBusyId(''); } };
