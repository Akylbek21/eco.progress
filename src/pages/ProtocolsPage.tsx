...
const ProtocolsPage = () => {
  ...
  return <div className="space-y-6 pb-10">
    ...
    <CreateProtocolWizardModal open={createModalOpen} orderId={params.get('orderId') || ''} orderServiceItemId={params.get('orderServiceItemId') || ''} onClose={() => setCreateModalOpen(false)} onCreated={(protocol) => { setCreateModalOpen(false); void queryClient.invalidateQueries({ queryKey: ['protocols'] }); if (protocol.orderId) void queryClient.invalidateQueries({ queryKey: ['order', String(protocol.orderId)] }); navigate(`/staff/protocols/${protocol.id}`); }} />
  </div>;
};
