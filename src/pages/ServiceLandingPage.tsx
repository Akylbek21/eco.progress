...
const landingPages: Record<string, ServiceLanding> = {
  ...
  'waste-transportation': {
    ...
    title: 'Вывоз отходов',
    metaTitle: 'Вывоз отходов в Шымкенте | ecoprogress.kz',
    description: 'Сбор, транспортировка, утилизация, переработка и документы по отходам для организаций Шымкента.',
    h1: 'Вывоз и утилизация отходов в Шымкенте',
    intro: 'Организуем сбор отходов, транспортировку, утилизацию и документы для бизнеса в Шымкенте.',
  ...
  'waste-recycling': {
    ...
    metaTitle: 'Утилизация отходов в Шымкенте | ecoprogress.kz',
    description: 'Утилизация и переработка отходов в Шымкенте с документальным сопровождением для организаций.',
    h1: 'Утилизация и переработка отходов в Шымкенте',
    intro: 'Поможем подобрать законный способ утилизации или переработки отходов в Шымкенте и подготовить документы.',
...
Object.assign(landingPages, {
  ndv: {
    slug: 'ndv',
    title: 'Проект нормативов допустимых выбросов',
    metaTitle: 'Разработка проекта НДВ в Казахстане | ECOPROGRESS',
    description: 'Разработка проекта нормативов допустимых выбросов для предприятий Казахстана: инвентаризация источников, расчеты и сопровождение.',
    h1: 'Разработка проекта нормативов допустимых выбросов',
    intro: 'Проведем инвентаризацию источников, выполним расчеты выбросов и подготовим материалы НДВ для предприятия.',
    forWhom: ['производственным предприятиям', 'объектам с котельными', 'складам и базам', 'компаниям при реконструкции'],
    includes: ['инвентаризация источников', 'расчет выбросов', 'подготовка проекта НДВ', 'проверка исходных данных', 'сопровождение процедуры'],
    documents: ['реквизиты и адрес объекта', 'описание технологии', 'перечень оборудования', 'предыдущие проекты и разрешения при наличии'],
    duration: 'Срок зависит от количества источников, полноты исходных данных и категории объекта.',
    faq: [['Когда нужен проект НДВ?', 'При наличии стационарных источников выбросов и для обоснования экологических нормативов объекта.'], ['Можно начать без старого проекта?', 'Да, начнем с инвентаризации и списка необходимых исходных данных.']],
    leadDefault: 'Проект НДВ',
    image: '/cottonbro.jpg',
  },
  puo: {
    slug: 'puo',
    title: 'Программа управления отходами',
    metaTitle: 'Разработка программы управления отходами | ECOPROGRESS',
    description: 'Разработка программы управления отходами для предприятий: анализ образования отходов, мероприятия, показатели и сопровождение по РК.',
    h1: 'Разработка программы управления отходами',
    intro: 'Проанализируем виды и объемы отходов, подготовим целевые показатели, мероприятия и программу управления отходами.',
    forWhom: ['промышленным объектам', 'строительным компаниям', 'предприятиям с регулярными отходами', 'объектам перед проверкой'],
    includes: ['анализ потоков отходов', 'целевые показатели', 'мероприятия по сокращению', 'план передачи и утилизации', 'подготовка программы ПУО'],
    documents: ['перечень и объемы отходов', 'паспорта отходов', 'договоры и акты', 'описание производственных процессов'],
    duration: 'Срок определяется количеством видов отходов и готовностью исходных данных.',
    faq: [['Какие отходы включают в ПУО?', 'В программу включают отходы, фактически образующиеся на объекте, с их объемами и способами обращения.'], ['Поможете с паспортами отходов?', 'Да, при необходимости подготовку паспортов можно включить в состав работ.']],
    leadDefault: 'Программа управления отходами',
    image: '/utilizacija-othodov-3.jpg',
  },
  ovos: {
    slug: 'ovos',
    title: 'Оценка воздействия на окружающую среду',
    metaTitle: 'Разработка ОВОС для проекта в Казахстане | ECOPROGRESS',
    description: 'Подготовка ОВОС и материалов экологической оценки для строительства, реконструкции и запуска объектов в Казахстане.',
    h1: 'Разработка оценки воздействия на окружающую среду',
    intro: 'Оценим планируемое воздействие проекта, подготовим материалы ОВОС и сопроводим экологическую процедуру.',
    forWhom: ['застройщикам', 'новым производствам', 'проектам реконструкции', 'объектам при изменении технологии'],
    includes: ['анализ проектных решений', 'оценка видов воздействия', 'мероприятия по снижению влияния', 'подготовка материалов ОВОС', 'сопровождение процедуры'],
    documents: ['описание проекта', 'генеральный план', 'технологические решения', 'данные по выбросам, воде и отходам'],
    duration: 'Срок зависит от масштаба проекта, состава воздействий и полноты проектных данных.',
    faq: [['Когда нужна ОВОС?', 'Необходимость и формат оценки определяются видом намечаемой деятельности и характеристиками проекта.'], ['Можно проверить проект до начала работ?', 'Да, предварительный аудит помогает определить маршрут и недостающие исходные данные.']],
    leadDefault: 'ОВОС',
  ...
  'waste-management': {
    ...
    metaTitle: 'Утилизация отходов для бизнеса в Шымкенте | ECOPROGRESS',
    description: 'Утилизация, вывоз, переработка и документы по отходам для бизнеса в Шымкенте: паспорта, договоры и акты.',
    h1: 'Утилизация отходов и документы в Шымкенте',
    intro: 'Поможем определить вид отходов, организовать передачу на утилизацию в Шымкенте и подготовить подтверждающие документы.',
...
const ServiceLandingPage = ({ slug }: { slug: string }) => {
  ...
  const schema = [
    ...
    },
    {
      ...
      areaServed: ['waste-transportation', 'waste-recycling', 'waste-management'].includes(page.slug) ? 'Шымкент' : 'Казахстан',
