...
const ServiceLandingPage = ({ slug }: { slug: string }) => {
  ...
  const staticContent = import.meta.env.DEV && service ? serviceContentMap.get(service.slug) : undefined;
  ...
  return <div className="bg-white">
    ...
    <section className="relative isolate overflow-hidden bg-eco-900 px-4 py-16 text-white sm:px-8 sm:py-24">
      ...
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <div><p className="text-sm font-bold uppercase tracking-[0.2em] text-accent">{service.category}</p><h1 className="mt-4 text-4xl font-bold leading-tight sm:text-6xl">{content.hero.title}</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-white/76">{content.hero.subtitle}</p><p className="mt-4 text-xl font-bold text-accent">{formatKztPrice(service.pricing)}</p><div className="mt-8 grid gap-3 sm:flex sm:flex-wrap"><Button asChild className="w-full bg-accent text-eco-900 hover:bg-accent/90 sm:w-auto"><a href="#lead">Оставить заявку</a></Button><WhatsAppButton label="Оставить заявку через WhatsApp" message={createBlankWhatsAppRequestMessage(service.title)} className="w-full sm:w-auto" /></div></div>
    ...
    <section id="lead" className="px-4 py-16 sm:px-8 sm:py-20"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.8fr_1.2fr]"><div><h2 className="text-3xl font-bold text-eco-900">Получить консультацию по услуге</h2><p className="mt-4 leading-7 text-slate-600">Опишите объект и задачу. Специалист проверит применимость требований, перечень исходных документов и условия выполнения работ.</p></div><WhatsAppLeadForm source={`service_bottom_${service.slug}_whatsapp`} defaultService={service.title} /></div></section>
  </div>;
};
