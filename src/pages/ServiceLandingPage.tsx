  ...
  return <div className="bg-white">
    ...
    <section className="relative isolate overflow-hidden bg-eco-900 px-4 py-16 text-white sm:px-8 sm:py-24">
      ...
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <div><p className="text-sm font-bold uppercase tracking-[0.2em] text-accent">{content.hero.eyebrow || service.category}</p><h1 className="mt-4 text-4xl font-bold leading-tight sm:text-6xl">{content.hero.title}</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-white/76">{content.hero.subtitle}</p>{content.hero.benefits?.length ? <ul className="mt-6 grid gap-3" aria-label="Преимущества">{content.hero.benefits.map((benefit) => <li key={benefit} className="flex items-center gap-2 text-sm font-semibold text-white/90"><CheckCircle2 className="shrink-0 text-accent" size={19} aria-hidden="true" /><span>{benefit}</span></li>)}</ul> : null}<p className="mt-4 text-xl font-bold text-accent">{content.summary.priceText}</p><div className="mt-8 grid gap-3 sm:flex sm:flex-wrap"><Button asChild className="w-full bg-accent text-eco-900 hover:bg-accent/90 sm:w-auto"><a href="#lead">{content.hero.primaryCta}</a></Button><WhatsAppButton label="Отправить документы в WhatsApp" message={content.commercial.documentsWhatsAppMessage} className="w-full sm:w-auto" /></div></div>
        <LeadForm source={`service_${service.slug}_hero`} formId={`service_${service.slug}_hero`} ctaId="hero_calculate" sourcePage={`/services/${service.slug}`} title="Короткая заявка" submitLabel={content.hero.primaryCta} compact defaultService={service.title} serviceSlug={service.slug} />
      </div>
    </section>
