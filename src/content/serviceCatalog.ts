...
export type ServicePageType = 'service' | 'direction-overview';

export const serviceGroups: Array<{ category: ServiceCategory; title: string; description: string }> = [
  { category: 'Проектирование', title: 'Экологическое проектирование', description: 'Проекты и программы для строительства и действующих объектов.' },
  { category: 'Разрешения', title: 'Разрешения', description: 'Разрешительные материалы и сопровождение экологических процедур.' },
  { category: 'Лаборатория', title: 'Лабораторные исследования', description: 'Отбор проб, измерения и протоколы по согласованным показателям.' },
  { category: 'Отходы', title: 'Отходы', description: 'Документы, вывоз, утилизация и размещение отходов.' },
  { category: 'Предприятия', title: 'Сопровождение предприятий', description: 'Регулярная экологическая работа и подготовка к проверкам.' },
];
export const getServiceGroup = (category: ServiceCategory) => serviceGroups.find((group) => group.category === category)!;
export const serviceGroupAnchor = (category: ServiceCategory) => `direction-${serviceGroups.findIndex((group) => group.category === category) + 1}`;
export const getServiceCatalogGroup = (service: { slug: string; category: ServiceCategory }): ServiceCategory =>
  service.slug === 'waste-passport' ? 'Отходы' : service.category;
...
export const serviceCatalog: ServiceCatalogItem[] = [
  item({ slug: 'ecological-documents', title: 'Экологические документы', category: 'Проектирование', pageType: 'direction-overview', shortDescription: 'Обзор экологических документов, программ, отчётов и разрешительных материалов для бизнеса.', image: '/cottonbro.jpg', pricing: { priceFrom: 180000, currency: 'KZT', calculatorBasePrice: 180000, requiresCalculation: true }, duration: { minDays: 10, text: 'от 10 рабочих дней' }, showOnHome: true, showInCalculator: false, showInTariffs: true, relatedServiceSlugs: ['environmental-design', 'environmental-permits', 'ecological-support'], relatedArticleSlugs: ['ekologicheskie-dokumenty-too-kazakhstan'] }),
  item({ slug: 'environmental-design', title: 'Экологическое проектирование', category: 'Проектирование', pageType: 'direction-overview', shortDescription: 'Обзор экологических проектов для строительства и действующих объектов.', pricing: { priceFrom: 180000, priceTo: 350000, currency: 'KZT', calculatorBasePrice: 180000, requiresCalculation: true }, showInCalculator: false, showInTariffs: true, relatedServiceSlugs: ['ndv', 'ovos', 'puo'], relatedArticleSlugs: [] }),
  ...
  item({ slug: 'ndv', title: 'Проект нормативов допустимых выбросов', shortTitle: 'Проект НДВ', category: 'Проектирование', shortDescription: 'Инвентаризация источников, расчёты выбросов и подготовка проекта НДВ.', relatedServiceSlugs: ['environmental-design', 'environmental-permits', 'ecological-support'], relatedArticleSlugs: [] }),
  item({ slug: 'puo', title: 'Программа управления отходами', shortTitle: 'ПУО', category: 'Проектирование', shortDescription: 'Анализ потоков отходов, целевые показатели и программа управления отходами.', relatedServiceSlugs: ['ecological-documents', 'waste-recycling', 'ecological-support'], relatedArticleSlugs: [] }),
  item({ slug: 'ovos', title: 'Оценка воздействия на окружающую среду', shortTitle: 'ОВОС', category: 'Проектирование', shortDescription: 'Подготовка ОВОС и материалов экологической оценки для проектов.', relatedServiceSlugs: ['environmental-design', 'environmental-permits', 'ecological-support'], relatedArticleSlugs: [] }),
  item({ slug: 'environmental-permits', title: 'Экологические разрешения и декларации', category: 'Разрешения', shortDescription: 'Подготовка разрешительных материалов и сопровождение процедур по требованиям РК.', relatedServiceSlugs: ['environmental-design', 'ecological-documents', 'ecological-support'], relatedArticleSlugs: [] }),
  ...
  item({ slug: 'waste-management', title: 'Комплексное обращение с отходами', category: 'Отходы', shortDescription: 'Вывоз, утилизация и документальное сопровождение отходов в рамках одной заявки.', image: '/jose.jpg', areaServed: { type: 'SHYMKENT_ONLY', regions: ['shymkent'], description: 'Комплексная услуга доступна в Шымкенте.', remote: false, onSite: true }, pricing: { priceFrom: 120000, currency: 'KZT', calculatorBasePrice: 120000, requiresCalculation: true }, showInCalculator: true, showInTariffs: true, relatedServiceSlugs: ['waste-transportation', 'waste-recycling'], relatedArticleSlugs: [] }),
  ...
  item({ slug: 'ecological-support', title: 'Эколог на аутсорсинге', category: 'Предприятия', shortDescription: 'Внешняя экологическая команда для регулярного контроля документов, ПЭК, отчётности, отходов и экологических рисков предприятия.', image: '/media/ecologist-outsourcing-hero.png', pricing: { priceFrom: 350000, currency: 'KZT', calculatorBasePrice: 350000, priceText: 'в месяц', requiresCalculation: true }, showOnHome: true, showInCalculator: true, showInTariffs: true, relatedServiceSlugs: ['environmental-audit', 'ecological-documents', 'program-pek', 'report-pek'], relatedArticleSlugs: [], seo: { title: 'Эколог на аутсорсинге в Казахстане | Экологическое сопровождение предприятий | ECOPROGRESS', description: 'Комплексное экологическое сопровождение предприятий по Казахстану: ПЭК, экологическая отчетность, отходы, разрешения, лабораторный контроль, проверки и экологические риски. Абонентское сопровождение ECOPROGRESS.' } }),
];
