...
export const isPublishableCaseStudy = (item: CaseStudy): boolean => {
  const requiredText = [item.id, item.slug, item.title, item.service, item.serviceSlug, item.industry, item.city, item.region, item.objectType, item.objectCategory, item.problem, item.initialData, item.result, item.completedAt, item.updatedAt];
  return item.status === 'published'
    ...
    && (item.images ?? []).every((image) => Boolean(image.url.trim()) && Boolean(image.alt.trim())
