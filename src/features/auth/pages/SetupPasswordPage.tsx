...
export default function SetupPasswordPage() {
  ...
  useEffect(() => {
    if (!token) {
      showError('Токен установки пароля отсутствует');
      return;
    }
    ...
    const validateToken = async () => {
      try {
        await api.post('/auth/validate-setup-token', { token });
        setIsTokenValid(true);
      } catch (err) {
        showError('Ссылка для установки пароля недействительна или истекла');
          ...
          <Card sx={{ width: '100%' }}>
            <CardContent>
              <Alert severity="error">
                <Typography>Ссылка для установки пароля недействительна или истекла.</Typography>
                <Typography variant="body2" sx={{ mt: 1 }}>
                  Попросите администратора отправить вам новое приглашение.
                </Typography>
              </Alert>
              <Button 
                fullWidth 
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
        <Card sx={{ width: '100%', boxShadow: 3 }}>
          <CardContent sx={{ p: 4 }}>
            <Stack spacing={3}>
              <Box>
                <Typography variant="h5" fontWeight={800}>Установка пароля</Typography>
