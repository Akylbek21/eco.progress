...
import { mapProtocolWizardToRequest } from '../mappers/mapProtocolWizardToRequest';
...
export type CreateProtocolWizardModalProps = { open: boolean; onClose: () => void; onCreated: (protocol: Protocol) => void };

const CreateProtocolWizardModal = ({ open, onClose, onCreated }: CreateProtocolWizardModalProps) => {
  ...
  const values = watch();
  const [step,setStep] = useState(0); const [maxVisited,setMaxVisited] = useState(0); const [error,setError] = useState('');
  ...
  const submittingRef = useRef(false); const titleRef = useRef<HTMLElement | null>(null);
  ...
  const devicesQuery = useQuery({ queryKey: ['measurement-devices','protocol-wizard'], queryFn: () => getAvailableMeasurementDevices(), enabled: open });
  ...
  const objects = (objectsQuery.data || []).filter((item) => item.status === 'ACTIVE' && !item.virtual); const employees = (employeesQuery.data || []).filter((item) => item.active); const devices = devicesQuery.data || [];
  ...
  useEffect(() => { if (!open) return; const stored = sessionStorage.getItem(DRAFT_KEY); setDraftPrompt(Boolean(stored)); setError(''); setSuccess(null); }, [open]);
  ...
  const mutation = useMutation({ mutationFn: (payload: ReturnType<typeof mapProtocolWizardToRequest>) => protocolService.quickCreateProtocol(payload) });
  ...
  const changeCompany = (id: string) => { setValue('companyId',id,{ shouldDirty:true }); setValue('objectId','',{ shouldDirty:true }); };
  ...
  const createProtocol = async () => { if (submittingRef.current || mutation.isPending) return; if (issues.length) { setError('Исправьте блокирующие ошибки перед созданием.'); setStep(7); return; } submittingRef.current = true; setError(''); try { const created = await mutation.mutateAsync(mapProtocolWizardToRequest(getValues())); if (!created.id) throw new Error('Backend не вернул идентификатор созданного протокола.'); sessionStorage.removeItem(DRAFT_KEY); setSuccess(created); window.setTimeout(() => onCreated(created),400); } catch (requestError) { const normalized = normalizeProtocolError(requestError); const apiError = normalizeApiError(requestError,'Не удалось создать протокол.'); setError(normalized.resultIndex === undefined ? apiError.message : `Строка ${normalized.resultIndex + 1}: ${apiError.message}`); if (normalized.resultIndex !== undefined) setStep(5); } finally { submittingRef.current = false; } };
