...
export default function BasicDataStep({ templates, companies, objects, lockedCompanyId, onCompanyChange }: {
  templates: ProtocolTemplate[]; companies: Company[]; objects: CompanyObject[]; lockedCompanyId?: string; onCompanyChange: (id: string) => void;
}) {
  ...
  return <section className="space-y-5">
    ...
    {pekContext && <div className="rounded-xl border border-eco-200 bg-eco-50 p-4 text-sm font-semibold text-eco-900">Создаётся для задачи ПЭК{form.sourceNumber ? `: источник ${form.sourceNumber}` : ''}. Данные из ПЭК уже заполнены.</div>}
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <label className="text-sm font-semibold">Компания<select value={form.companyId} disabled={Boolean(lockedCompanyId)} onChange={(event) => onCompanyChange(event.target.value)} className={`${inputClass} mt-1 disabled:bg-slate-100`}><option value="">Выберите компанию</option>{companies.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
