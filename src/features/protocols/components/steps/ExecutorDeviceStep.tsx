...
export default function ExecutorDeviceStep({ laboratories, employees, devices, laboratoriesLoading, employeesLoading, devicesLoading, onLaboratoryChange }: {
  ...
  return <section className="space-y-5"><div><h3 id="wizard-step-title" tabIndex={-1} className="text-xl font-black">Кто проводил измерения</h3><p className="mt-1 text-sm text-slate-500">Лаборатория и текущий сотрудник подставляются автоматически, если они настроены.</p></div><div className="grid gap-4 rounded-2xl border border-slate-200 p-4 md:grid-cols-2 sm:p-5">
    <label className="text-sm font-semibold">Лаборатория *<select value={form.laboratoryId} aria-invalid={Boolean(errors.laboratoryId)} disabled={laboratoriesLoading} onChange={(event) => onLaboratoryChange(event.target.value)} className={`${inputClass} mt-1 disabled:bg-slate-100 ${errors.laboratoryId ? 'border-rose-400' : ''}`}><option value="">{laboratoriesLoading ? 'Загрузка лабораторий…' : 'Выберите лабораторию'}</option>{laboratories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>{errors.laboratoryId?.message && <span className="mt-1 block text-xs font-semibold text-rose-700">{errors.laboratoryId.message}</span>}</label>
    <label className="text-sm font-semibold">Исполнитель *<select value={form.executorId} aria-invalid={Boolean(errors.executorId)} disabled={!form.laboratoryId || employeesLoading} onChange={(event) => setValue('executorId', event.target.value, { shouldDirty: true })} className={`${inputClass} mt-1 disabled:bg-slate-100 ${errors.executorId ? 'border-rose-400' : ''}`}><option value="">{employeesLoading ? 'Загрузка сотрудников…' : form.laboratoryId ? 'Выберите сотрудника' : 'Сначала выберите лабораторию'}</option>{employees.map((item) => <option key={item.executorId} value={item.executorId}>{item.fullName}</option>)}</select>{errors.executorId?.message && <span className="mt-1 block text-xs font-semibold text-rose-700">{errors.executorId.message}</span>}{form.laboratoryId && !employeesLoading && employees.length === 0 && <span className="mt-1 block text-xs font-normal text-amber-700">В лаборатории нет доступных активных сотрудников.</span>}</label>
    <label className="text-sm font-semibold md:col-span-2">Прибор по умолчанию (можно выбрать позже)<select value={form.defaultMeasurementDeviceId} disabled={devicesLoading} onChange={(event) => setValue('defaultMeasurementDeviceId', event.target.value, { shouldDirty: true })} className={`${inputClass} mt-1 disabled:bg-slate-100`}><option value="">{devicesLoading ? 'Загрузка приборов…' : 'Выбрать отдельно для каждого показателя'}</option>{devices.map((item) => <option key={item.id} value={item.id}>{item.name} · поверка до {date(item.verificationValidUntil)}</option>)}</select><span className="mt-1 block text-xs font-normal text-slate-500">Выбранный прибор автоматически добавится ко всем строкам без прибора.</span></label>
  </div></section>;
}
