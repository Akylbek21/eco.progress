...
const ProtocolWizardFooter = ({ step, total, submitting, canContinue, canSaveDraft, retrying = false, saveState, nextLabel = 'Продолжить', createLabel = 'Создать протокол', onBack, onNext, onCreate, onSaveDraft }: Props) => (
  <footer className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-white px-4 py-4 sm:px-6">
    ...
    <div className="flex flex-wrap items-center justify-end gap-3">
      ...
      <Button type="button" variant="secondary" disabled={!canSaveDraft || submitting} onClick={onSaveDraft}>{submitting ? 'Сохраняем…' : retrying ? 'Повторить сохранение' : 'Сохранить сейчас'}</Button>
    {step < total - 1
