...
const ProtocolNextStepCard = ({ protocol, missing }: Props) => {
  ...
  if (['READY', 'READY_TO_SIGN', 'READY_FOR_APPROVAL', 'APPROVED'].includes(status)) { title = 'Протокол готов к подписи'; text = 'Подпишите протокол через NCALayer.'; }
  if (status === 'UNDER_REVIEW') { title = 'Протокол на проверке'; text = 'Это старый протокол с прежним процессом согласования.'; }
  if (status === 'NEEDS_REVISION' || status === 'RETURNED_FOR_CORRECTION') { title = 'Нужно исправить'; text = revisionComment ? `Причина: «${revisionComment}»` : 'Исправьте отмеченные данные и снова подготовьте протокол к подписи.'; }
