...
const ProtocolNextStepCard = ({ protocol, missing }: Props) => {
  ...
  return (
    <section className={`rounded-2xl border p-5 shadow-sm ${missing.length && ['DRAFT', 'RETURNED_FOR_REVISION'].includes(status) ? 'border-amber-200 bg-amber-50' : 'border-emerald-200 bg-emerald-50'}`}>
      <h2 className="text-lg font-black text-slate-950">{title}</h2>
      <p className="mt-1 text-sm text-slate-700">{text}</p>
      {missing.length > 0 && ['DRAFT', 'RETURNED_FOR_REVISION'].includes(status) && (
