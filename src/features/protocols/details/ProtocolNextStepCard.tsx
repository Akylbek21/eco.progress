...
const ProtocolNextStepCard = ({ protocol, missing }: Props) => {
  ...
  if (status === 'CALCULATED') {
    title = missing.length ? `До следующего этапа осталось: ${missing.length}` : 'Результаты рассчитаны';
    text = missing.length
      ? 'Исправьте перечисленные данные. После этого появится кнопка «Отправить на утверждение».'
      : 'Все обязательные данные заполнены. Используйте доступное действие для отправки на утверждение.';
  }
  if (status === 'READY_FOR_APPROVAL') { title = 'Протокол готов к утверждению'; text = 'Доступное действие определено backend и показано в карточке протокола.'; }
  if (status === 'APPROVED') { title = 'Утверждён, ожидает подписи'; text = 'Протокол ещё не завершён. Его можно подписать или вернуть на доработку, если backend разрешил действие.'; }
  ...
  if (status === 'SIGNED') { title = 'Подписан / завершён'; text = 'Сохранённый подписанный документ готов к просмотру и скачиванию.'; }
  if (status === 'PUBLISHED') { title = 'Опубликован клиенту'; text = 'Подписанный протокол опубликован и доступен клиенту.'; }
  ...
  return (
    <section className={`rounded-2xl border p-5 shadow-sm ${missing.length && ['DRAFT', 'CALCULATED', 'NEEDS_REVISION'].includes(status) ? 'border-amber-200 bg-amber-50' : 'border-emerald-200 bg-emerald-50'}`}>
      <h2 className="text-lg font-black text-slate-950">{title}</h2>
      <p className="mt-1 text-sm text-slate-700">{text}</p>
      {missing.length > 0 && ['DRAFT', 'CALCULATED', 'NEEDS_REVISION'].includes(status) && (
        <details open={status === 'CALCULATED'} className="mt-3 text-sm text-amber-950">
          <summary className="cursor-pointer font-bold">Что нужно исправить</summary>
