    ...
    <div ref={wrapperRef} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-30 mt-2 w-72 overflow-hidden rounded-xl border border-slate-200 bg-white p-2 text-sm shadow-xl">
          {((protocol.hasDocx && permissions.canRegenerateDocx) || (!protocol.hasDocx && permissions.canGenerateDocx)) && <button type="button" onClick={() => run(onGenerateDocx)} className="w-full rounded-lg px-3 py-2 text-left font-semibold hover:bg-slate-50">{protocol.hasDocx ? 'Перегенерировать DOCX' : 'Сгенерировать DOCX'}</button>}
          {((protocol.hasPdf && permissions.canRegeneratePdf) || (!protocol.hasPdf && permissions.canGeneratePdf)) && <button type="button" onClick={() => run(onGeneratePdf)} className="w-full rounded-lg px-3 py-2 text-left font-semibold hover:bg-slate-50">{protocol.hasPdf ? 'Перегенерировать PDF' : 'Сгенерировать PDF'}</button>}
          ...
          {(permissions.canReturnToDraft || permissions.canReturnForRevision) && <button type="button" onClick={() => run(onReturnForRevision)} className="w-full rounded-lg px-3 py-2 text-left font-semibold text-amber-800 hover:bg-amber-50">Вернуть на доработку</button>}
