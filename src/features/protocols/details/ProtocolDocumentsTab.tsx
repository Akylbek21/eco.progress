...
const ProtocolDocumentsTab = ({ protocol, busy, canGenerate, canSign, onGenerate, onDocx, onPdf, onSign }: Props) => (
  <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-black">Документы протокола</h2><p className="mt-1 text-sm text-slate-500">Версия документа: {protocol.version || 1}</p></div>{(!protocol.hasDocx || !protocol.hasPdf) && <button type="button" disabled={busy || !canGenerate} title={!canGenerate ? 'Предпросмотр недоступен для текущего статуса протокола' : undefined} onClick={onGenerate} className="rounded-xl bg-eco-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">Сформировать предпросмотр</button>}</div>
    <div className="mt-5 divide-y divide-slate-100">
      ...
      <div className="flex items-center justify-between gap-4 py-4"><div><h3 className="font-black">Подписи сотрудников</h3><p className="mt-1 text-sm text-slate-500">Подписей: {protocol.signatureCount} из {protocol.maxSignatures}. Полный список показан в карточке протокола.</p></div><button type="button" disabled={busy || !canSign} onClick={onSign} className="font-bold text-eco-700 disabled:cursor-not-allowed disabled:text-slate-400">{protocol.signedByCurrentUser ? 'Вы подписали' : 'Подписать'}</button></div>
    </div>
  </section>
);
