...
const ProtocolDetailsView = ({ protocol, permissions, missing, workflowErrors, busy, signing, onBack, onEdit, onCalculate, onCheckNormatives, onReady, onApprove, onSign, onPublish, onPreview, onGenerateDocx, onGeneratePdf, onDocx, onPdf, onCorrection, onReturnForRevision, onCancel, onArchive, onReplacement, initialTab = 'results' }: Props) => {
  ...
  const effectivePermissions = signBlocked ? { ...permissions, canSign: false } : permissions;
  ...
  return (
    <div className="space-y-4 pb-24">
      <ProtocolHeader protocol={protocol} permissions={permissions} busy={busy} primaryDisabled={primaryBlocked} primaryLabel={primary.label} onBack={onBack} onPrimary={runPrimary} onDocx={onDocx} onGenerateDocx={onGenerateDocx} onGeneratePdf={onGeneratePdf} onCorrection={onCorrection} onReturnForRevision={onReturnForRevision} onCancel={onCancel} onArchive={onArchive} onHistory={() => setActiveTab('history')} />
      ...
      <ProtocolSignaturesCard protocol={protocol} permissions={effectivePermissions} signing={signing} onSign={onSign} />
      ...
      {activeTab === 'documents' && <ProtocolDocumentsTab protocol={protocol} busy={busy} canGenerateDocx={permissions.canGenerateDocx} canRegenerateDocx={permissions.canRegenerateDocx} canGeneratePdf={permissions.canGeneratePdf} canRegeneratePdf={permissions.canRegeneratePdf} canPreview={protocol.status === 'SIGNED' ? permissions.canDownloadPdf : permissions.canGeneratePreview} canDownloadPdf={permissions.canDownloadPdf} canDownloadDocx={permissions.canDownloadDocx} canSign={effectivePermissions.canSign} onPreview={onPreview} onGenerateDocx={onGenerateDocx} onGeneratePdf={onGeneratePdf} onDocx={onDocx} onPdf={onPdf} onSign={onSign} />}
