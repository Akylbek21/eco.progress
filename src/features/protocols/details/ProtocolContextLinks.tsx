...
const ProtocolContextLinks = ({ protocol }: { protocol: Protocol }) => {
  ...
  return (
    <section className="rounded-2xl border border-eco-200 bg-eco-50/50 p-5">
      ...
      {pekLinks.length > 0 && <div className="mt-3 space-y-3">{pekLinks.map((pek, index) => { const query = new URLSearchParams({ tab: 'sources', protocolId: String(protocol.id), ...(pek.controlItemId ? { controlItemId: String(pek.controlItemId) } : {}), ...(pek.monitoringPointId ? { monitoringPointId: String(pek.monitoringPointId) } : {}) }); return <article key={`${pek.reportId}-${index}`} className="rounded-xl border bg-white p-4 text-sm"><p><strong>Программа:</strong> {pek.programNumber || `Программа №${pek.programId}`}</p><p><strong>Отчёт:</strong> {pek.reportName || pek.reportPeriod || `Отчёт №${pek.reportId}`}</p><p><strong>Контроль:</strong> {pek.controlItemName || '—'}</p><p><strong>Точка:</strong> {pek.monitoringPointName || '—'}</p><Link className="mt-3 inline-block font-bold text-eco-700" to={`/staff/pek/reports/${pek.reportId}?${query}`}>Открыть отчёт ПЭК</Link></article>; })}</div>}
