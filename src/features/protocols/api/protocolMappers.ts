...
export const mapProtocolFormToPatchRequest = (
  ...
  testing: {
    ...
    sampleNumber: ambient ? null : optionalText(payload.sampleNumber),
    samplingPlace: ambient ? null : optionalText(payload.samplingPlace ?? payload.measurementPlace),
    samplingDepth: ambient ? null : optionalText(payload.samplingDepth),
  ...
  environment: mapProtocolEnvironmentToRequest(payload.environment, ambient ? {
    ...(payload.conditions ?? {}), sampleNumber: null, samplingDepth: null, samplingPlace: null,
  } : payload.conditions),
