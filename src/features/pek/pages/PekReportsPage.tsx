...
const PekReportsPage = () => {
  ...
  return <div className="space-y-5">
    ...
    <section className="grid gap-3 rounded-2xl border bg-white p-4 md:grid-cols-2 xl:grid-cols-3 min-[1800px]:grid-cols-5">
    ...
    {!companyId || !objectId ? <PekState title="Выберите компанию и объект" message="После выбора будет загружен список отчётов ПЭК." /> : reports.isLoading ? <PekLoading /> : reports.isError ? <PekQueryError error={reports.error} resource="Отчёты ПЭК" retry={() => void reports.refetch()} /> : !reports.data?.content.length ? <PekState title="Для выбранного объекта отчётов нет" /> : <>
      <div className="overflow-x-auto rounded-2xl border bg-white"><table className="w-full min-w-[1180px] table-fixed text-sm"><thead className="sticky top-0 bg-slate-50 text-left"><tr>{['ID', 'Компания', 'Объект', 'Период', 'Срок сдачи', 'Статус', 'Протоколы', 'Последний сбор', 'Ответственный', 'Действия'].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody>{reports.data.content.map((report) => <tr key={report.id} className="h-20 border-t align-middle"><td className="px-4 py-2 font-bold">{report.id}</td><td className="px-4 py-2"><EntityName value={report.company} fallback="—" className="line-clamp-2 break-words" /></td><td className="px-4 py-2"><EntityName value={report.object} fallback="—" className="line-clamp-2 break-words" /></td><td className="px-4 py-2"><span className="line-clamp-2">{report.periodStart} — {report.periodEnd}</span></td><td className="px-4 py-2">{report.submissionDueDate || '—'}</td><td className="px-4 py-2"><PekStatusBadge status={report.status} /></td><td className="px-4 py-2">{report.linkedProtocolCount}</td><td className="px-4 py-2"><span className="line-clamp-2 break-words" title={report.lastCollectedAt || undefined}>{report.lastCollectedAt || '—'}</span></td><td className="px-4 py-2"><EntityName value={report.responsibleUser} fallback="—" className="line-clamp-2 break-words" /></td><td className="px-4 py-2"><Link className="font-bold text-eco-700" to={`/staff/pek/reports/${report.id}`}>Открыть</Link></td></tr>)}</tbody></table></div>
      <div className="flex flex-wrap items-center justify-between gap-3"><button type="button" disabled={!filters.page} onClick={() => update('page', String((filters.page || 0) - 1))} className="rounded-full border px-4 py-2 disabled:opacity-40">Назад</button><span className="text-center">Страница {(filters.page || 0) + 1} из {reports.data.totalPages || 1} · всего {reports.data.totalElements}</span><button type="button" disabled={(filters.page || 0) + 1 >= reports.data.totalPages} onClick={() => update('page', String((filters.page || 0) + 1))} className="rounded-full border px-4 py-2 disabled:opacity-40">Далее</button></div>
    </>}
  </div>;
};
