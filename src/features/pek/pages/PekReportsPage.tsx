...
const PekReportsPage = () => {
  ...
  return <div className="space-y-5">
    ...
    {!companyId || !objectId ? <PekState title="Выберите компанию и объект" message="Backend требует companyId и objectId для загрузки списка отчётов." /> : reports.isLoading ? <PekLoading /> : reports.isError ? <PekQueryError error={reports.error} resource="Отчёты ПЭК" retry={() => void reports.refetch()} /> : !reports.data?.content.length ? <PekState title="Для выбранного объекта отчётов нет" /> : <>
