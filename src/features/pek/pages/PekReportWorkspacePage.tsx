...
const PekReportWorkspacePage = () => {
  ...
  return <div className="space-y-5">
    ...
    {tab === 'sources' && <section className="space-y-4 rounded-2xl border bg-white p-5">
      ...
      {sources.isLoading ? <PekLoading /> : sources.isError ? <PekQueryError error={sources.error} resource="источники отчёта" retry={() => void sources.refetch()} /> : !sources.data?.length ? <PekState title="Источники не найдены" message="Запустите сбор данных или измените фильтр." /> : <div className="max-h-[65vh] overflow-auto"><table className="w-full min-w-[2200px] text-sm"><thead className="sticky top-0 z-10 bg-white"><tr className="border-b text-left"><th className="p-2">Протокол</th><th>Показатель</th><th>Значение</th><th>Норматив</th><th>Отбор / измерение</th><th>Методика / лаборатория</th><th>Позиция программы</th><th>Сопоставление</th><th>Причина</th><th>Действия</th></tr></thead><tbody>{sources.data.map((source) => <tr key={source.id} className={`border-b align-top ${source.excluded || source.matchStatus === 'STALE' ? 'bg-slate-100 text-slate-500' : ''}`}><td className="p-2"><p className="font-semibold">№ {source.protocolNumber}</p><p>{source.protocolDate || 'Дата не указана'}</p><p>{source.protocolStatus || 'Статус не указан'}</p></td><td><p>{source.indicatorCode || '—'}</p><p className="font-semibold">{source.indicatorName || 'Не сопоставлен'}</p></td><td><p>{source.value ?? source.valueText ?? '—'} {source.unit || ''}</p>{source.valueText && source.value != null && <p className="text-xs">{source.valueText}</p>}</td><td><p>{source.normativeValue ?? '—'}</p><p>{source.comparisonType || '—'}</p><p className={source.isExceedance ? 'font-bold text-rose-700' : ''}>{source.isExceedance == null ? '—' : source.isExceedance ? 'Превышение' : 'В норме'}</p></td><td><p>{source.samplingPlace || '—'}</p><p>{source.measurementDate || '—'}</p></td><td><p>{source.methodology || '—'}</p><p>{source.laboratoryName || '—'}</p></td><td><p>{source.controlItemName || `Позиция №${source.controlItemId || '—'}`}</p><p>{source.programIndicatorName || '—'}</p></td><td>{source.matchStatus === 'STALE' ? 'Устаревшая связь' : source.excluded ? 'Исключён' : matchLabels[source.matchStatus] || source.matchStatus}</td><td>{source.matchStatus === 'STALE' ? 'Источник больше не соответствует условиям отчёта и не участвует в расчётах' : source.exclusionReason || source.matchReason || '—'}</td><td><div className="flex flex-wrap gap-2">{canMutateSources && !source.excluded && ['UNMATCHED', 'AMBIGUOUS'].includes(source.matchStatus) && <MuiButton size="small" onClick={() => { setSelectedSource(source); setControlItemId(''); setIndicatorId(''); }}>Сопоставить вручную</MuiButton>}{canMutateSources && !source.excluded && source.matchStatus !== 'STALE' && <MuiButton size="small" color="error" onClick={() => setExcludeSource(source)}>Исключить</MuiButton>}{canMutateSources && source.excluded && source.matchStatus !== 'STALE' && <MuiButton size="small" disabled={restore.isPending} onClick={() => restore.mutate(source)}>Восстановить</MuiButton>}</div></td></tr>)}</tbody></table></div>}
    </section>}
