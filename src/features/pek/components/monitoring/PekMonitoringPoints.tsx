...
import { coordinateError, serializeCoordinates } from '../../utils/pekCoordinates';
...
const PekMonitoringPoints = ({ programId, monitoringId, editable }: { programId: number; monitoringId: number; editable: boolean }) => {
  ...
  const save = useMutation({
    mutationFn: () => editing === 'new'
      ? pekApi.createMonitoringPoint(programId, monitoringId, { name: form.name.trim(), coordinates: serializeCoordinates(form.latitude, form.longitude), description: form.description.trim() || null })
      : pekApi.updateMonitoringPoint(programId, editing!.id, editing!.version, { name: form.name.trim(), coordinates: serializeCoordinates(form.latitude, form.longitude), description: form.description.trim() || null }),
  ...
  const openCreate = () => { save.reset(); setForm(emptyForm); setEditing('new'); };
  ...
  const latitudeValid = !coordinateError(form.latitude, 'latitude');
  const longitudeValid = !coordinateError(form.longitude, 'longitude');
  const pairValid = Boolean(form.latitude.trim()) === Boolean(form.longitude.trim());
  const conflict = save.isError && [409, 412].includes(mapPekError(save.error).status || 0);
  ...
  return <div className="mt-4 rounded-xl bg-slate-50 p-4">
    ...
    {remove.isError && <Alert severity="error">{mapPekError(remove.error).message}</Alert>}
    ...
    <Dialog open={Boolean(editing)} onClose={() => !save.isPending && setEditing(null)} fullWidth maxWidth="sm">
      ...
      <DialogContent><div className="mt-2 grid gap-4">
        ...
        {!pairValid && <Alert severity="error">Укажите и широту, и долготу.</Alert>}
        {conflict && <Alert severity="warning">Закройте форму и откройте точку заново, чтобы загрузить актуальную версию.</Alert>}
        {save.isError && <Alert severity="error">{mapPekError(save.error).message}</Alert>}
      </div></DialogContent>
      <DialogActions><Button disabled={save.isPending} onClick={() => setEditing(null)}>Отмена</Button><Button variant="contained" disabled={save.isPending || !editable || conflict || !pairValid || !form.name.trim() || !latitudeValid || !longitudeValid} onClick={() => save.mutate()}>Сохранить</Button></DialogActions>
    </Dialog>
