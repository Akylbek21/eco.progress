...
const PekProgramDocuments = ({ programId, documents, readOnly }: {
  programId: number;
  ...
  return <div className="space-y-4">
    ...
    >
      <label className="block font-bold">Файл
        <input type="file" onChange={(event) => selectFile(event.target.files?.[0] || null)} className="mt-2 block w-full" />
      </label>
      <p className="mt-2 text-sm text-slate-500">Перетащите файл сюда. Максимальный размер — 25 МБ.</p>
      <label className="mt-3 block font-bold">Тип документа
        <input value={documentType} onChange={(event) => setDocumentType(event.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2" />
      </label>
      {file && <p className="mt-2 text-sm">Выбран: {file.name}</p>}
    ...
    <div className="space-y-2">
      {documents.map((document) => (
        <div key={document.id} className="flex items-center justify-between rounded-xl bg-slate-50 p-3">
          <div><strong>{document.originalFileName || document.fileName || `Документ №${document.id}`}</strong><p className="text-xs text-slate-500">{document.documentType || 'Тип не указан'}</p></div>
