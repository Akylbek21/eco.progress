...
const PekReportPackageCard = ({ report }: { report: PekReport }) => {
  ...
  if (packageQuery.isError || !packageQuery.data) return <section className="rounded-2xl border bg-white p-5"><PekQueryError error={packageQuery.error} resource="комплект ПЭК" retry={() => void packageQuery.refetch()} /></section>;
  ...
  const canRegenerate = data.availableActions.regeneratePackage === true;
  ...
  return <section className="space-y-5 rounded-2xl border bg-white p-5">
    ...
    <div className="grid gap-3 rounded-xl bg-slate-50 p-4 text-sm sm:grid-cols-4">
      ...
      <div><span className="text-slate-500">Сформировал</span><p className="font-bold">{data.generatedBy?.name || '—'}</p></div>
    </div>
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {canRegenerate && <Button variant="contained" disabled={busy || missingFields.length > 0} onClick={() => generate.mutate()}>Пересформировать</Button>}
