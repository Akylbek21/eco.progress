...
const OfficialSignatures = ({ report }: { report: PekReport }) => {
  ...
  return <section className="space-y-3 rounded-2xl border bg-white p-5">
    <div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="font-black">Подписи официального отчёта</h2><p className="text-sm text-slate-500">{report.officialSignatory ? `Подписант: ${report.officialSignatory.name}${report.officialSignatory.position ? `, ${report.officialSignatory.position}` : ''}${report.officialSignatory.iinConfigured ? '' : ' · ИИН не настроен'}` : 'Первый руководитель не назначен в настройках ПЭК.'}</p></div>{report.availableActions.signOfficialDocument === true && report.officialSignatory?.currentUserCanSign === true && !officialStale && <Button color="success" variant="contained" disabled={sign.isPending} onClick={() => sign.mutate()}>Подписать официальный PDF</Button>}</div>
