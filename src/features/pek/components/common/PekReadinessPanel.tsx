...
export default function PekReadinessPanel({ readiness, onIssueClick }: {
  ...
  return <section className="rounded-2xl border bg-white p-5" aria-label="Панель готовности">
    ...
    {readiness.issues.length > 0 && <ul className="mt-4 space-y-2">{[...blocking, ...warnings].map((issue, index) => <li key={`${issue.code}-${issue.entityId || ''}-${index}`}><button type="button" onClick={() => onIssueClick?.(issue)} className={`flex w-full items-start gap-2 rounded-xl border p-3 text-left ${issue.severity === 'BLOCKING' ? 'border-rose-200 bg-rose-50' : 'border-amber-200 bg-amber-50'}`}><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><span><strong>{issue.severity === 'BLOCKING' ? 'Блокирует отправку' : 'Предупреждение'}</strong><span className="block text-sm">{issue.message}</span>{issue.section && <span className="text-xs text-slate-500">Раздел: {issue.section}{issue.field ? ` · поле: ${issue.field}` : ''}</span>}</span></button></li>)}</ul>}
  </section>;
}
