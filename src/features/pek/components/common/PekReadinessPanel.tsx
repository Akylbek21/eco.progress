...
export default function PekReadinessPanel({ readiness, onIssueClick, maxVisibleIssues, onShowAll }: {
  ...
  const orderedIssues = [...blocking, ...warnings];
  const visibleIssues = maxVisibleIssues == null ? orderedIssues : orderedIssues.slice(0, maxVisibleIssues);
  const hiddenIssueCount = orderedIssues.length - visibleIssues.length;
  return <section className="border border-slate-300 bg-white" aria-label="Панель готовности">
    <div className="flex flex-wrap items-center gap-6 border-b border-slate-200 px-4 py-3"><div><h2 className="text-base font-black">Готовность: {readiness.completionPercent == null ? '—' : `${readiness.completionPercent}%`}</h2><p className="text-xs text-slate-600">{readiness.ready ? 'Проверка пройдена' : `Утверждение заблокировано. Проблем: ${blocking.length}`}</p></div><div className="flex gap-4 text-xs"><span><b className="text-rose-700">{blocking.length}</b> ошибок</span><span><b className="text-amber-700">{warnings.length}</b> предупреждений</span></div>{readiness.ready ? <CheckCircle2 className="ml-auto h-5 w-5 text-emerald-700" aria-label="Готово" /> : <AlertCircle className="ml-auto h-5 w-5 text-rose-700" aria-label="Не готово" />}</div>
    {visibleIssues.length > 0 && <div className="divide-y divide-slate-200">{visibleIssues.map((issue, index) => <button key={`${issue.code}-${issue.entityId || ''}-${index}`} type="button" onClick={() => onIssueClick?.(issue)} className="flex w-full items-center gap-3 px-4 py-2 text-left text-sm hover:bg-slate-50"><AlertTriangle className={`h-4 w-4 shrink-0 ${isBlocking(issue) ? 'text-rose-600' : 'text-amber-600'}`} /><span className="min-w-0 flex-1"><span className="font-semibold">{issue.message}</span><span className="block text-xs text-slate-500">{issue.section ? sectionLabels[issue.section] || 'Раздел программы' : 'Проверка программы'}{issue.field ? ` · ${issue.field}` : ''}</span></span><span className={`shrink-0 rounded-full px-2 py-1 text-[11px] font-bold ${isBlocking(issue) ? 'bg-rose-50 text-rose-700' : 'bg-amber-50 text-amber-700'}`}>{isBlocking(issue) ? 'Блокирует отправку' : 'Предупреждение'}</span><span className="shrink-0 text-xs font-bold text-eco-800">{issueLabels[issue.code] || 'Открыть'} →</span></button>)}</div>}
    {hiddenIssueCount > 0 && <button type="button" onClick={onShowAll} className="w-full border-t border-slate-200 px-4 py-3 text-left text-sm font-bold text-eco-800 hover:bg-slate-50">Показать остальные проблемы ({hiddenIssueCount}) →</button>}
  </section>;
}
