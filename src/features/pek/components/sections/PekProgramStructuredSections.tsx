...
const StructuredSection = <T extends VersionedItem, TForm,>({ program, definition }: { program: PekProgram; definition: SectionDefinition<T, TForm> }) => {
  ...
  return <section className="space-y-4 rounded-2xl border bg-white p-5">
    ...
    <Dialog open={Boolean(editing)} onClose={() => !save.isPending && setEditing(null)} fullWidth maxWidth="md">
      ...
      <DialogContent><div className="mt-2 grid gap-4 sm:grid-cols-2">{definition.fields(form, set)}{!definition.valid(form) && <Alert className="sm:col-span-2" severity="info">Заполните поля, отмеченные звёздочкой.</Alert>}{save.isError && <Alert className="sm:col-span-2" severity="error">{mapPekError(save.error).message}</Alert>}</div></DialogContent>
...
const AssigneeField = ({ value, onChange, options, loading = false, error = false, required = false }: { value: number | null; onChange: (value: number | null) => void; options: Array<{ id: number; name: string }>; loading?: boolean; error?: boolean; required?: boolean }) => <TextField select label={`Ответственный${required ? ' *' : ''}`} value={value ?? ''} disabled={loading} error={error} helperText={loading ? 'Загрузка сотрудников…' : error ? 'Не удалось загрузить сотрудников' : !options.length ? 'Нет доступных сотрудников' : undefined} onChange={(event) => onChange(event.target.value ? Number(event.target.value) : null)}><MenuItem value="">Не выбран</MenuItem>{options.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</TextField>;
...
const inspectionStatuses: Record<string, string> = { PLANNED: 'Запланирована', COMPLETED: 'Завершена', OVERDUE: 'Просрочена', CANCELLED: 'Отменена' };
const PekProgramStructuredSections = ({ program, section }: { program: PekProgram; section?: PekStructuredSectionKey }) => {
  ...
  const inspection: SectionDefinition<PekInternalInspection, PekInternalInspectionRequest> = {
    ...
    empty: () => ({ plannedDate: null, actualDate: null, inspectionType: '', findings: '', correctiveActionRequired: false, responsibleUserId: null, status: 'PLANNED', department: '', frequencyType: '', violations: '', correctiveActions: '' }),
    toForm: ({ plannedDate, actualDate, inspectionType, findings, correctiveActionRequired, responsibleUserId, status, department, frequencyType, violations, correctiveActions }) => ({ plannedDate, actualDate, inspectionType, findings, correctiveActionRequired, responsibleUserId, status, department, frequencyType, violations, correctiveActions }),
    ...
    summary: (item) => <TextSummary title={item.inspectionType || 'Проверка'} details={<><p>{item.plannedDate || 'Без даты'} · {item.frequencyType || 'без периодичности'} · {inspectionStatuses[item.status] || item.status}</p><p>{item.department || 'Подразделение не указано'}</p><p>{item.findings || 'Результаты не указаны'}</p>{item.violations && <p>Нарушения: {item.violations}</p>}{item.correctiveActions && <p>Меры: {item.correctiveActions}</p>}</>} />,
    fields: (form, set) => <><TextField type="date" label="Плановая дата *" InputLabelProps={{ shrink: true }} value={form.plannedDate || ''} onChange={(e) => set('plannedDate', e.target.value || null)} /><TextField type="date" label="Фактическая дата" InputLabelProps={{ shrink: true }} value={form.actualDate || ''} onChange={(e) => set('actualDate', e.target.value || null)} /><TextField label="Тип проверки *" value={form.inspectionType || ''} onChange={(e) => set('inspectionType', e.target.value)} /><TextField label="Подразделение" value={form.department || ''} onChange={(e) => set('department', e.target.value)} /><TextField label="Периодичность" value={form.frequencyType || ''} onChange={(e) => set('frequencyType', e.target.value)} /><TextField select label="Статус" value={form.status} onChange={(e) => set('status', e.target.value)}>{Object.entries(inspectionStatuses).map(([status, label]) => <MenuItem key={status} value={status}>{label}</MenuItem>)}</TextField><AssigneeField value={form.responsibleUserId} options={users} onChange={(value) => set('responsibleUserId', value)} /><FormControlLabel control={<Checkbox checked={form.correctiveActionRequired} onChange={(e) => set('correctiveActionRequired', e.target.checked)} />} label="Требуется корректирующее действие" /><TextField className="sm:col-span-2" multiline minRows={3} label="Результаты" value={form.findings || ''} onChange={(e) => set('findings', e.target.value)} /><TextField className="sm:col-span-2" multiline minRows={3} label="Выявленные нарушения" value={form.violations || ''} onChange={(e) => set('violations', e.target.value)} /><TextField className="sm:col-span-2" multiline minRows={3} label="Корректирующие действия" value={form.correctiveActions || ''} onChange={(e) => set('correctiveActions', e.target.value)} /></>,
  };
  const qa: SectionDefinition<PekMeasurementQa, PekMeasurementQaRequest> = {
    key: 'measurement-qa', title: 'Контроль качества измерений', description: 'Как проверяется точность измерений, кто отвечает и когда проводится следующая проверка.', addLabel: 'Добавить контроль качества',
    ...
    query: (signal) => pekApi.getMeasurementQa(program.id, signal), create: (form) => pekApi.createMeasurementQa(program.id, form), update: (item, form) => pekApi.updateMeasurementQa(program.id, item.id, item.version, form), remove: (item) => pekApi.deleteMeasurementQa(program.id, item.id, item.version), valid: (form) => Boolean(form.parameter.trim() && form.qaProcedure?.trim() && form.frequency?.trim() && form.responsibleUserId && (!form.lastCheckDate || !form.nextCheckDate || form.nextCheckDate >= form.lastCheckDate)),
    ...
    fields: (form, set) => <><TextField label="Контролируемый параметр *" value={form.parameter} onChange={(e) => set('parameter', e.target.value)} /><TextField label="Периодичность *" value={form.frequency || ''} onChange={(e) => set('frequency', e.target.value)} /><AssigneeField required value={form.responsibleUserId} options={users} onChange={(value) => set('responsibleUserId', value)} /><TextField type="date" label="Последняя проверка" InputLabelProps={{ shrink: true }} value={form.lastCheckDate || ''} onChange={(e) => set('lastCheckDate', e.target.value || null)} /><TextField type="date" label="Следующая проверка" InputLabelProps={{ shrink: true }} value={form.nextCheckDate || ''} onChange={(e) => set('nextCheckDate', e.target.value || null)} /><TextField className="sm:col-span-2" multiline minRows={3} label="Процедура контроля качества *" value={form.qaProcedure || ''} onChange={(e) => set('qaProcedure', e.target.value)} /></>,
  };
  ...
  const responsibility: SectionDefinition<PekResponsibility, PekResponsibilityRequest> = {
    ...
    query: (signal) => pekApi.getResponsibilities(program.id, signal), create: (form) => pekApi.createResponsibility(program.id, form), update: (item, form) => pekApi.updateResponsibility(program.id, item.id, item.version, form), remove: (item) => pekApi.deleteResponsibility(program.id, item.id, item.version), valid: (form) => Boolean(form.roleLabel.trim() && form.userId && form.duties?.trim()),
    ...
    fields: (form, set) => <><TextField label="Роль *" value={form.roleLabel} onChange={(e) => set('roleLabel', e.target.value)} /><AssigneeField required value={form.userId} options={users} onChange={(value) => set('userId', value)} /><TextField className="sm:col-span-2" multiline minRows={4} label="Обязанности *" value={form.duties || ''} onChange={(e) => set('duties', e.target.value)} /></>,
  };
