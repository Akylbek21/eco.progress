...
import { resolveDocumentActions } from '../model/documentActions';
...
export default function DocumentDetailsPage() {
  ...
  const versions = useQuery({ queryKey: scope ? documentFlowKeys.versions(scope, id) : ['document-flow', 'tenant-unresolved', id, 'versions'], queryFn: ({ signal }) => documentFlowApi.versions(id, undefined, signal), enabled: Boolean(scope) && tab === 2 && Number.isSafeInteger(id) });
  const attachments = useQuery({ queryKey: scope ? documentFlowKeys.attachments(scope, id) : ['document-flow', 'tenant-unresolved', id, 'attachments'], queryFn: ({ signal }) => documentFlowApi.attachments(id, undefined, signal), enabled: Boolean(scope) && tab === 3 && Number.isSafeInteger(id) });
  ...
  const document = documentQuery.data;
  const activeOwnAssignment = route.data?.steps.flatMap((step) => step.assignments).find((assignment) =>
    assignment.userId !== undefined
    && String(assignment.userId) === String(user?.id)
    && ['AVAILABLE', 'VIEWED'].includes(assignment.status)
  );
  const resolvedActions = resolveDocumentActions(document.availableActions, activeOwnAssignment ? ['SIGN'] : []);
  ...
  return (
    <Stack spacing={3}>
      ...
      {tab === 1 && <Stack spacing={2}>
        {resolvedActions.unavailableBecauseBackendContract.includes('SIGN') && <BackendContractBlocker title="Внутреннее подписание недоступно" reason="Backend не возвращает разрешение SIGN в availableActions для активного задания." technicalCode="DF_ACTION_SIGN_MISSING" endpoint={`/api/document-flow/documents/${id}`} missingField="availableActions: SIGN" />}
