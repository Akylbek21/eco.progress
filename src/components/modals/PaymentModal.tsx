  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} loading={loading}>
      <form onSubmit={submit} className="grid gap-4">
        ...
        <label className="text-sm font-semibold text-slate-700">Дата оплаты<input name="date" type="date" defaultValue={new Date().toISOString().slice(0, 10)} className="input-focus mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3" /></label>
        ...
        <label className="text-sm font-semibold text-slate-700">Файл чека / поручения<input name="file" type="file" className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3" /></label>
        ...
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button type="button" variant="secondary" onClick={onClose}>Отмена</Button><Button type="submit">{mode === 'full' ? 'Подтвердить оплату' : 'Сохранить оплату'}</Button></div>
      </form>
    </Modal>
  );
};
