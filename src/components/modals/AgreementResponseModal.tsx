...
export type AgreementResponseAction = 'accepted' | 'signed' | 'sent_without_signature' | 'revision_requested';
...
const AgreementResponseModal = ({ isOpen, documentName, loading = false, onClose, onSubmit }: AgreementResponseModalProps) => {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await onSubmit({
        action: String(form.get('action') || 'accepted') as AgreementResponseAction,
      ...
      <form onSubmit={submit} className="grid gap-4">
        ...
        <label className="text-sm font-semibold text-slate-700">
          ...
          <select name="action" className="input-focus mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3">
            ...
            <option value="signed">Подписать и отправить</option>
            <option value="sent_without_signature">Отправить без подписи</option>
