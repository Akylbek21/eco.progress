...
type Props = {
  ...
  testingBasis?: string;
  onTestingBasisChange?: (value: string) => void;
...
const ProtocolTestingForm = ({ templateId, value, measurementDate, readOnly, onMeasurementDateChange, onChange, testingBasis, onTestingBasisChange, printVisibility, onPrintVisibilityChange }: Props) => (
  <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      {onTestingBasisChange && <label className="space-y-1.5 text-sm font-semibold text-slate-700 md:col-span-2 xl:col-span-3"><span>Основание для испытаний</span><textarea rows={2} className={inputClass} disabled={readOnly} value={testingBasis || ''} placeholder="Например: договор, заявка или программа производственного контроля" onChange={(e) => onTestingBasisChange(e.target.value)} /><ProtocolPrintVisibilityToggle field="testBasis" visibility={printVisibility} readOnly={readOnly} onChange={onPrintVisibilityChange} /></label>}
