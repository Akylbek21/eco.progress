...
type Props = { templateId: ProtocolTemplateId; value: ProtocolTestingData; readOnly: boolean; onChange: (value: ProtocolTestingData) => void };
...
const ProtocolTestingForm = ({ templateId, value, readOnly, onChange }: Props) => (
  <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      <label className="space-y-1.5 text-sm font-semibold text-slate-700"><span>Дата отбора</span><input type="date" className={inputClass} disabled={readOnly} value={value.samplingDate || ''} onChange={(e) => onChange({ ...value, samplingDate: e.target.value })} /></label>
