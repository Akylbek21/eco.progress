...
const ProtocolGeneralForm = ({ protocol, readOnly, onChange }: Props) => (
  <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <label className="space-y-1.5 text-sm font-semibold text-slate-700"><span>Дата протокола</span><input type="date" className={inputClass} disabled={readOnly} value={protocol.protocolDate || ''} onChange={(e) => onChange({ protocolDate: e.target.value })} /><ProtocolPrintVisibilityToggle field="protocolDate" visibility={protocol.printVisibility} readOnly={readOnly} onChange={(printVisibility) => onChange({ printVisibility })} /></label>
      <label className="space-y-1.5 text-sm font-semibold text-slate-700"><span>Форма</span><input className={inputClass} disabled={readOnly} value={protocol.formCode || ({ industrial_emissions: 'Ф 01', water_wastewater: 'Ф 02', ambient_air: 'Ф 03', physical_factors: 'Ф 04', microclimate: 'Ф 04', lighting: 'Ф 04', noise_vibration: 'Ф 04', soil: 'Почва' } as Record<string, string>)[protocol.templateId] || ''} onChange={(e) => onChange({ formCode: e.target.value })} /><ProtocolPrintVisibilityToggle field="formCode" visibility={protocol.printVisibility} readOnly={readOnly} onChange={(printVisibility) => onChange({ printVisibility })} /></label>
      <label className="space-y-1.5 text-sm font-semibold text-slate-700"><span>Приложение</span><input className={inputClass} disabled={readOnly} value={protocol.application || ''} onChange={(e) => onChange({ application: e.target.value })} placeholder="При наличии" /><ProtocolPrintVisibilityToggle field="application" visibility={protocol.printVisibility} readOnly={readOnly} onChange={(printVisibility) => onChange({ printVisibility })} /></label>
      ...
      <label className="space-y-1.5 text-sm font-semibold text-slate-700"><span>Утверждающий</span><input className={inputClass} disabled={readOnly} value={protocol.approver || ''} onChange={(e) => onChange({ approver: e.target.value })} /><ProtocolPrintVisibilityToggle field="approver" visibility={protocol.printVisibility} readOnly={readOnly} onChange={(printVisibility) => onChange({ printVisibility })} /></label>
    </div>
  </section>
);
