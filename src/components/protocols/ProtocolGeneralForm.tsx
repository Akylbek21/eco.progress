...
const ProtocolGeneralForm = ({ protocol, readOnly, onChange }: Props) => (
  <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <label className="space-y-1.5 text-sm font-semibold text-slate-700"><span>Форма</span><input className={inputClass} disabled={readOnly} value={protocol.formCode || ({ industrial_emissions: 'Ф 01', water: 'Ф 02', ambient_air: 'Ф 03', physical_factors: 'Ф 04', microclimate: 'Ф 04', lighting: 'Ф 04', noise_vibration: 'Ф 04', soil: 'Почва' } as Record<string, string>)[protocol.templateId] || ''} onChange={(e) => onChange({ formCode: e.target.value })} /></label>
      <label className="space-y-1.5 text-sm font-semibold text-slate-700"><span>Номер приложения</span><input className={inputClass} disabled={readOnly} value={protocol.appendixNumber || ''} onChange={(e) => onChange({ appendixNumber: e.target.value })} placeholder="При наличии" /></label>
