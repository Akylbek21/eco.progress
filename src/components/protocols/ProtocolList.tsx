...
const primaryLabel = (protocol: Protocol) => {
  ...
  if (['READY', 'READY_TO_SIGN', 'READY_FOR_APPROVAL', 'APPROVED', 'CALCULATED'].includes(status)) return 'Подписать';
  if (status === 'SIGNED' && protocol.hasPdf) return 'Скачать PDF';
  return 'Открыть';
};

const ProtocolRowActions = ({ protocol, busy, onOpen, onArchive, onReplace, onDownload }: {
  protocol: Protocol; busy: boolean; onOpen: Props['onOpen']; onArchive: Props['onArchive'];
  onReplace: Props['onReplace']; onDownload: Props['onDownload'];
}) => {
  const [open, setOpen] = useState(false);
  const primary = primaryLabel(protocol);
  const runPrimary = () => primary === 'Скачать PDF' ? onDownload(protocol, 'pdf') : onOpen(protocol);
...
const ProtocolList = ({ protocols, loading = false, busyId, onOpen, onArchive, onReplace, onDownload }: Props) => {
  ...
  return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="space-y-3 p-3 md:hidden">{protocols.map((protocol) => <article key={protocol.id} className="rounded-xl border border-slate-200 p-4">
      ...
      <div className="mt-4"><ProtocolRowActions protocol={protocol} busy={busyId === protocol.id} onOpen={onOpen} onArchive={onArchive} onReplace={onReplace} onDownload={onDownload} /></div>
    </article>)}</div>
