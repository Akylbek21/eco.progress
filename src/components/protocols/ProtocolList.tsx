...
const ProtocolList = ({ protocols, role, loading = false, busyId, onOpen, onArchive, onReplace, onDownload }: Props) => (
  <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="overflow-x-auto">
      <table className="w-full min-w-[1850px] table-fixed text-left text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {loading ? Array.from({ length: 8 }).map((_, row) => <tr key={row} className="h-16 animate-pulse">{Array.from({ length: 14 }).map((__, cell) => <td key={cell} className="px-4 py-3"><div className="h-4 rounded bg-slate-100" /></td>)}</tr>) : protocols.map((protocol) => {
