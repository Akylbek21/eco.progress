        ...
        <tbody className="divide-y divide-slate-100">
          {loading ? Array.from({ length: 8 }).map((_, row) => <tr key={row} className="h-16 animate-pulse">{Array.from({ length: 16 }).map((__, cell) => <td key={cell} className="px-4 py-3"><div className="h-4 rounded bg-slate-100" /></td>)}</tr>) : protocols.map((protocol) => {
            ...
            return <tr key={protocol.id} tabIndex={0} onClick={() => onOpen(protocol)} onKeyDown={(event) => { if (event.key === 'Enter') onOpen(protocol); }} className={`cursor-pointer align-top hover:bg-slate-50 focus:bg-eco-50 focus:outline-none ${protocol.status === 'ARCHIVED' ? 'bg-slate-50/80 text-slate-600' : ''}`}>
              <td className="truncate px-4 py-4 font-bold text-slate-900">{protocol.protocolNumber || protocol.number || '—'}</td><td className="px-4 py-4">{formatDate(protocol.protocolDate)}</td><td className="truncate px-4 py-4" title={protocol.companySnapshot?.companyName}>{protocol.companySnapshot?.companyName || '—'}</td><td className="truncate px-4 py-4" title={protocol.companySnapshot?.objectName}>{protocol.companySnapshot?.objectName || '—'}</td><td className="px-4 py-4">{templateName(protocol.templateId, protocol.templateName)}</td><td className="truncate px-4 py-4">{protocol.laboratory?.laboratoryName || '—'}</td><td className="truncate px-4 py-4">{protocol.executor || protocol.laboratory?.executor || '—'}</td><td className="px-4 py-4"><ProtocolStatusBadge status={protocol.status} /></td><td className="px-4 py-4 font-semibold">{protocol.signatureCount}/{protocol.maxSignatures}</td><td className="px-4 py-4"><NormativeStatusBadge status={protocol.complianceResult} /></td><td className="px-4 py-4">{protocol.version}</td><td className="px-4 py-4">{[protocol.hasDocx && `DOCX${protocol.docxFileId ? ` #${protocol.docxFileId}` : ''}`, protocol.hasPdf && `PDF${protocol.pdfFileId ? ` #${protocol.pdfFileId}` : ''}`].filter(Boolean).join(' · ') || '—'}</td><td className="px-4 py-4">{protocol.publishedAt || protocol.publishedToClientAt ? `Опубликован · ${formatDate(protocol.publishedAt || protocol.publishedToClientAt)}` : '—'}</td><td className="px-4 py-4">{protocol.pekReportId ? `Отчёт #${protocol.pekReportId}` : protocol.pekProgramId ? `Программа #${protocol.pekProgramId}` : '—'}</td><td className="px-4 py-4">{formatDate(protocol.updatedAt)}</td>
              <td className="px-4 py-3" onClick={(event) => event.stopPropagation()}><div className="flex justify-end gap-2"><button type="button" className={`${iconButton} text-eco-800 ring-eco-200`} aria-label={`${permissions.canSave ? 'Изменить' : 'Открыть'} протокол ${protocol.protocolNumber}`} onClick={() => onOpen(protocol)}>{permissions.canSave ? <Edit3 className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>{permissions.canDownload && protocol.hasDocx && <button type="button" disabled={busy} className={`${iconButton} text-eco-800 ring-eco-200`} aria-label={`Скачать DOCX ${protocol.protocolNumber}`} onClick={() => onDownload(protocol, 'docx')}><Download className="h-4 w-4" /></button>}{permissions.canDownload && protocol.hasPdf && <button type="button" disabled={busy} className={`${iconButton} text-blue-800 ring-blue-200`} aria-label={`Скачать PDF ${protocol.protocolNumber}`} onClick={() => onDownload(protocol, 'pdf')}><Download className="h-4 w-4" /></button>}{permissions.canCreateCorrection && <button type="button" className={`${iconButton} text-violet-800 ring-violet-200`} aria-label={`Создать исправленную версию ${protocol.protocolNumber}`} onClick={() => onReplace(protocol)}><RotateCw className="h-4 w-4" /></button>}{permissions.canArchive && <button type="button" className={`${iconButton} text-rose-700 ring-rose-200`} aria-label={`Архивировать протокол ${protocol.protocolNumber}`} onClick={() => onArchive(protocol)}><Archive className="h-4 w-4" /></button>}</div></td>
            </tr>;
          })}
        </tbody>
      </table>
