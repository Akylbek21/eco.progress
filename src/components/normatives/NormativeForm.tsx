...
const numericTypes: NormativeValueType[] = ['EXACT', 'LE', 'GE'];
...
const initialValues = (record?: LegacyNormativeRecord | null): NormativeFormValues => ({
  ...
  valueType: record?.comparisonType === 'RANGE' ? 'RANGE' : record?.comparisonType === 'GREATER_OR_EQUAL' ? 'GE' : record?.comparisonType === 'EQUAL' ? 'EXACT' : record?.comparisonType === 'ABSENT' ? 'REFERENCE_ONLY' : record?.comparisonType === 'INFO' ? 'TEXT' : 'LE',
...
const NormativeForm = ({ initial, busy = false, onCancel, onSave }: Props) => {
  ...
  const submit = handleSubmit(async (values) => {
    ...
    try { await onSave(normalized); } catch (error) {
      ...
      const backendFieldMap: Record<string, keyof NormativeFormValues> = { indicator: 'indicatorName', sourceDocumentCode: 'documentCode', templateId: 'protocolType', min: 'minValue', max: 'maxValue', conditionJson: 'conditions', factorCode: 'indicatorCode', casNumber: 'cas', chemicalFormula: 'formula' };
      Object.entries(parsed.fieldErrors || {}).forEach(([field, message]) => {
        const target = backendFieldMap[field] || (field in values ? field as keyof NormativeFormValues : undefined);
  ...
  return <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
    {field('documentCode', 'Код исходного документа', { required: true })}{field('category', 'Объект исследования', { required: true })}{field('subCategory', 'Подтип норматива')}{field('indicatorName', 'Название показателя', { required: true })}{field('indicatorCode', 'Код фактора')}{field('cas', 'CAS')}{field('formula', 'Формула')}{field('protocolType', 'templateId', { required: true })}
    <label className="space-y-1.5 text-sm font-semibold"><span>Тип значения</span><select {...register('valueType')} disabled={submitting} className={inputClass}>{(['EXACT', 'LE', 'GE', 'RANGE', 'TEXT', 'REFERENCE_ONLY'] as NormativeValueType[]).map((item) => <option key={item} value={item}>{({ EXACT: 'Точное значение', LE: 'Меньше или равно', GE: 'Больше или равно', RANGE: 'Диапазон', TEXT: 'Информационная запись', REFERENCE_ONLY: 'Требуется отсутствие' } as Record<NormativeValueType, string>)[item]}</option>)}</select></label>
    ...
    {(valueType === 'TEXT' || valueType === 'REFERENCE_ONLY') && <p className="rounded-xl bg-slate-50 p-3 text-sm text-slate-600 sm:col-span-2">Для этого типа backend не принимает числовое значение. Пояснение можно сохранить в условиях применения.</p>}
