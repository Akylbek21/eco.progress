...
const mockResponse = async (config: InternalAxiosRequestConfig): Promise<AxiosResponse> => {
  ...
  if ((parts[0] === 'staff' || parts[0] === 'client') && parts[1] === 'orders') {
    ...
    if (scope === 'client' && orderId && parts[3] === 'contract' && parts[4] === 'sign' && method === 'post') {
      return ok(config, signMockOrderContract(orderId, String(payload.provider || 'NCALayer')));
    }

    if (scope === 'client' && orderId && parts[3] === 'pay' && method === 'post') {
      return ok(config, updateMockPayment(orderId, 'paid', { method: String(payload.method || 'bank_transfer'), paidAmount: getMockOrderById(orderId)?.totalAmount }));
    }
    ...
    if (scope === 'staff' && orderId && parts[3] === 'contract-status' && method === 'patch') {
      return ok(config, updateMockContract(orderId, String(payload.status || 'prepared'), payload.comment ? String(payload.comment) : undefined));
    }
    ...
    if (orderId && parts[3] === 'laboratory') {
      ...
      if (parts[4] === 'measurement') {
        ...
        if (method === 'post' && parts[5] === 'respond') return ok(config, updateMockLaboratoryMeasurementAgreementStatus(orderId, payload.action === 'reschedule' ? 'reschedule_requested' : 'accepted_by_client', payload.comment ? String(payload.comment) : undefined)?.laboratoryMeasurementAgreement);
      }
      if (parts[4] === 'results') {
    ...
    if (orderId && parts[3] === 'quarters') {
      ...
      if (quarterId && parts[5] === 'documents' && method === 'post') return ok(config, addMockQuarterDocument(orderId, quarterId, {
        ...
        documentType: String(payload.documentType || 'client_data') as never,
...
const updateByStatusPayload = (orderId: string, payload: Record<string, unknown>) => {
  if (payload.status) return updateMockOrderStatus(orderId, String(payload.status) as OrderStatus);
