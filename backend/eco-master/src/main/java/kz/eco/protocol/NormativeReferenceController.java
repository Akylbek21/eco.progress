...
public class NormativeReferenceController {
    ...
    /** Module spec §4: creating/editing/archiving the shared normative dictionary must not be
     *  reachable by LABORATORY (class-level LAB_PROTOCOL allowed it before this) - only
     *  ADMIN/DIRECTOR, enforced here in the backend, not just hidden in the frontend. */
    @PostMapping
    @PreAuthorize(SecurityExpressions.NORMATIVE_MANAGE)
