...
public class AdminAccessGrantController {
    ...
    @PostMapping
    @PreAuthorize(kz.ecoprogress.documentflow.admin.DocumentFlowAdminSecurityExpressions.DOCUMENT_FLOW_ACCESS_MANAGE)
    public ApiResponse<AdminOrganizationAccessDto> grant(@Valid @RequestBody AccessGrantRequest request,
        ...
        if (outcome instanceof DocumentFlowIdempotencyService.ReturnExisting) {
            AdminOrganizationAccessDto dto = service.getAccessContextForOrganization(request.organizationId());
            return ApiResponse.ok(dto, "Доступ уже был предоставлен ранее (idempotent replay)");
        }
        Long recordId = ((DocumentFlowIdempotencyService.Proceed) outcome).recordId();

        try {
            AdminOrganizationAccessDto dto = service.grantAccess(request, actor);
            idempotencyService.complete(recordId, request.organizationId());
            return ApiResponse.ok(dto, "Доступ предоставлен");
