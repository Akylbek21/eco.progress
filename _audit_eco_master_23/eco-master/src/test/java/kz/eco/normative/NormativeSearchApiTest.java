...
class NormativeSearchApiTest {
    ...
    /** Fixes verification: query must search name+code+CAS+formula and an empty (blank-string)
     *  filter param must be ignored entirely, never treated as "restrict to exact empty match" -
     *  see NormativeDirectoryService#applyFilters, every filter there is isBlank()-guarded. */
    @Test
    @WithMockUser(roles = "LABORATORY")
    void search_emptyCategoryCodeFilter_isIgnoredNotTreatedAsExactMatch() throws Exception {
        mockMvc.perform(get("/api/normatives/search")
                        .param("query", "азот")
                        .param("categoryCode", ""))
                .andExpect(status().isOk())
    ...
    @Test
    @WithMockUser(roles = "LABORATORY")
    void search_byFormula_findsByChemicalFormula() throws Exception {
        mockMvc.perform(get("/api/normatives/search").param("q", "NO2"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.records[?(@.pollutantCode == '0301')]").exists());
    }
    ...
    @Test
    @WithMockUser(roles = "LABORATORY")
    void health_reportsImportStatusAndCounts() throws Exception {
        mockMvc.perform(get("/api/normatives/health"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.total").exists())
                .andExpect(jsonPath("$.data.active").exists())
                .andExpect(jsonPath("$.data.importStatus").exists());
    }
}
