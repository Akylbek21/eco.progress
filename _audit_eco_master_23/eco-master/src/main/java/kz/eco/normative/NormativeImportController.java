...
public class NormativeImportController {
    ...
    public ApiResponse<NormativeImportDtos.ImportConfirmResponse> importConfirm(
        ...
        try {
            ...
            if (isUnexpected(e)) {
                // Транзакция confirm уже откатилась - фиксируем FAILED отдельной транзакцией.
                try {
                    importService.markImportFailed(importId, e.getMessage());
                } catch (RuntimeException ignored) {
                    // статус FAILED - best effort, исходная ошибка важнее
                }
            }
            throw e;
        }
    ...
    /** Статус конкретного импорта (PREVIEW / PROCESSING / COMPLETED / FAILED / ROLLED_BACK). */
    @GetMapping("/imports/{importId}")
    @PreAuthorize(SecurityExpressions.NORMATIVE_IMPORT)
    public ApiResponse<NormativeImportDtos.ImportStatusResponse> importStatus(@PathVariable Long importId) {
        return ApiResponse.ok(importService.getImportStatus(importId), "OK");
    }

    /** Канонический URL отката импорта. Ответ - актуальный статус импорта (ROLLED_BACK). */
    @PostMapping({"/imports/{importId}/rollback",
            // совместимость со старыми клиентами
            "/import/{importId}/rollback", "/import/rollback/{importId}"})
    @PreAuthorize(SecurityExpressions.NORMATIVE_IMPORT_ROLLBACK)
    public ApiResponse<NormativeImportDtos.ImportStatusResponse> rollback(@PathVariable Long importId) {
        return ApiResponse.ok(importService.rollbackImport(importId), "Импорт откачен");
    }

    private static boolean isUnexpected(Exception e) {
        return !(e instanceof kz.eco.common.exception.BadRequestException
                || e instanceof kz.eco.common.exception.ConflictException
                || e instanceof kz.eco.common.exception.NotFoundException
                || e instanceof kz.eco.common.exception.ValidationException
