...
public interface PekProgramRepository extends JpaRepository<PekProgram, Long> {
    ...
    /** Item 11 (regulation admin config): a regulation edition already stamped on a program can
     *  never be deleted. */
    ...
    /** Tenant-scoped listing for a non-global caller (list endpoints filter at the query level,
     *  never in-memory - see PekAccessService#resolveAccessibleCompanyIds). */
    List<PekProgram> findByCompanyIdIn(List<Long> companyIds);
    ...
    @Query("""
            ...
            where p.status = kz.eco.pek.PekProgramStatus.ACTIVE
              ...
              and (:companyIds is null or p.companyId in :companyIds)
