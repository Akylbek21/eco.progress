...
public class PekReportDocumentGenerationService {
    ...
    private final kz.eco.pek.PekOfficialReportDataService officialReportDataService;
    private final kz.eco.pek.PekReportReadinessService readinessService;
    ...
    public PekReportDocumentGenerationService(PekReportRepository reportRepository,
                                               ...
                                               kz.eco.pek.PekOfficialReportDataService officialReportDataService,
                                               kz.eco.pek.PekReportReadinessService readinessService) {
    ...
    public PekReportDocumentVersion generateOfficialDocx(Long reportId, Long userId) {
        PekReport report = lockReport(reportId);
        ...
        try {
            ...
            deleteStoredFileOnRollback(meta.fileId());
        } catch (IOException e) {
    ...
    public PekReportDocumentVersion generateOfficialPdf(Long reportId, Long userId) {
        ...
        try {
            ...
            deleteStoredFileOnRollback(docxMeta.fileId());
            ...
            deleteStoredFileOnRollback(pdfMeta.fileId());
        } catch (IOException e) {
    ...
    private OfficialPekReportDocValues buildOfficialValues(PekReport report) {
        ...
        var general = new OfficialPekReportDocValues.GeneralInfo(
                ...
                // Module fix: actual capacity relates to THIS reporting period only - PekProgram no
                // longer carries an actualCapacity field at all (moved to PekReport in full).
        ...
        // Module fix: the normative+actual official tables - one computation shared with GET
        // .../official-data and readiness, never re-derived here.
        kz.eco.pek.dto.PekApiDtos.OfficialReportData officialData = officialReportDataService.getOfficialData(
                report.getId(), report, readinessService.evaluate(report));
        ...
        return new OfficialPekReportDocValues(
                ...
                report.getActualCapacityUnit(), officialData.laboratory(), officialData.applicability(),
                officialData.tables());
    }
    ...
    /**
     * Takes the report row under a real {@code SELECT ... FOR UPDATE} before anything reads the
     * "latest version" counter.
     *
     * <p>The next document version is {@code MAX(version) + 1} over the whole report - one shared,
     * gapless sequence across OFFICIAL and INTERNAL, so the two kinds of document stay orderable
     * against each other. Computing that without a lock is a lost-update race: two concurrent
     * generations (official DOCX + internal PDF, or a document + the ZIP package, which generates
     * the final PDF as a side effect) both read the same MAX, both insert the same number, and one
     * dies on {@code uk_pek_report_document_versions_report_version}. That unique index is the
     * second line of defence; this lock is the first.
     *
     * <p>The lock is held for the rest of the caller's transaction, which is why every generation
     * path takes it as its FIRST step - before rendering, before storing files - so lock ordering
     * is identical everywhere and read-MAX/render/insert is atomic.
     * {@code PekReportPackageService#generate} already takes the same lock on the same row before
     * calling in here; re-entering it from the same transaction is a no-op.
     */
    private PekReport lockReport(Long id) {
        return reportRepository.findByIdForUpdate(id)
                .orElseThrow(() -> new NotFoundException("Отчёт ПЭК не найден: " + id));
    }

    /**
     * Files are written to storage before the version row commits, so a rollback after a successful
     * store (a constraint failure, or the caller's transaction failing for any other reason) would
     * leave a blob nothing references. Registering the deletion with the transaction cleans the
     * orphan up on rollback and leaves it alone on commit.
     */
    private void deleteStoredFileOnRollback(String fileId) {
        if (fileId == null
