...
public class PekProgramSectionsService {
    ...
    public void deletePoint(Long programId, Long id, Long version) {
        ...
        PekMonitoringPoint p = pointRepository.findByIdAndProgramId(id, programId)
                .orElseThrow(() -> new NotFoundException("Точка мониторинга не найдена: " + id));
        checkVersion(version, p.getVersion());
        List<PekProgramControlItem> boundItems = controlItemRepository.findByMonitoringPointId(p.getId());
        if (!boundItems.isEmpty()) {
            throw new ConflictException("Точку мониторинга \"" + p.getName() + "\" нельзя удалить: к ней привязаны"
                    + " позиции контроля (" + boundItems.stream().map(PekProgramControlItem::getName)
                    .collect(java.util.stream.Collectors.joining(", "))
                    + "). Сначала выберите для них другую точку или \"все точки\".",
                    "PEK_MONITORING_POINT_IN_USE");
        }
    ...
    private void applyInspection(PekProgramInternalInspection e, InternalInspectionRequest r) {
        ...
        if (r.frequencyType() != null && !r.frequencyType().isBlank()) {
            e.setFrequencyType(enumValue(PekFrequencyType.class, r.frequencyType()));
        }
        e.setViolations(r.violations());
        e.setCorrectiveActions(r.correctiveActions());
    }
    ...
    private InternalInspectionDto dto(PekProgramInternalInspection e) {
        return new InternalInspectionDto(e.getId(), e.getProgramId(),
                ...
                e.getDepartment(), e.getFrequencyType() == null ? null : e.getFrequencyType().name(),
                e.getViolations(), e.getCorrectiveActions());
    }
