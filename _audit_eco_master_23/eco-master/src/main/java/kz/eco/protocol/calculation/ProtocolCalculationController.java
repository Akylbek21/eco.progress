...
public class ProtocolCalculationController {
    ...
    public ApiResponse<ProtocolCalculationDtos.RawMeasurementsResponse> getRawMeasurements(
        ...
        return ApiResponse.ok(calculationService.getRawMeasurements(protocolId, resultId, CurrentUser.get().getId()));
    }
    ...
    public ApiResponse<ProtocolCalculationDtos.SaveRawMeasurementsResponse> saveRawMeasurements(
        ...
        return ApiResponse.ok(calculationService.saveRawMeasurements(
                protocolId, resultId, request, CurrentUser.get().getId()), "Сырые данные сохранены");
    }
    ...
    public ApiResponse<ProtocolCalculationDtos.CalculationResultResponse> calculateResult(
            @PathVariable Long protocolId, @PathVariable Long resultId,
            @RequestBody(required = false) ProtocolCalculationDtos.CalculateRequest request) {
        Long version = requireVersion(request);
        return ApiResponse.ok(calculationService.calculateResult(protocolId, resultId, version, CurrentUser.get().getId()));
    }
    ...
    public ApiResponse<ProtocolCalculationDtos.ProtocolCalculationSummaryResponse> calculateProtocol(
            ...
            @RequestBody(required = false) ProtocolCalculationDtos.CalculateRequest request) {
        Long version = requireVersion(request);
        return ApiResponse.ok(calculationService.calculateProtocol(protocolId, version, CurrentUser.get().getId()));
    }

    /** Module fix: version used to be optional (null meant "skip the optimistic-lock check"),
     *  which let a stale client blindly overwrite a concurrently-edited protocol's calculation
     *  results. Every calculate/raw-measurements call must now name the version it read. */
    private static Long requireVersion(ProtocolCalculationDtos.CalculateRequest request) {
        if (request == null || request.version() == null) {
            throw new kz.eco.common.exception.BadRequestException("Укажите version", "VERSION_REQUIRED");
        }
    ...
    public ApiResponse<List<ProtocolCalculationDtos.CalculationHistoryResponse>> getCalculationHistory(
            @PathVariable Long protocolId, @PathVariable Long resultId) {
        return ApiResponse.ok(calculationService.getCalculationHistory(protocolId, resultId, CurrentUser.get().getId()));
    }
}
