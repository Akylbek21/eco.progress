...
public class Protocol {
    ...
    /** P1 module fix item 8: the exact PDF snapshot approve() certified - frozen at approve()
     *  time from pdfSha256/contentVersion, distinct from pdfSourceContentVersion/pdfSha256 (which
     *  describe whatever PDF is CURRENTLY attached and move on every regeneration). sign() must
     *  re-validate against THESE frozen values, not the live pdf* fields, so a PDF regenerated
     *  after approve() (which should be impossible via the normal API once APPROVED, but is
     *  checked explicitly rather than only relied upon) can never be silently signed instead of
     *  the one that was actually approved. */
    @Column(name = "approved_pdf_hash", length = 64)
    private String approvedPdfHash;

    @Column(name = "approved_content_version")
    private Long approvedContentVersion;
    ...
    /** True when the current pdfFileId was produced by the OpenPDF fallback renderer (LibreOffice
     *  unavailable/failed), not a real LibreOffice conversion - see
     *  ProtocolDocumentGenerationService#buildPdf. Signing a fallback PDF is blocked by default
     *  (module spec: never let a client sign a document that doesn't reflect the real template). */
    private boolean pdfIsFallback;

    @Column(length = 64)
    private String docxFileId;

    /** The protocol's contentVersion at the moment the current pdfFileId was rendered - module
     *  fix: sign() must never sign a PDF that no longer reflects the protocol's actual content.
     *  clearGeneratedDocuments() nulls this out alongside docxFileId/pdfFileId on every
     *  content-changing mutation, so in practice a stale value should never survive to be
     *  compared - this field is the explicit, checkable proof of that invariant rather than the
     *  only thing enforcing it. */
    @Column(name = "pdf_source_content_version")
    private Long pdfSourceContentVersion;
    ...
    /** Soft link to a specific line item within the order named by orderId (module spec §8).
     *  kz.eco.order.Order does not currently model line items as their own entity/table - this
     *  stores the caller-supplied identifier as-is (validated only for non-blank + requiring
     *  orderId to also be set) rather than fabricating an OrderServiceItem entity/FK ahead of the
     *  order module actually needing one; ProtocolService rejects it if orderId is absent so the
     *  pair can never be stored inconsistently. */
    @Column(length = 64)
    ...
    /** Module spec §5: JPA's own {@code version} above is not a reliable optimistic-lock token by
     *  itself - editing a child row (ProtocolResult, MeasurementDevice, ProtocolSignature,
     *  environment conditions, ...) does not always flush an UPDATE against THIS row, so
     *  {@code version} can stay unchanged while the protocol's actual content changed underneath a
     *  concurrent editor. contentVersion is bumped explicitly by ProtocolService at every mutation
     *  that changes what the protocol means (header fields, results, devices, signatures,
     *  environment conditions, normative linkage, PEK links) - see ProtocolService#bumpContentVersion.
     *  Starts at 0 so a freshly-created protocol with no edits yet reports contentVersion=0. */
    @Column(nullable = false)
    ...
    public String getApprovedPdfHash() { return approvedPdfHash; }
    public void setApprovedPdfHash(String approvedPdfHash) { this.approvedPdfHash = approvedPdfHash; }
    public Long getApprovedContentVersion() { return approvedContentVersion; }
    public void setApprovedContentVersion(Long approvedContentVersion) { this.approvedContentVersion = approvedContentVersion; }
    ...
    public boolean isPdfIsFallback() { return pdfIsFallback; }
    public void setPdfIsFallback(boolean pdfIsFallback) { this.pdfIsFallback = pdfIsFallback; }
    public String getDocxFileId() { return docxFileId; }
    public void setDocxFileId(String docxFileId) { this.docxFileId = docxFileId; }
    public Long getPdfSourceContentVersion() { return pdfSourceContentVersion; }
    public void setPdfSourceContentVersion(Long v) { pdfSourceContentVersion = v; }
    ...
    /** Single source of truth for "can DELETE /{id} remove this protocol right now", shared by
     *  ProtocolService.delete() (the actual enforcement) and ProtocolPermissionService (the
     *  canDelete flag the frontend renders from) so the two can never drift apart - hiding the
     *  button in the UI is never itself the security boundary.
     *  CANCELLED/REPLACED/ARCHIVED are excluded even though some of those paths never set signedAt
     *  (e.g. DRAFT -> CANCELLED) - those are terminal workflow states meant to be reached and left
     *  via cancel()/archive(), not deleted; the "signed or published" condition alone would let a
     *  cancelled-but-never-signed protocol be deleted, silently erasing an order's cancellation
     *  history, which the rest of this codebase treats as worth keeping. */
