...
const serviceCityFallbacksPath = path.join(root, 'deploy', 'nginx-host', 'snippets', 'service-city-fallbacks.generated.conf');
...
const invalidServiceCityFallbacks = serviceCityFallbacks.filter(({ source, destination }) =>
  registryByPath.has(source) || registryByPath.get(destination)?.robots !== 'index,follow');
if (invalidServiceCityFallbacks.length) {
  throw new Error(`Invalid service-city fallbacks: ${invalidServiceCityFallbacks.map(({ source, destination }) => `${source} -> ${destination}`).join(', ')}`);
}
const serviceCityFallbackConfig = [
  '# Generated by npm run generate:sitemap. Do not edit manually.',
  '# Retired service-city combinations redirect to the indexable nationwide service page.',
  ...serviceCityFallbacks.flatMap(({ source, destination }) => [
    `location = ${source} { return 301 https://ecoprogress.kz${destination}$is_args$args; }`,
    `location = ${source}/ { return 301 https://ecoprogress.kz${destination}$is_args$args; }`,
  ]),
  '',
].join('\n');
fs.writeFileSync(serviceCityFallbacksPath, serviceCityFallbackConfig, 'utf8');
const linksToNoindex = seoPages.flatMap((page) => ['services', 'relatedLinks'].flatMap((field) =>
  (page[field] || [])
    .filter((item) => registryByPath.get(item.path)?.robots === 'noindex,follow')
    .map((item) => `/${page.slug} [${field}] -> ${item.path}`)));
if (linksToNoindex.length) {
  throw new Error(`Generated internal links target noindex pages:\n${linksToNoindex.slice(0, 20).join('\n')}`);
}
