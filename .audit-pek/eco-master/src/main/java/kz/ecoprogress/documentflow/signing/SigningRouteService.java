...
public class SigningRouteService {
    ...
    public SigningRoute createRoute(Long documentId, SigningRouteDtos.CreateSigningRouteRequest request, Long userId) {
        ...
        buildSteps(route, request.steps(), document.getOrganizationId());
    ...
    private void buildSteps(SigningRoute route, List<SigningRouteDtos.CreateStepRequest> stepRequests,
        ...
        for (SigningRouteDtos.CreateStepRequest stepReq : stepRequests) {
            ...
            for (SigningRouteDtos.CreateAssignmentRequest req : assignmentReqs) {
                ...
                if (signerType == SignerType.ORGANIZATION_MEMBER) {
                    assignment.setUserId(requireValidOrganizationMemberSigner(req.userId(), organizationId));
                } else {
    ...
    /**
     * Module spec §6: closes the confirmed IDOR where an ORGANIZATION_MEMBER assignment's
     * {@code userId} was saved straight from the client with no check at all - a caller could
     * name any user id, including one from a completely different organization, and that user
     * would end up with a real signing assignment on this document. Requires the id to belong to
     * an ACTIVE membership of the document's own organization and to hold SIGN_DOCUMENT.
     */
    private Long requireValidOrganizationMemberSigner(Long userId, Long organizationId) {
        if (userId == null) {
            throw new BadRequestException("Укажите userId для внутреннего подписанта", "ASSIGNMENT_USER_REQUIRED");
        }
        DocumentFlowMembership membership = membershipRepository.findByOrganizationIdAndUserId(organizationId, userId)
                .orElseThrow(() -> new BadRequestException(
                        "Пользователь не состоит в организации документа", "ASSIGNMENT_USER_NOT_MEMBER"));
        if (membership.getStatus() != MembershipStatus.ACTIVE) {
            throw new BadRequestException(
                    "Участник организации неактивен", "ASSIGNMENT_USER_NOT_ACTIVE");
        }
        if (!membership.getRoleCode().defaultPermissions().contains(DocumentFlowPermission.SIGN_DOCUMENT)) {
            throw new BadRequestException(
                    "У участника организации нет права подписи", "ASSIGNMENT_USER_CANNOT_SIGN");
        }
        return userId;
    }
    ...
    public SigningRoute prepareForSigning(Long documentId, SigningRouteDtos.PrepareForSigningRequest request, Long userId) {
        ...
        if (document.getStatus() != DocumentStatus.DRAFT && document.getStatus() != DocumentStatus.READY_FOR_SIGNING
                && document.getStatus() != DocumentStatus.PREPARED_FOR_SIGNING) {
        ...
        if (request != null && request.expectedVersion() != null
            ...
            throw new ConflictException("Документ был изменён другим пользователем", "DOCUMENT_VERSION_CONFLICT");
        }

        // 12. lock the version (idempotent)
        documentVersionService.lock(version.getId());

        if (document.getStatus() != DocumentStatus.PREPARED_FOR_SIGNING) {
            documentService.transitionStatus(documentId, DocumentStatus.PREPARED_FOR_SIGNING,
                    "Документ подготовлен к подписанию");
        }
    ...
    public SigningRoute sendForSigning(Long documentId, Long userId) {
        Document document = documentService.getOrThrow(documentId);
        return sendForSigning(documentId, new SigningRouteDtos.SendForSigningRequest(document.getVersion()), userId);
    }

    @Transactional
    public SigningRoute sendForSigning(Long documentId, SigningRouteDtos.SendForSigningRequest request, Long userId) {
        Document document = documentService.getOrThrow(documentId);
        accessService.requireWriteAccess(userId, document.getOrganizationId());
        if (request == null || request.expectedVersion() == null) {
            throw new kz.eco.common.exception.BadRequestException("Требуется expectedVersion", "VERSION_REQUIRED");
        }
        if (!request.expectedVersion().equals(document.getVersion())) {
            throw new ConflictException("Документ был изменён другим пользователем", "DOCUMENT_VERSION_CONFLICT");
        }
        if (document.getStatus() != DocumentStatus.PREPARED_FOR_SIGNING
                && document.getStatus() != DocumentStatus.READY_FOR_SIGNING) {
            throw new ConflictException("Сначала подготовьте документ к подписанию", "DOCUMENT_INVALID_STATUS");
        }
        SigningRoute route = getDraftOrActiveRouteOrThrow(documentId);
    ...
    public SigningRoute updateRoute(Long documentId, SigningRouteDtos.CreateSigningRouteRequest request, Long userId) {
        ...
        buildSteps(route, request.steps() != null ? request.steps() : List.of(), document.getOrganizationId());
