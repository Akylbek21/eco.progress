...
public class CmsDocumentVerificationService {
    ...
    public Result verify(String cmsBase64, byte[] documentBytes, String expectedSha256Hash) {
        ...
        try {
            ...
            // Some crypto providers reject an otherwise structurally valid CMS when the embedded
            // certificate is outside its validity window. Preserve that precise, already proven
            // status instead of degrading it to the misleading generic INVALID_CMS result.
            if (certificateStatus == VerificationStatus.EXPIRED_CERTIFICATE) {
                return new Result(null, certInfo, certificateStatus);
            }
            throw new UnprocessableEntityException(e.getMessage(), "INVALID_CMS");
        }
        if (!info.verified()) {
        ...
        return new Result(info, certInfo, certificateStatus);
    }
}
