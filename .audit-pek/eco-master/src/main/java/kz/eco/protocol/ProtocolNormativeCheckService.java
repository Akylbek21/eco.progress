...
public class ProtocolNormativeCheckService {
    ...
    private void applyNormativeFromSnapshot(ProtocolResult result) {
        ...
        if (normativeId != null) {
            NormativeResolutionOutcome outcome = applyResolvedNormativeById(result, normativeId);
            if (outcome != NormativeResolutionOutcome.RESOLVED) {
                result.setInternalStatus(outcome == NormativeResolutionOutcome.INACTIVE
                        ? ResultInternalStatus.NORMATIVE_INACTIVE
                        : ResultInternalStatus.NORMATIVE_NOT_FOUND);
            }
            return;
        }
        // DSM snapshot: sourceDocumentCode was stored when the normative was resolved from a DSM
        // directory (MongoDB). Apply the stored min/max/comparisonType directly — no reason required
        // because this is not manual entry.
        String sourceDocumentCode = stringValue(values.get("sourceDocumentCode"));
        if (sourceDocumentCode != null && !sourceDocumentCode.isBlank()) {
            BigDecimal snapMin = readDecimal(first(values, "normativeMin", "min"));
            BigDecimal snapMax = readDecimal(first(values, "normativeMax", "max"));
            BigDecimal snapValue = readDecimal(first(values, "normativeValue", "value"));
            String snapCmp = stringValue(first(values, "comparisonType"));
            result.setNormativeValue(snapValue);
            result.setMinValue(snapMin);
            result.setMaxValue(snapMax);
            if (snapCmp != null && !snapCmp.isBlank()) {
                result.setComparisonType(ComparisonType.fromApi(snapCmp));
            }
            return;
        }
        // Canonical keys (P1 fix): normativeValue/normativeMin/normativeMax/manualNormativeReason
        // with backward-compat aliases value/min/max/reason for existing stored rows.
        BigDecimal manualValue = readDecimal(first(values, "normativeValue", "value"));
        BigDecimal manualMin = readDecimal(first(values, "normativeMin", "min"));
        BigDecimal manualMax = readDecimal(first(values, "normativeMax", "max"));
        String comparison = stringValue(first(values, "comparisonType"));
        if (comparison != null && comparison.isBlank()) comparison = null;
        if (manualValue == null && manualMin == null && manualMax == null && comparison == null) {
            result.setInternalStatus(ResultInternalStatus.NORMATIVE_NOT_SELECTED);
            return;
        }
        // RANGE requires normativeMin+normativeMax; normativeValue is optional for RANGE.
        if ("RANGE".equalsIgnoreCase(comparison) || "BETWEEN".equalsIgnoreCase(comparison)) {
            if (manualMin == null || manualMax == null) {
                throw new kz.eco.common.exception.BadRequestException(
                        "RANGE требует normativeMin и normativeMax", "MANUAL_NORMATIVE_RANGE_INCOMPLETE");
            }
        }
        String reason = stringValue(first(values, "manualNormativeReason", "reason"));
        if (reason == null || reason.isBlank()) {
            throw new kz.eco.common.exception.BadRequestException(
                    "Укажите причину ручного ввода норматива (manualNormativeReason)", "MANUAL_NORMATIVE_REASON_REQUIRED");
        }
        result.setNormativeValue(manualValue);
        result.setMinValue(manualMin);
        result.setMaxValue(manualMax);
        if (comparison != null) {
            result.setComparisonType(ComparisonType.fromApi(comparison));
        }
    }

    public enum NormativeResolutionOutcome { RESOLVED, NOT_FOUND, INACTIVE }

    /** Public entry point for quick-create (module spec: "normativeId должен быть приоритетным
     *  источником норматива, не должен подменяться повторным автопоиском"). Quick-create used to
     *  always run its own automatic search first and only fall back to a client-sent normativeId
     *  if that search came up empty - meaning a user's deliberate selection could be silently
     *  overridden by an unrelated auto-match. This delegates to the exact same server-authoritative
     *  by-id resolution addResult/updateResult already use via applyNormativeFromSnapshot, so both
     *  entry points behave identically: never trust the client's value/min/max/comparisonType, only
     *  the id, always re-loaded fresh from the database. */
    public boolean applyClientSelectedNormativeById(ProtocolResult result, Long normativeId) {
        return applyResolvedNormativeById(result, normativeId) == NormativeResolutionOutcome.RESOLVED;
    }

    /** Same as {@link #applyClientSelectedNormativeById} but distinguishes NOT_FOUND from
     *  INACTIVE (module spec §10: NORMATIVE_INACTIVE is a different issue code than
     *  NORMATIVE_NOT_FOUND) - use this when the caller can surface that distinction. */
    public NormativeResolutionOutcome resolveClientSelectedNormativeById(ProtocolResult result, Long normativeId) {
        return applyResolvedNormativeById(result, normativeId);
    }
     ...
     *  Returns NOT_FOUND if the id never existed / was deleted, INACTIVE if it exists but is
     *  archived - in both non-RESOLVED cases the caller must NOT fall back to any client-sent
     *  value. */
    private NormativeResolutionOutcome applyResolvedNormativeById(ProtocolResult result, Long normativeId) {
        Optional<NormativeRecord> record = normativeRecordRepository.findById(normativeId);
        if (record.isPresent()) {
            NormativeRecord rec = record.get();
            if (!rec.isActive()) {
                return NormativeResolutionOutcome.INACTIVE;
            }
            ...
            return NormativeResolutionOutcome.RESOLVED;
        }
        ...
        return NormativeResolutionOutcome.NOT_FOUND;
    }
    ...
    private static final java.util.Set<ResultInternalStatus> PRESERVED_NO_NORMATIVE_STATUSES = java.util.Set.of(
            ResultInternalStatus.NORMATIVE_NOT_FOUND,
    ...
    public void compareResult(ProtocolResult result, List<String> warnings) {
        ...
        if (result.getComparisonType() == null) {
            // NORMATIVE_NOT_FOUND/NORMATIVE_NOT_SELECTED/NORMATIVE_INACTIVE were already set by
            // applyNormativeFromSnapshot precisely because there's no comparisonType to compare
            // against - don't let this generic branch downgrade them to a plain INFO.
            if (!PRESERVED_NO_NORMATIVE_STATUSES.contains(result.getInternalStatus())) {
