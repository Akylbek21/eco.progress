...
public class ProtocolCalculationService {
    ...
    @Transactional
    public ProtocolCalculationDtos.SaveRawMeasurementsResponse saveRawMeasurements(Long protocolId, Long resultId,
        ...
        auditService.log(protocolId, ProtocolAuditAction.UPDATED, protocol.getStatus(), protocol.getStatus(),
                userId, "Сохранены сырые данные измерений (resultId=" + resultId + ")");

        ProtocolResult saved = resultRepository.findById(resultId).orElse(result);
        // Module fix P0 item 1: protocol.getVersion() already reflects the post-saveAndFlush JPA
        // @Version increment (contentVersionService.bump saveAndFlush's the same managed entity) -
        // no extra reload/re-query is needed or performed here, which would risk reading a version
        // bumped by someone else's concurrent write instead of this transaction's own result.
        return new ProtocolCalculationDtos.SaveRawMeasurementsResponse(
                protocol.getVersion(), ProtocolResultResponseMapper.toResponse(saved));
    }

    @Transactional(readOnly = true)
    public ProtocolCalculationDtos.RawMeasurementsResponse getRawMeasurements(Long protocolId, Long resultId, Long userId) {
    ...
    @Transactional
    public ProtocolCalculationDtos.CalculationResultResponse calculateResult(Long protocolId, Long resultId,
                                                                              Long version, Long userId) {
        Protocol protocol = protocolRepository.findById(protocolId)
                .orElseThrow(() -> new NotFoundException("Протокол не найден"));
        checkVersion(protocol, version);
        accessService.assertCanEdit(userId, protocol);
        ProtocolCalculationDtos.CalculationResultResponse response = calculateResult(protocolId, resultId, userId);
        clearGeneratedDocuments(protocol);
        contentVersionService.bump(protocol);
        protocolRepository.saveAndFlush(protocol);
        auditService.log(protocolId, ProtocolAuditAction.UPDATED, protocol.getStatus(), protocol.getStatus(),
                userId, "Выполнен расчёт строки результата (resultId=" + resultId + ")");
        // P0 item 1: inject the post-flush version so the caller doesn't need a separate GET.
        return new ProtocolCalculationDtos.CalculationResultResponse(
                response.protocolId(), response.resultId(), response.result(), response.uncertaintyValue(),
                response.normativeValue(), response.internalStatus(), response.calculationStatus(),
                response.calculationMessage(), response.warnings(), response.row(), protocol.getVersion());
    }

    /** Internal, unversioned entry point - reused by {@link #calculateProtocol} so the
     *  version is checked once for the whole batch rather than once per row. */
    private ProtocolCalculationDtos.CalculationResultResponse calculateResult(Long protocolId, Long resultId, Long userId) {
        ProtocolResult result = getResultOrThrow(protocolId, resultId);
        Protocol protocol = protocolRepository.findById(protocolId)
                .orElseThrow(() -> new NotFoundException("Протокол не найден"));
    ...
    @Transactional
    public ProtocolCalculationDtos.ProtocolCalculationSummaryResponse calculateProtocol(Long protocolId, Long version, Long userId) {
        Protocol protocol = protocolRepository.findById(protocolId)
                .orElseThrow(() -> new NotFoundException("Протокол не найден"));
        checkVersion(protocol, version);
        accessService.assertCanEdit(userId, protocol);
        ...
        for (ProtocolResult r : results) {
            try {
                ...
                rows.add(new ProtocolCalculationDtos.CalculationResultResponse(
                        ...
                        CalculationStatus.ERROR.name(), e.getMessage(), List.of(e.getMessage()), null, null));
            }
        }
        ...
        if (!results.isEmpty()) {
            ...
            protocolRepository.saveAndFlush(protocol);
            auditService.log(protocolId, ProtocolAuditAction.UPDATED, protocol.getStatus(), protocol.getStatus(),
                    userId, "Выполнен расчёт протокола (" + results.size() + " строк)");
        }
    ...
    @Transactional(readOnly = true)
    public List<ProtocolCalculationDtos.CalculationHistoryResponse> getCalculationHistory(Long protocolId, Long resultId, Long userId) {
        accessService.assertCanView(userId, protocolId);
    ...
    private ProtocolCalculationDtos.CalculationResultResponse buildResponse(Long protocolId, ProtocolResult result, List<String> warnings) {
        return new ProtocolCalculationDtos.CalculationResultResponse(
                ...
                result.getCalculationStatus(), result.getCalculationMessage(),
                warnings, ProtocolResultResponseMapper.toResponse(result), null);
    }

    /** Module fix: version is now mandatory for calculate/raw-measurements - a caller must always
     *  name the version it read (400 VERSION_REQUIRED if omitted), and a mismatch is a 409 the
     *  caller must resolve by re-fetching before retrying. */
