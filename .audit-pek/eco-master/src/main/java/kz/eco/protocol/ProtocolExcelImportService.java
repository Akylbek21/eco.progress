...
public class ProtocolExcelImportService {
    ...
    @Transactional
    public ProtocolApiDtos.ProtocolResponse importExcel(Long protocolId, MultipartFile file, Long version, Long userId) throws IOException {
        if (file == null || file.isEmpty()) {
            throw new BadRequestException("Загрузите файл Excel");
        }
        // WorkbookFactory auto-detects legacy .xls (HSSF) vs .xlsx (XSSF) from the stream, unlike
        // hardcoding XSSFWorkbook which throws on legacy .xls files.
        try (Workbook workbook = WorkbookFactory.create(file.getInputStream())) {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                protocolService.addResult(protocolId, values, userId);
            }
        }
        protocolService.checkNormatives(protocolId, version, userId);
