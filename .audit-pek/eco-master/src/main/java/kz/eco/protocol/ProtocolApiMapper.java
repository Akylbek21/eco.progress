...
public class ProtocolApiMapper {
    ...
    ProtocolApiDtos.ProtocolResponse toProtocol(Protocol protocol, ProtocolTemplate template, List<ProtocolResult> results) {
        return toProtocol(protocol, template, results, null, null, List.of());
    }
    ...
    ProtocolApiDtos.ProtocolResponse toProtocol(Protocol protocol, ProtocolTemplate template,
        ...
        return toProtocol(protocol, template, results, env, null, List.of());
    }

    ProtocolApiDtos.ProtocolResponse toProtocol(Protocol protocol, ProtocolTemplate template,
                                                List<ProtocolResult> results, ProtocolEnvironmentConditions env,
                                                java.util.Map<String, List<kz.eco.common.ApiFieldError>> actionBlockers,
                                                List<ProtocolApiDtos.SamplingPointResponse> samplingPoints) {
        ...
        ProtocolApiDtos.ProtocolPermissions permissions =
        ...
        return new ProtocolApiDtos.ProtocolResponse(
                ...
                protocol.getVersion(),
                protocol.getContentVersion(),
                // Module spec §6: the "which revision was/would be signed" token - every existing
                // signature row for this protocol shares the same protocolVersion by construction
                // (see ProtocolService.sign()'s javadoc: only the first signature's save() ever
                // changes protocol.version while signatures are being collected), so any row's
                // value is the current signing revision; unsigned, it's whatever WOULD be captured
                // if signed right now.
                !signatureRows.isEmpty() ? signatureRows.get(0).getProtocolVersion() : protocol.getVersion(),
                permissions,
                permissions.toAvailableActions(),
                signatureRows.size(),
                signingProperties.getMaxSignatures(),
                signedByCurrentUser,
                signatures,
                protocol.getOrderId(),
                protocol.getOrderServiceItemId(),
    ...
    private ProtocolApiDtos.EnvironmentData toEnvironment(ProtocolEnvironmentConditions env) {
        ...
        return new ProtocolApiDtos.EnvironmentData(
                ...
                new ProtocolApiDtos.EnvironmentData.TypeConditions(
                        env.getSeason(),
                        env.getWorkCategory(),
                        env.getRoomType(),
                        env.getWorkplaceType(),
                        env.getLightingType(),
                        env.getNoiseType(),
                        env.getVisualWorkCategory(),
                        env.getNormLevel(),
                        env.getSampleNumber(),
                        env.getSamplingDepth(),
                        env.getSamplingPlace(),
                        env.getWaterType(),
                        env.getWaterUseCategory(),
                        env.getFactorType()
                )
        );
    }
    ...
    List<ProtocolApiDtos.HistoryItem> loadHistory(Long protocolId) {
        return auditLogRepository.findByProtocolIdOrderByCreatedAtDescIdDesc(protocolId).stream()
