...
public class DataSeeder implements CommandLineRunner {
    ...
    private News news(String id, String title, String excerpt, String category, LocalDate date,
        ...
        // Seeded demo articles are already-reviewed, live content - matches the migration's
        // treatment of pre-existing rows (backfilled to PUBLISHED) rather than defaulting new
        // rows to DRAFT, so dev/demo environments show indexable articles out of the box.
        n.setReviewStatus(ContentStatus.PUBLISHED);
        userRepository.findByEmailIgnoreCase("admin@ecoprogress.kz").ifPresent(admin -> {
            n.setAuthorId(admin.getId());
            n.setReviewerId(admin.getId());
        });
        n.setReviewedAt(java.time.Instant.now());
        return n;
    }
