...
class ProtocolSigningApiTest extends ProtocolApiTestSupport {
    ...
    void setUp() throws Exception {
        ...
        addResultViaDraftBatch(mockMvc, String.valueOf(protocolId), 0);

        Long v0 = protocolRepository.findById(protocolId).orElseThrow().getVersion();
        ...
        Long v1 = protocolRepository.findById(protocolId).orElseThrow().getVersion();
        protocolService.readyForApproval(protocolId, v1, labUser.getId());
        Long v2 = protocolRepository.findById(protocolId).orElseThrow().getVersion();
        protocolService.approve(protocolId, v2, labUser.getId());
    }
    ...
    private MvcResult signAs(User user, UserRole role) throws Exception {
        ...
        Long version = protocolRepository.findById(protocolId).orElseThrow().getVersion();
        String body = "{\"cmsSignatureBase64\":\"" + cms + "\", \"version\": " + version + "}";
    ...
    /** Module fix (closing the sign bypass): the controller's @PreAuthorize(LAB_PROTOCOL) is
     *  intentionally broad (it also allows a LABORATORY user to complete+sign a READY protocol on
     *  their own - module spec item 1's shortcut, see laboratoryRole_canSignReadyProtocolWithoutApproval
     *  below), but ProtocolService.sign() itself now re-checks the exact same role rule
     *  ProtocolPermissionService.canSign advertises: once a protocol has reached APPROVED (the
     *  formal review path), only a supervisor (ADMIN/DIRECTOR/HEAD) may sign it - a LABORATORY user
     *  calling the endpoint directly must be rejected, not silently allowed through. */
    @Test
    void laboratoryRole_cannotSignApprovedProtocol() throws Exception {
        byte[] pdfBytes = protocolService.downloadPdf(protocolId, labUser.getId()).inputStream().readAllBytes();
        String cms = TestCmsSigner.signAttached(pdfBytes);
        Long version = protocolRepository.findById(protocolId).orElseThrow().getVersion();
        MvcResult result = mockMvc.perform(post("/api/protocols/" + protocolId + "/sign")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"cmsSignatureBase64\":\"" + cms + "\", \"version\": " + version + "}"))
                .andReturn();
        assertEquals(409, result.getResponse().getStatus(), result.getResponse().getContentAsString());
        assertEquals("PROTOCOL_SIGN_FORBIDDEN", JsonPath.read(result.getResponse().getContentAsString(), "$.code"));
    }
