...
/**
 * MUST stay {@code @Transactional} - see IdempotentImportTest's class javadoc for why: without
 * it, the (now-removed) manual {@code deleteAll()} cleanup here was a real committed delete
 * against the whole shared normative_records/pollutants/import_batches tables, permanently
 * wiping the startup-seeded DSM_70/32/15/138 data for every test class that happened to run
 * afterward in the same JVM. @Transactional makes every write here (including the imports this
 * class exists to test) roll back automatically at the end of each test method instead.
 *
 * <p>The {@link #cleanupBefore()} step below is now load-bearing in a way it wasn't when this
 * class relied on @AfterEach-only cleanup: NormativeResourceSeeder commits real DSM_70/32/15/138
 * data at application startup, and that data is visible (not rolled back) in every test's own
 * transaction - re-importing the same MPC_atmospheric_air_with_pollutant_codes.xls.xls file
 * without first clearing existing rows makes every record a duplicate (imported()==0), which is
 * exactly what several tests here assert should NOT happen.
 */
@SpringBootTest
@Transactional
