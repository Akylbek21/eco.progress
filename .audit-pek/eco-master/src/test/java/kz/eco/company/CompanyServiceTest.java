...
class CompanyServiceTest {
    ...
    void setup() {
        ...
        // ADMIN (global CompanyAccessService access, module: tenant isolation) - this suite tests
        // plain service behavior, not the COMPANY_* role matrix (see CompanyApiTest for that), and
        // calling CompanyService directly bypasses @PreAuthorize entirely so only
        // CompanyAccessService's membership check is in play here.
    ...
    void createAndSearchAndUpdateAndArchiveCompany() {
        ...
        CompanyDtos.CompanyDto archived = companyService.archive(created.id(), updated.version(), userId);
    ...
    void deleteUsedCompanyArchivesIt() {
        ...
        // delete() is now always a soft-archive (module spec: no physical DELETE anywhere in this
        // module), regardless of whether the company is referenced by protocols.
        String message = companyService.delete(company.id(), company.version(), userId);
        Company reloaded = companyRepository.findById(company.id()).orElseThrow();
        assertEquals("Компания архивирована", message);
