...
Основной текущий workflow заявки:
...
- `Договор`;
- `Счет на оплату`;
- рабочий этап услуги: `Проектирование`, `Лаборатория`, `Вывоз` или `Утилизация`;
- `Проверка результата`;
- `Готово`;
- `Завершено`;
- `Отменено`.

Годовая заявка использует статус `annual_active` и остается активной весь период договора. Она закрывается только после завершения всех 4 кварталов, загрузки результатов по каждому кварталу и отсутствия активных долгов.

Старые статусы `new`, `in_review`, `waiting_client_documents`, `contract_pending`, `payment_pending`, `in_work`, `quality_check`, `ready`, `completed`, `cancelled` допустимы только как входные значения миграции старых данных и нормализуются при чтении.
...
1. Создать `orders` со статусом `Консультация` или `annual_active` для годовой заявки.
...
- `new`, `in_review`, `waiting_client_documents`, `contract_pending`, `payment_pending`, `in_work`, `quality_check`, `ready`, `completed`, `cancelled` больше не являются основной бизнес-логикой;
- старые значения нормализуются в текущие русскоязычные этапы;
- для годовых заявок незавершенный статус нормализуется в `annual_active`;
...
`node_modules` и `dist` не должны быть частью исходников. Для чистой проверки:
...
На Windows в песочнице Codex `vite/esbuild` может получить `Access is denied`; вне песочницы команда `npm run build` должна проходить.

### Квартальные оплаты и долги

- Источник финансовых данных квартала: `financeContracts[].quarterlySchedule[]`.
- `Order.quarters[]` хранит рабочее состояние, документы, результаты и комментарии; финансовые поля подтягиваются из договора.
- Оплата квартала всегда идет через общий сервис квартальной финансовой логики, создает `PaymentTransaction`, пересчитывает остаток и обновляет активный долг.
- Один квартал имеет не больше одного активного долга: дедупликация идет по `contractId + quarterItemId` или `contractId + invoiceNumber`.
- В `/dashboard/payments` вкладка `Все` показывает разовые и квартальные оплаты; долги показываются отдельно во вкладке `Долги`.
