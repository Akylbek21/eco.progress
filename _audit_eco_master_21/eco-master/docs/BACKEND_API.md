...
18. [Сотрудники (`/api/admin/users`)](#сотрудники-apiadminusers)
...
| `DIRECTOR` | Директор — руководство, доступ к CMS-контенту |
...
Единственный источник истины — enum `kz.eco.user.UserRole`; всё, кроме `CLIENT`, считается
staff-ролью (`UserRole.staffRoles()`). Роли `LAB_HEAD` в системе нет ни на уровне enum, ни в
security/permissions — запросы с ней отклоняются с `400 INVALID_ROLE`.
...
Завершение регистрации сотрудника, созданного администратором. Публичный (токен из письма
и есть аутентификация). Одноразовый токен действует 3 суток.
...
**Response:** `{ "data": null, "message": "Пароль установлен", "success": true }`.

После успеха: у пользователя проставляется `passwordHash`, статус `pending_setup` → `active`,
токен помечается `USED` (повторное использование невозможно), все ранее выданные JWT
инвалидируются, пишется audit-событие `PASSWORD_SETUP_COMPLETED`.

**Ошибки:** `SETUP_TOKEN_INVALID` (токен не найден / уже использован / отозван),
`SETUP_TOKEN_EXPIRED` (истёк срок), `PASSWORD_TOO_SHORT`.

Вариант с токеном в пути — `POST /api/auth/setup-password/{token}` — сохранён для уже
разосланных ссылок. Предпочтителен вариант с токеном в теле: токен не попадает в логи
прокси и историю браузера.
...
```json
{
  "data": {
    ...
    "position": null,
    "status": "active",
    "companyPermissions": { "COMPANY_VIEW": true },
    "permissions": ["PEK_VIEW", "PEK_PROGRAM_CREATE", "..."]
  },
  "message": null
}
```

### Поле `permissions` (права ПЭК)

`Set<String>` с разрешениями `PEK_*` для роли пользователя. Возвращается везде, где отдаётся
`UserResponse`: `GET /api/auth/me`, `POST /api/auth/login`, `POST /api/auth/staff/login`,
`POST /api/auth/register`.

Источник — `kz.eco.pek.PekPermissionMatrix`, которая **выводится из тех же** SpEL-выражений
`PekSecurityExpressions`, что стоят в `@PreAuthorize` на эндпоинтах ПЭК. Отдельной копии матрицы
нет — поэтому список не может разойтись с реальными проверками.

Поле **справочное, для отрисовки UI**. Авторизация им не подменяется: каждый эндпоинт ПЭК
по-прежнему проверяет `@PreAuthorize` и company-scope через `PekAccessService`. Подделка
`permissions` на клиенте ничего не даёт. Фронтенду не нужно (и нельзя) добавлять обход
вида «ADMIN разрешено всё» — ADMIN получает полный набор с бэкенда.

Для `CLIENT` набор пустой. Полный перечень имён — `PekPermissionMatrix.allPermissions()`.
...
Скрипт проходит сценарий из ТЗ: вход под ADMIN, `GET /api/auth/me` со всеми 22 правами,
открытие раздела ПЭК, создание программы, создание отчёта, настройки ПЭК. По умолчанию
данные не пишет — права на запись проверяются пробой (403 = права нет, любой другой код =
гейт пройден). Реальное создание включается флагом `--with-writes --company-id N --object-id N`.
...
## Сотрудники: `/api/admin/users`

Весь модуль доступен **только роли `ADMIN`** (`@PreAuthorize("hasRole('ADMIN')")` на контроллере
плюс правило `/api/admin/**` в `SecurityConfig`). Сотрудник без роли ADMIN получает `403` в том
числе при попытке изменить чужую запись напрямую по `id` (PATCH `/api/admin/users/{id}`).

### Жизненный цикл сотрудника

```
ADMIN создаёт сотрудника  ->  status=pending_setup, passwordHash=null
        -> генерируется одноразовый setup-токен (в БД только SHA-256 хэш, TTL 3 суток)
        -> письмо со ссылкой кладётся в email_outbox (та же очередь с ретраями, что и остальная почта)
сотрудник вызывает POST /api/auth/setup-password  ->  passwordHash проставлен, токен USED, status=active
```

Администратор **никогда не задаёт пароль** и пароль **никогда не отправляется письмом**.
Письмо содержит только ссылку. Сырой токен не логируется и не попадает в audit — в audit
пишется только факт события и срок действия.

### `POST /api/admin/users`

Поля `password` в запросе нет. Обязательны: `email` (валидный, уникальный), `name`, `role`.
Для сотрудника дополнительно обязательны `phone`, `city`, `position`. `iin` необязателен,
но если указан — ровно 12 цифр и уникален среди пользователей.

Допустимые роли (единый источник истины — enum `kz.eco.user.UserRole`):
`ADMIN`, `DIRECTOR`, `HEAD`, `MANAGER`, `ACCOUNTANT`, `ECOLOGIST`, `LABORATORY`,
`WASTE_SPECIALIST`, а также `CLIENT` (для клиентских кабинетов). Роли `LAB_HEAD` в системе
нет — запрос с ней возвращает `400 INVALID_ROLE`.

**Коды ошибок:** `EMAIL_ALREADY_EXISTS` (409), `IIN_ALREADY_EXISTS` (409), `INVALID_ROLE` (400),
`INVALID_IIN_FORMAT` (400), `VALIDATION_ERROR` (400). Стек-трейсы клиенту не возвращаются
никогда — при внутренней ошибке приходит `traceId` для поиска в логах.

### `GET /api/admin/users`

Параметры: `page` (с нуля, по умолчанию 0), `limit` (1..100, по умолчанию 20), `search`
(по email и имени), `status`, `role`, `sort` (`поле,asc|desc`; поля: `email`, `name`, `role`,
`status`, `createdAt`).

По умолчанию удалённые (`status=deleted`) не показываются. Ошибка параметров возвращается как
`400`, а **не** маскируется пустым списком.

**Response** (`ApiResponse<PageResponse<AdminUserResponse>>`):
```json
{
  "success": true,
  "data": {
...
Частичное обновление: переданы только изменяемые поля. Непереданные поля сохраняются как есть —
редактирование ФИО/телефона/должности **не сбрасывает** `passwordHash`, статус, роль и активный
setup-токен.

Перевести `pending_setup` → `active` вручную нельзя, пока пользователь не установил пароль по
ссылке (иначе получился бы «активный» аккаунт без пароля, в который невозможно войти):
`400 PASSWORD_SETUP_PENDING`. То же ограничение действует для `PATCH /api/admin/users/{id}/status`.

### Audit

В `audit_logs` (`entityType=USER`, `entityId` = id затронутого пользователя) пишутся события:
`USER_CREATED`, `SETUP_LINK_CREATED`, `PASSWORD_SETUP_COMPLETED`, `USER_UPDATED`,
`USER_STATUS_CHANGED`, `USER_ROLE_CHANGED`, `USER_DELETED`.

Ни одно из них не содержит пароль, хэш пароля или сырой setup-токен.
