...
class PekProgramFieldRoundTripTest {
    ...
    void allFormFields_surviveCreateGetPatchGetAutosaveGet() throws Exception {
        ...
        MvcResult created = mvc.perform(post("/api/pek/programs")
                ...
                .andExpect(jsonPath("$.data.facilitySnapshot.designCapacityUnit").value("т/год"))
        ...
        assertThat(column("design_capacity_unit", programId)).isEqualTo("т/год");
        ...
        for (int i = 1; i <= 3; i++) {
            String body = """
                {"facilitySnapshot": {"facilityInformation": null, "kato": null, "binSnapshot": null,
                 ...
                 "productionCharacteristics": null, "designCapacityUnit": "шаг %d",
            ...
            assertThat(returned)
                    ...
                    .isEqualTo(persisted);
            assertThat(column("design_capacity_unit", programId)).isEqualTo("шаг " + i);
            version = returned;
        }
    }
}
