...
class CompanyPaginationAndLifecycleApiTest {
    ...
    /** Attaches auth to a single request explicitly, rather than relying on the ambient
     *  SecurityContextHolder value - needed when authenticating as a second identity after an
     *  earlier mockMvc.perform() already ran within the same test method (the security filter
     *  chain's context-persistence machinery does not reliably pick up a SecurityContextHolder
     *  mutation made between two perform() calls in that scenario). */
    private RequestPostProcessor asRole(UserRole role) {
        User user = new User();
        user.setEmail("company-lifecycle-" + role + "-" + System.nanoTime() + "@ecoprogress.kz");
        user.setPasswordHash(passwordEncoder.encode("demo123"));
        user.setName(role + " Tester");
    ...
    void filter_archivedOnly_excludesActiveCompanies() throws Exception {
        ...
        mockMvc.perform(post("/api/companies/" + archivedId + "/archive").param("version", "0")).andExpect(status().isOk());
    ...
    void archive_cascadesToActiveObjects() throws Exception {
        ...
        mockMvc.perform(post("/api/companies/" + companyId + "/archive").param("version", "0")).andExpect(status().isOk());
    ...
    void restore_reactivatesCompany_butNotItsObjects() throws Exception {
        ...
        mockMvc.perform(post("/api/companies/" + companyId + "/restore").param("version", "1"))
                .andExpect(status().isOk())
    ...
    void restoreObject_reactivatesAnArchivedObject() throws Exception {
        ...
        mockMvc.perform(post("/api/companies/" + companyId + "/objects/" + objectId + "/archive").param("version", "0"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.status").value("ARCHIVED"));

        mockMvc.perform(post("/api/companies/" + companyId + "/objects/" + objectId + "/restore").param("version", "1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.status").value("ACTIVE"));
    }
    ...
    void archive_forbiddenForManager() throws Exception {
        ...
        mockMvc.perform(post("/api/companies/" + companyId + "/archive").param("version", "0").with(asRole(UserRole.MANAGER)))
    ...
    void update_normalizesPhoneAndBin() throws Exception {
        ...
        mockMvc.perform(patch("/api/companies/" + companyId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"phone\": \"+7 (701) 234-56-78\", \"version\": 0}"))
                .andExpect(status().isOk())
