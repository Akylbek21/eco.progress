-- V3: Create protocol module tables without FK constraints.
-- Indexes are inside CREATE TABLE to be compatible with MySQL 8.
...
CREATE TABLE IF NOT EXISTS protocol_audit_logs (
    ...
    comment VARCHAR(500),
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_pal_protocol_id (protocol_id)
);
...
CREATE TABLE IF NOT EXISTS company_objects (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_co_company_id (company_id)
);
