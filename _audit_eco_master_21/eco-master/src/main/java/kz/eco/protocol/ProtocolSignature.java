...
public class ProtocolSignature {
    ...
    /** Storage pointer to THIS signer's own CMS blob (module spec item 5) - previously only the
     *  first signer's CMS fileId was persisted anywhere (on the legacy Protocol.signatureFileId
     *  field); every additional signer's CMS bytes were stored but the pointer to them was
     *  discarded, making that signature's own CMS unrecoverable after the request completed. */
