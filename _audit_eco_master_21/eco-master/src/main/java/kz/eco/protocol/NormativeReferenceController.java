...
import kz.eco.normative.NormativeHealthService;
...
@PreAuthorize(SecurityExpressions.NORMATIVE_VIEW)
public class NormativeReferenceController {
    ...
    private final NormativeHealthService healthService;
    ...
    public NormativeReferenceController(NormativeReferenceService normativeService,
                                        ...
                                        NormativeHealthService healthService) {
    ...
    public ApiResponse<Map<String, Object>> health() {
        return ApiResponse.ok(healthService.health(), "OK");
    }
    ...
    public ApiResponse<Map<String, Object>> search(
        ...
        String resolvedQuery = SearchTextUtils.firstNonBlank(query, q, search, casNumber, formula);
    ...
    private static Map<String, Object> emptySearchPayload(int page, int size) {
        ...
        payload.put("matchMode", "STRICT_ACTIVE");
        payload.put("filtersApplied", Map.of());
        payload.put("filtersIgnored", List.of());
        return payload;
    }

    /* Изменение справочника - RBAC проверяется здесь, в backend, а не только скрытием кнопок:
     * создание/редактирование - NORMATIVE_EDIT (ADMIN/DIRECTOR/HEAD),
     * архивирование/восстановление - NORMATIVE_ARCHIVE (ADMIN/DIRECTOR),
     * LABORATORY и MANAGER - только чтение (class-level NORMATIVE_VIEW). */

    @GetMapping("/{id:\\d+}")
    public ApiResponse<ProtocolApiDtos.NormativeRecord> get(@PathVariable Long id) {
        return ApiResponse.ok(normativeManagementService.get(id), "OK");
    }

    @PostMapping
    @PreAuthorize(SecurityExpressions.NORMATIVE_EDIT)
    public org.springframework.http.ResponseEntity<ApiResponse<ProtocolApiDtos.NormativeRecord>> create(
        ...
        return org.springframework.http.ResponseEntity.status(org.springframework.http.HttpStatus.CREATED)
                .body(ApiResponse.ok(normativeManagementService.create(request), "Норматив создан"));
    }

    @RequestMapping(value = "/{id:\\d+}", method = {RequestMethod.PUT, RequestMethod.PATCH})
    @PreAuthorize(SecurityExpressions.NORMATIVE_EDIT)
    ...
    /** Soft-delete: норматив архивируется (active=false), физически не удаляется - на него могут
     *  ссылаться протоколы и журналы. Возвращает архивированный DTO. */
    @DeleteMapping("/{id:\\d+}")
    @PreAuthorize(SecurityExpressions.NORMATIVE_ARCHIVE)
    public ApiResponse<ProtocolApiDtos.NormativeRecord> delete(@PathVariable Long id) {
    ...
    @PreAuthorize(SecurityExpressions.NORMATIVE_ARCHIVE)
    public ApiResponse<ProtocolApiDtos.NormativeRecord> restore(@PathVariable Long id) {
        NormativeManagementService.RestoreResult result = normativeManagementService.restore(id);
        return ApiResponse.ok(result.record(), result.alreadyActive() ? "Норматив уже активен" : "Норматив восстановлен");
    }
    ...
    @PreAuthorize(SecurityExpressions.NORMATIVE_IMPORT)
