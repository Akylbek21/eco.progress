...
public class PekReportReadinessService {
    ...
    public PekApiDtos.ReadinessResponse evaluate(PekReport report) {
        ...
        // Module fix: the official (state-facing) report is a distinct set of blocking checks on
        // top of PLAN/FACT - 100% plan/fact completion must NOT mean READY while the official
        // normative/actual tables are incomplete (module spec: "100% PLAN/FACT не должно означать
        // READY, если официальный отчёт неполный"). Additive only - never replaces the checks above.
        issues.addAll(officialReportDataService.officialReadinessIssues(report, settings.isRequireOfficialReportComplete()));
