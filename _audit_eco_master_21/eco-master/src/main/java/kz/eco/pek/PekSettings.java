...
public class PekSettings {
    ...
    /** Module fix (official report structure): whether GENERAL_DATA_COMPLETE/ACTUAL_CAPACITY_REQUIRED/
     *  LABORATORY_REQUIRED/ACCREDITATION_REQUIRED/OFFICIAL_TABLES_GENERATED/REQUIRED_TABLE_INCOMPLETE/
     *  MISSING_xxx/UNRESOLVED_EXCEEDANCE (see PekOfficialReportDataService#officialReadinessIssues)
     *  actually block submit/approve/activate, or only surface as warnings. Defaults to false so
     *  every existing company's report workflow keeps behaving exactly as before this iteration -
     *  a company opts into the stricter official-report gate explicitly, the same as every other
     *  blockSubmitWith* flag here. */
    @Column(name = "require_official_report_complete", nullable = false) private boolean requireOfficialReportComplete = false;
    ...
    public boolean isRequireOfficialReportComplete(){return requireOfficialReportComplete;} public void setRequireOfficialReportComplete(boolean v){requireOfficialReportComplete=v;}
