...
/**
 * File-type allowlist check for the CRM document library, replicating the validation pattern used
 * by kz.eco.signaturedoc.SignatureDocumentFileValidator (extension AND declared Content-Type AND,
 * where feasible, magic bytes) WITHOUT importing/depending on that module - each module keeps its
 * own independent allowlist/implementation per project convention.
 *
 * <p>Explicitly rejects executables/HTML/JS/SVG and double extensions (e.g. {@code file.pdf.exe})
 * by only recognizing the single final extension against a closed allowlist - a name like
 * {@code file.pdf.exe} resolves to extension {@code exe}, which is not in {@link #ALLOWED}.
 */
final class CrmDocumentFileValidator {
    ...
    private static final Map<String, AllowedType> ALLOWED = Map.ofEntries(
            ...
            Map.entry("jpeg", new AllowedType(Set.of("jpg", "jpeg"), Set.of("image/jpeg"))),
            Map.entry("png", new AllowedType(Set.of("png"), Set.of("image/png"))),
            Map.entry("zip", new AllowedType(Set.of("zip"), Set.of("application/zip", "application/x-zip-compressed")))
    );
    ...
    private CrmDocumentFileValidator() {
    }

    static void validate(String originalFileName, String declaredContentType, byte[] content, long maxFileSizeBytes) {
        if (originalFileName == null || originalFileName.isBlank()) {
            throw new BadRequestException("Имя файла не указано", "CRM_DOCUMENT_FILE_TYPE_NOT_ALLOWED");
        }
        sanitizeFileName(originalFileName);

        if (content.length == 0) {
            throw new BadRequestException("Файл пуст", "CRM_DOCUMENT_FILE_TYPE_NOT_ALLOWED");
        }
        // Business-level size check, independent of (and stricter/clearer than) the servlet-level
        // spring.servlet.multipart.max-file-size - see application.properties, currently 30MB, so
        // this 413 fires before the generic multipart limit would ever kick in.
        if (content.length > maxFileSizeBytes) {
            throw new PayloadTooLargeException(
                    "Размер файла превышает допустимый лимит (" + (maxFileSizeBytes / (1024 * 1024)) + " МБ)",
                    "CRM_DOCUMENT_FILE_TOO_LARGE");
        }

        String extension = extensionOf(originalFileName);
        AllowedType allowedType = ALLOWED.get(extension);
        if (allowedType == null) {
            throw new BadRequestException("Недопустимый тип файла: ." + extension, "CRM_DOCUMENT_FILE_TYPE_NOT_ALLOWED");
        }
        ...
        if (!normalizedContentType.isEmpty() && !allowedType.mimeTypes().contains(normalizedContentType)) {
            throw new BadRequestException(
                    ...
                    "CRM_DOCUMENT_FILE_TYPE_NOT_ALLOWED");
        }
    ...
    static String sanitizeFileName(String originalFileName) {
        ...
        if (name.contains("..") || name.contains("/") || name.contains("\\") || name.isBlank()) {
            throw new BadRequestException("Недопустимое имя файла", "CRM_DOCUMENT_FILE_TYPE_NOT_ALLOWED");
        }
        return name;
    }
    ...
    private static String extensionOf(String fileName) {
        ...
        if (dot < 0 || dot == name.length() - 1) {
            throw new BadRequestException("У файла отсутствует расширение", "CRM_DOCUMENT_FILE_TYPE_NOT_ALLOWED");
        }
    ...
    private static void validateMagicBytes(String extension, byte[] content) {
        boolean ok = switch (extension) {
            ...
            case "docx", "xlsx", "pptx", "zip" -> startsWith(content, ZIP_MAGIC);
            ...
            // only enforcement, documented limitation (matches signaturedoc's approach).
            default -> true;
        };
        if (!ok) {
            throw new BadRequestException(
