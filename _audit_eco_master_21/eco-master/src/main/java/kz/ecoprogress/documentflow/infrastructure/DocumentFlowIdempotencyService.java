...
public class DocumentFlowIdempotencyService {
    ...
    /** A PROCESSING row older than this is presumed to belong to a request whose original thread
     *  crashed/timed out before it could ever call complete()/fail() (process kill, uncaught
     *  error not covered by the controller's catch, ...) - module fix: previously PROCESSING was
     *  terminal forever in that case, so every retry with the same key got stuck on 409
     *  IDEMPOTENCY_KEY_IN_PROGRESS permanently. Self-heals by treating it exactly like FAILED. */
    private static final java.time.Duration PROCESSING_STALE_AFTER = java.time.Duration.ofMinutes(5);
        ...
        return switch (row.getStatus()) {
            ...
            case PROCESSING -> {
                if (row.getCreatedAt() != null
                        && row.getCreatedAt().isBefore(LocalDateTime.now().minus(PROCESSING_STALE_AFTER))) {
                    yield new Proceed(transactionalOps.resetForRetry(row.getId()).getId());
                }
