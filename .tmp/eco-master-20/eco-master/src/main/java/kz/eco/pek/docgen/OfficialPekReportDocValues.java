...
public record OfficialPekReportDocValues(
        ...
        /* Module fix: the actual normative+fact official tables (emissions/instrumental
           measurements/calculated emissions/ambient air/wastewater/water/soil/radiation/marine),
           built by PekOfficialReportDataService from PekReportResultRow - the same computation
           GET /api/pek/reports/{id}/official-data and readiness read, reused here rather than
           re-derived a third time. Appended at the end so positional construction of the fields
           above stays unaffected. */
        String actualCapacityUnit,
        PekApiDtos.LaboratorySnapshotDto laboratory,
        List<PekApiDtos.TableApplicability> tableApplicability,
        PekApiDtos.OfficialTables officialTables
) {
