...
public class PekReportProtocolSource {
    ...
    /** {@code manual} became a RESERVED word in MySQL 8.4 (alongside PARALLEL/QUALIFY/TABLESAMPLE);
     *  an unquoted identifier is a syntax error there, so the column name is explicitly quoted.
     *  Hibernate translates the backticks into the dialect's own quoting, so H2 in tests is fine. */
