 ...
 *
 * <p><b>Company-scope enforcement (Iteration 1 of the PEK module overhaul) does NOT live here.</b>
 * These SpEL expressions only ever answer "does this user's role permit this kind of action at
 * all" - they have no way to express "and only for companies this user actually belongs to". That
 * check now lives in {@link PekAccessService} ({@code requireCompanyAccess}/
 * {@code requireProgramAccess}/{@code requireReportAccess}/{@code requireObjectAccess}), called
 * imperatively from {@link PekController} alongside the {@code @PreAuthorize} on each endpoint.
 * Do not try to fold company-scoping into these constants - there is no per-user company data
 * available to a static SpEL role expression, and duplicating the check in two places would be
 * exactly the kind of scattered logic this class was written to avoid.
 */
public final class PekSecurityExpressions {
    ...
    public static final String PEK_REPORT_SIGN =
            "hasAnyRole('ADMIN','DIRECTOR','HEAD','ECOLOGIST')";
    public static final String PEK_REPORT_SUBMIT = "hasAnyRole('ADMIN','DIRECTOR','HEAD','ECOLOGIST')";
    public static final String PEK_REPORT_EXPORT = PEK_VIEW;

    public static final String PEK_ADMIN =
            "hasAnyRole('ADMIN','DIRECTOR')";
    public static final String PEK_SETTINGS_VIEW = PEK_VIEW;
    public static final String PEK_SETTINGS_EDIT = "hasAnyRole('ADMIN','DIRECTOR','HEAD')";

    /** Global (not company-scoped) "Настройка ПЭК" administration - regulation versions, deadline
     *  rules, official-table configuration. Deliberately NOT the same as {@link #PEK_SETTINGS_EDIT}:
     *  that one is a per-company operational-defaults editor gated through
     *  {@link PekAccessService#canEditSettings}, scoped to companies the caller belongs to. This
     *  configuration is nation-wide (one regulation catalog for every tenant), so it is gated on
     *  role alone with no company-membership check at all - the endpoints behind it never accept a
     *  companyId. There is no separate platform-admin role in this codebase (searched
     *  kz.eco.auth/kz.eco.user - only the company-scoped {@code ADMIN} role exists), so this reuses
     *  {@code ADMIN} in a role-only (non-company-scoped) sense; if a true platform-admin role is
     *  introduced later, narrow this to that instead. Regular staff (including LABORATORY) get only
     *  {@link #PEK_REGULATION_VIEW} - read access, never write. */
    public static final String PEK_REGULATION_ADMIN = "hasRole('ADMIN')";
    public static final String PEK_REGULATION_VIEW = PEK_VIEW;
