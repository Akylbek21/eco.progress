...
public class ProtocolResult {
    ...
    /** FK to protocol_sampling_points. Null for legacy rows and non-SZZ protocols.
     *  Mandatory for AMBIENT_AIR_SZZ (enforced at service layer, not DB constraint). */
    ...
    @Column(name = "avg_value", precision = 20, scale = 6)
    private BigDecimal avgValue;

    @Column(name = "result_value", precision = 20, scale = 6)
    private BigDecimal resultValue;

    @Column(name = "cas_number", length = 40)
    private String casNumber;

    @Column(name = "formula", length = 80)
    private String formula;

    @Column(name = "normative_document", length = 300)
    ...
    public Long getSamplingPointId() { return samplingPointId; }
    public void setSamplingPointId(Long samplingPointId) { this.samplingPointId = samplingPointId; }
