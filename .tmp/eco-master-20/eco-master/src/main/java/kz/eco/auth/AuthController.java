...
import kz.eco.auth.dto.SetupPasswordWithTokenRequest;
...
public class AuthController {
    ...
    /** Body-token form - keeps the one-time token out of URLs (and therefore out of access logs
     *  and browser history). Equivalent to the {@code /setup-password/{token}} variant below,
     *  which stays for links already sent out. */
    @PostMapping("/setup-password")
    public ApiResponse<Void> setupPassword(@Valid @RequestBody SetupPasswordWithTokenRequest request) {
        authService.setupPassword(request.token(), request.password());
