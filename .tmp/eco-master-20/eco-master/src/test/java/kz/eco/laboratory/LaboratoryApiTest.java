...
class LaboratoryApiTest {
    ...
    @Test
    void eligibleEmployees_withLaboratoryId_excludesEmployeesOfAnotherLaboratory() throws Exception {
        Laboratory otherLab = new Laboratory();
        otherLab.setName("Other Laboratory");
        otherLab.setAddress("г. Астана");
        otherLab.setAccreditationNumber("KZ.OTHER.001");
        otherLab.setAccreditationValidUntil(LocalDate.of(2030, 12, 31));
        otherLab.setActive(true);
        laboratoryRepository.save(otherLab);

        User otherLabUser = new User();
        otherLabUser.setEmail("other-lab-user-" + System.nanoTime() + "@ecoprogress.kz");
        otherLabUser.setPasswordHash(passwordEncoder.encode("demo123"));
        otherLabUser.setName("Other Lab User");
        otherLabUser.setRole(UserRole.LABORATORY);
        otherLabUser.setType(ClientType.staff);
        userRepository.save(otherLabUser);

        LaboratoryEmployee otherEmployee = new LaboratoryEmployee();
        otherEmployee.setLaboratoryId(otherLab.getId());
        otherEmployee.setUserId(otherLabUser.getId());
        otherEmployee.setFullName(otherLabUser.getName());
        otherEmployee.setEmail(otherLabUser.getEmail());
        otherEmployee.setRole("LABORATORY");
        otherEmployee.setActive(true);
        employeeRepository.save(otherEmployee);

        mockMvc.perform(get("/api/laboratories/eligible-employees")
                        .param("laboratoryId", String.valueOf(laboratoryId))
                        .with(authentication(headAuth)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data[?(@.userId == " + otherLabUser.getId() + ")]").isEmpty());
    }

    @Test
    void eligibleEmployees_withUnknownLaboratoryId_returns404() throws Exception {
        mockMvc.perform(get("/api/laboratories/eligible-employees")
                        .param("laboratoryId", "999999")
                        .with(authentication(headAuth)))
                .andExpect(status().isNotFound());
    }

    @Test
    void list_includeInactiveTrue_returnsArchivedLaboratories() throws Exception {
        Laboratory archived = new Laboratory();
        archived.setName("Archived Laboratory " + System.nanoTime());
        archived.setAddress("Адрес");
        archived.setAccreditationNumber("KZ.ARCHIVED." + System.nanoTime());
        archived.setAccreditationValidUntil(LocalDate.of(2030, 12, 31));
        archived.setActive(false);
        laboratoryRepository.save(archived);

        mockMvc.perform(get("/api/laboratories").param("includeInactive", "true"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[?(@.id == " + archived.getId() + ")]").exists());

        mockMvc.perform(get("/api/laboratories"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[?(@.id == " + archived.getId() + ")]").isEmpty());
    }
