...
public class PekProgramTestFixture {
    ...
    public Fixture programFor(Long companyId, Long objectId, int year, User owner,
        ...
        // A position without a point is CONFIGURATION_REQUIRED now, so the standard fixture binds
        // its single position to its single point, the way a correctly configured programme does.
        item.setMonitoringPointId(point.getId());
        controlItemRepository.saveAndFlush(item);
