...
class ProtocolServiceTest {
    ...
    void update_printVisibility_partiallyMergesWithoutResettingUntouchedFields() {
        ...
        ProtocolApiDtos.ProtocolResponse afterHidingOrgName = protocolService.update(id,
                updateRequestWithVisibility(new ProtocolApiDtos.ProtocolPrintVisibility(
                        false, null, null, null, null, null, null, null, null,
                        null, null, null, null, null, null, null, null, null, null), created.version()),
                userId);
        ...
        ProtocolApiDtos.ProtocolResponse afterNoOpUpdate = protocolService.update(id,
                updateRequestWithVisibility(null, afterHidingOrgName.version()), userId);
        ...
        ProtocolApiDtos.ProtocolResponse afterHidingTestBasis = protocolService.update(id,
                updateRequestWithVisibility(new ProtocolApiDtos.ProtocolPrintVisibility(
                        ...
                        null, null, null, null, null, null, null, null, null, null), afterNoOpUpdate.version()),
                userId);
        ...
        ProtocolApiDtos.ProtocolResponse afterShowingOrgName = protocolService.update(id,
                updateRequestWithVisibility(new ProtocolApiDtos.ProtocolPrintVisibility(
                        ...
                        null, null, null, null, null, null, null, null, null, null), afterHidingTestBasis.version()),
                userId);
    ...
    void update_acceptsFlatTestingMethodDocument_andGetReturnsItNested() {
        ...
        ProtocolApiDtos.UpdateProtocolRequest flatUpdate = new ProtocolApiDtos.UpdateProtocolRequest(
                ...
                null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, created.version());
    ...
    @Test
    void listTemplates_returnsTheNineCanonicalRegistryTypes() {
        List<ProtocolApiDtos.ProtocolTemplateResponse> templates = protocolService.listTemplates();
        assertEquals(9, templates.size(), "one entry per ProtocolTypeRegistry type, not per legacy DB template code");
    ...
    private ProtocolApiDtos.ProtocolResponse signWithRealCms(Long id, Long actingUserId) throws Exception {
        ...
        Long version = protocolService.get(id).version();
        return protocolService.sign(id, new ProtocolApiDtos.SignProtocolRequest(cms, version), actingUserId);
    }

    @Test
    void signedProtocol_cannotBeEdited() throws Exception {
        ProtocolApiDtos.ProtocolResponse p = createDraftWithResult();
        Long v0 = protocolService.get(Long.parseLong(p.id())).version();
        ProtocolApiDtos.ProtocolResponse updated = protocolService.update(Long.parseLong(p.id()), fullUpdate(v0), userId);
        ProtocolApiDtos.ProtocolResponse ready = protocolService.readyForApproval(Long.parseLong(p.id()), updated.version(), userId);
        protocolService.approve(Long.parseLong(p.id()), ready.version(), userId);
        signWithRealCms(Long.parseLong(p.id()), adminUserId);

        Long vFinal = protocolService.get(Long.parseLong(p.id())).version();
        assertThrows(Exception.class, () -> protocolService.update(Long.parseLong(p.id()), fullUpdate(vFinal), userId));
    }
    ...
    void replace_createsNewDraft() throws Exception {
        ...
        Long vSigned = protocolService.get(Long.parseLong(p.id())).version();
        ProtocolApiDtos.ProtocolResponse replacement = protocolService.replace(
                Long.parseLong(p.id()), new ProtocolApiDtos.ReplaceProtocolRequest("Опечатка", vSigned), userId);
    ...
    void replace_copiesPrintVisibilityAndDeviceBindingToTheNewDraft() throws Exception {
        ...
        Long v1 = protocolService.get(Long.parseLong(p.id())).version();
        ProtocolApiDtos.ProtocolResponse updated = protocolService.update(Long.parseLong(p.id()), fullUpdate(v1), userId);
        ProtocolApiDtos.ProtocolResponse ready = protocolService.readyForApproval(Long.parseLong(p.id()), updated.version(), userId);
        protocolService.approve(Long.parseLong(p.id()), ready.version(), userId);
        signWithRealCms(Long.parseLong(p.id()), adminUserId);
    ...
    @Test
    void generatePreview_returnsPdfBytes() throws Exception {
        ProtocolApiDtos.ProtocolResponse p = createDraftWithResult();
        byte[] pdf = protocolService.preview(Long.parseLong(p.id()), userId);
    ...
    void readyForApproval_requiresResults() {
        ...
        assertThrows(Exception.class, () -> protocolService.readyForApproval(Long.parseLong(p.id()), p.version(), userId));
    }
    ...
    void returnToDraft_fromReadyForApproval() {
        ...
        ProtocolApiDtos.ProtocolResponse updated = protocolService.update(Long.parseLong(p.id()), fullUpdate(v0), userId);
        ProtocolApiDtos.ProtocolResponse ready = protocolService.readyForApproval(Long.parseLong(p.id()), updated.version(), userId);
        ProtocolApiDtos.ProtocolResponse draft = protocolService.returnToDraft(Long.parseLong(p.id()), ready.version(), "Нужно исправить данные", userId);
        assertEquals("DRAFT", draft.status());
    }

    @Test
    void returnToDraft_requiresVersionAndReason() {
        ProtocolApiDtos.ProtocolResponse p = createDraftWithResult();
        long id = Long.parseLong(p.id());
        assertThrows(kz.eco.common.exception.BadRequestException.class,
                () -> protocolService.returnToDraft(id, null, "Причина", userId));
        assertThrows(kz.eco.common.exception.BadRequestException.class,
                () -> protocolService.returnToDraft(id, p.version(), null, userId));
        assertThrows(kz.eco.common.exception.BadRequestException.class,
                () -> protocolService.returnToDraft(id, p.version(), "   ", userId));
    }

    /** P1 module fix item 3: the caller's real reason must reach the audit log verbatim (not a
     *  fixed placeholder), and the log must show the actual APPROVED -> DRAFT transition with
     *  actor/timestamp/reason all present. */
    @Test
    void returnToDraft_fromApproved_recordsRealReasonInAudit() throws Exception {
        ProtocolApiDtos.ProtocolResponse p = createDraftWithResult();
        long id = Long.parseLong(p.id());
        Long v0 = protocolService.get(id).version();
        ProtocolApiDtos.ProtocolResponse updated = protocolService.update(id, fullUpdate(v0), userId);
        ProtocolApiDtos.ProtocolResponse ready = protocolService.readyForApproval(id, updated.version(), userId);
        ProtocolApiDtos.ProtocolResponse approved = protocolService.approve(id, ready.version(), userId);
        assertEquals("APPROVED", approved.status());

        String reason = "Обнаружена ошибка в единицах измерения после согласования";
        protocolService.returnToDraft(id, approved.version(), reason, userId);

        List<ProtocolAuditLog> logs = auditLogRepository.findByProtocolIdOrderByCreatedAtDescIdDesc(id);
        ProtocolAuditLog entry = logs.stream()
                .filter(l -> l.getAction() == ProtocolAuditAction.RETURNED_TO_DRAFT)
                .findFirst().orElseThrow(() -> new AssertionError("no RETURNED_TO_DRAFT audit entry found"));
        assertEquals(ProtocolStatus.APPROVED, entry.getOldStatus());
        assertEquals(ProtocolStatus.DRAFT, entry.getNewStatus());
        assertEquals(userId, entry.getUserId());
        assertEquals(reason, entry.getComment(), "the actual caller-supplied reason must be stored, not a fixed placeholder");
    ...
    private static ProtocolApiDtos.UpdateProtocolRequest updateRequestWithVisibility(
            ProtocolApiDtos.ProtocolPrintVisibility visibility, Long version) {
        return new ProtocolApiDtos.UpdateProtocolRequest(
                null, null, null, null, null, null, null, null, null, null, null,
                null, null, null, null, null, null, null, null, null, null, null, null, visibility, null, null, version
        );
    }
