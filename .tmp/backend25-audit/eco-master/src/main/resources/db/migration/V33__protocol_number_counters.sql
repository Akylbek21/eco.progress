...
-- `last_value` is backtick-quoted because LAST_VALUE is a reserved MySQL 8+ window-function name -
-- without quoting, this exact statement fails with a SQL syntax error (confirmed against a live
-- MySQL 8 instance), and the entity mapping (ProtocolNumberCounter.java) quotes it the same way so
-- every Hibernate-generated query stays consistent with the physical column.
