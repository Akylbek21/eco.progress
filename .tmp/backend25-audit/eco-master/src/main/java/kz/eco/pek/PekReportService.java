...
public class PekReportService {
    ...
    private final PekOfficialReportDataService officialReportDataService;
    ...
    public PekReportService(PekReportRepository reportRepository, PekProgramRepository programRepository,
                            ...
                            PekOfficialReportDataService officialReportDataService,
                            PekReportResultRowRepository resultRowRepository) {
        ...
        this.officialReportDataService = officialReportDataService;
        this.resultRowRepository = resultRowRepository;
    }

    /** GET /api/pek/reports/{id}/official-data - the normative+actual official report structure,
     *  assembled by {@link PekOfficialReportDataService} from persisted result rows. */
    @Transactional(readOnly = true)
    public PekApiDtos.OfficialReportData getOfficialData(Long id) {
        PekReport report = getOrThrow(id);
        return officialReportDataService.getOfficialData(id, report, readinessService.evaluate(report));
    }
    ...
    public PekApiDtos.ReportCreationContext creationContext(Long companyId, Long objectId, String periodTypeRaw,
        ...
        return creationContext(companyId, objectId, periodTypeRaw, year, quarter, null);
    }

    @Transactional(readOnly = true)
    public PekApiDtos.ReportCreationContext creationContext(Long companyId, Long objectId, String periodTypeRaw,
                                                             Integer year, Integer quarter, Long requestedProgramId) {
        ...
        List<PekProgram> active = object != null
                ...
                : List.of();
        List<PekApiDtos.ProgramResponse> activeResponses = programService.toResponses(active);

        // Program selection mirrors create(): an explicit programId must be one of this object's
        // ACTIVE programs; without one, only an unambiguous single ACTIVE program is auto-selected.
        PekProgram selected = null;
        if (object != null) {
            if (requestedProgramId != null) {
                selected = active.stream().filter(p -> p.getId().equals(requestedProgramId)).findFirst().orElse(null);
                if (selected == null) {
                    blockers.add("Выбранная программа ПЭК не является действующей программой этого объекта");
                }
                ...
                blockers.add("На объекте несколько действующих программ ПЭК - выберите programId явно");
            }
        }
        // Task 2: an ACTIVE program that simply doesn't cover this report's period must block
        // creation just as loudly as "no active program at all".
        if (selected != null && bounds != null && !programService.coversPeriod(selected, bounds[0], bounds[1])) {
            blockers.add("Программа ПЭК действует с " + selected.getValidFrom() + " по "
                    + selected.getValidUntil() + " и не покрывает выбранный период отчёта");
        }

        // Versions and deadline come only from the selected program - never invented when there
        // is none. The deadline is the same computation create() stamps on the report.
        String regulationVersion = selected != null ? selected.getRegulationCode() : null;
        String templateVersion = selected != null ? selected.getTemplateVersion() : null;
        String submissionDueDate = null;
        if (selected != null && bounds != null) {
            if (regulationVersion == null || templateVersion == null) {
                blockers.add("У действующей программы ПЭК не указана редакция НПА или версия формы");
            } else {
                try {
                    submissionDueDate = deadlineService.calculateFor(
                            PekReportType.classify(periodType, object.getSpecialMonitoringType()),
                            object.getSpecialMonitoringType(), regulationVersion, bounds[1]).toString();
                } catch (RuntimeException e) {
                    blockers.add("Не удалось рассчитать срок сдачи: " + e.getMessage());
                }
            }
            if ("v1-legacy".equals(templateVersion)) {
                warnings.add("Программа ПЭК использует устаревший шаблон формы (v1-legacy)");
            }
        }

        Long duplicateReportId = null;
        if (selected != null && periodType != null && year != null) {
            Integer quarterKey = periodType == PekPeriodType.YEAR ? null : quarter;
            duplicateReportId = reportRepository.findByObjectIdAndProgramIdAndPeriodKey(
                            object.getId(), selected.getId(), periodKeyOf(periodType, year, quarterKey))
                    .map(PekReport::getId)
                    .orElse(null);
        ...
        // Item 1: same classification create() uses to stamp PekReport.reportType - never computed
        // on the frontend. Only possible once we have both a period type and the object (for its
        // specialMonitoringType); otherwise blockers above already explain why.
        String reportType = periodType != null && object != null
                ? PekReportType.classify(periodType, object.getSpecialMonitoringType()).name()
                : null;

        // Item 2: DRAFT/RETURNED programs on this object that are behind the current regulation/
        // template edition - distinct from "programs" (ACTIVE, eligible to back a new report).
        List<PekApiDtos.ProgramRequiringUpdate> programsRequiringUpdate = object != null
                ? programService.findProgramsRequiringUpdate(companyId, object.getId())
                : List.of();
        ...
        return new PekApiDtos.ReportCreationContext(
                ...
                submissionDueDate, regulationVersion, templateVersion,
                activeResponses, selected != null ? selected.getId() : null, duplicateReportId, warnings, blockers,
                reportType, programsRequiringUpdate);
    }
    ...
    /** PATCH /api/pek/reports/{id}/general (module fix): the report-level fields that relate to
     *  THIS reporting period, currently just actualCapacity/actualCapacityUnit. Mirrors every other
     *  report mutation here - If-Match required, editable-status gated, company-scoped. */
    @Transactional
    public PekApiDtos.ReportResponse updateGeneral(Long id, PekApiDtos.UpdateReportGeneralRequest request, Long version) {
        PekReport report = getOrThrow(id);
        ...
        if (request.actualCapacity() != null) {
            report.setActualCapacity(request.actualCapacity().isBlank() ? null : request.actualCapacity().trim());
        }
        if (request.actualCapacityUnit() != null) {
            report.setActualCapacityUnit(request.actualCapacityUnit().isBlank() ? null : request.actualCapacityUnit().trim());
        }
        if (request.performedStudies() != null) report.setPerformedStudies(blankToNull(request.performedStudies()));
        if (request.monitoringResultsSummary() != null) report.setMonitoringResultsSummary(blankToNull(request.monitoringResultsSummary()));
        if (request.exceedancesSummary() != null) report.setExceedancesSummary(blankToNull(request.exceedancesSummary()));
        if (request.measuresTaken() != null) report.setMeasuresTaken(blankToNull(request.measuresTaken()));
        if (request.conclusion() != null) report.setConclusion(blankToNull(request.conclusion()));
        report.setUpdatedAt(LocalDateTime.now());
        // Actual capacity and the note's sections are printed in the report documents, so an edit
        // here makes any document already rendered from the old values stale. bump() flushes.
    ...
    PekApiDtos.ReportResponse toResponse(PekReport r) {
        ...
        List<String> linkedProtocolNumbers = sourceRepository.findByReportIdAndExcludedFalse(r.getId()).stream()
                .map(PekReportProtocolSource::getProtocolId).distinct()
                .map(id -> protocolRepository.findById(id).map(kz.eco.protocol.Protocol::getProtocolNumber).orElse(null))
                .filter(java.util.Objects::nonNull)
                .toList();
        PekApiDtos.ReadinessResponse readiness = readinessService.evaluate(r);
        String officialDataStatus = officialDataStatus(r, readiness);
        return new PekApiDtos.ReportResponse(
                ...
                iso(r.getRejectedAt()), r.getRejectionReason(), submissionRecord(r),
                r.getContentRevision() == null ? 0L : r.getContentRevision(),
                r.getRegulationVersion(), r.getRegulationCode(), r.getTemplateVersion(),
                r.getReportType() == null ? null : r.getReportType().name(),
                linkedProtocolNumbers, r.getActualCapacity(), r.getActualCapacityUnit(),
                officialReportDataService.laboratorySnapshot(r), officialDataStatus,
                readiness.warnings().stream().map(PekApiDtos.ReadinessIssue::message).toList(),
                readiness.blockingIssues().stream().map(PekApiDtos.ReadinessIssue::message).toList(),
                new PekApiDtos.ExplanatoryNoteFields(r.getPerformedStudies(), r.getMonitoringResultsSummary(),
                        r.getExceedancesSummary(), r.getMeasuresTaken(), r.getConclusion()));
    }

    /** Coarse status for a list view: NOT_COLLECTED (no official rows yet), INCOMPLETE (rows exist
     *  but readiness still has official-table blockers), COMPLETE otherwise. */
    private String officialDataStatus(PekReport r, PekApiDtos.ReadinessResponse readiness) {
        boolean hasOfficialRows = resultRowRepository.countByReportId(r.getId()) > 0;
        if (!hasOfficialRows) {
            return "NOT_COLLECTED";
        }
        boolean hasOfficialBlockers = readiness.issues().stream()
                .anyMatch(i -> "OFFICIAL_TABLES".equals(i.section()) || "LABORATORY".equals(i.section()));
        return hasOfficialBlockers ? "INCOMPLETE" : "COMPLETE";
    }

    /** Item 26: the recorded filing details must come back every time the report is reopened.
     *  Null until a submission has actually been recorded - an absent record and a record with
     *  empty fields mean different things to an auditor. */
    private PekApiDtos.SubmissionRecord submissionRecord(PekReport r) {
        if (r.getSubmissionMethod() == null && r.getSubmissionRecordedAt() == null) {
            return null;
        }
        PekApiDtos.UserShortDto submittedBy = r.getSubmittedBy() == null ? null
                : userRepository.findById(r.getSubmittedBy())
                        .map(u -> new PekApiDtos.UserShortDto(
                                u.getId(), u.getName(), u.getEmail(), u.getPosition()))
                        .orElse(null);
        return new PekApiDtos.SubmissionRecord(
                r.getSubmissionMethod() == null ? null : r.getSubmissionMethod().name(),
                r.getRegistrationNumber(),
                r.getSubmissionComment(),
                r.getConfirmationFileId(),
                iso(r.getSubmittedAt()),
                submittedBy,
                iso(r.getSubmissionRecordedAt()),
                iso(r.getSubmissionUpdatedAt()));
    }
    ...
    private java.util.Map<String, Boolean> availableActions(PekReport report) {
        ...
        // submit mirrors POST .../submit exactly: PEK_REPORT_SUBMIT role set (= author, no LABORATORY),
        // SIGNED, and the latest OFFICIAL document current and carrying a verified signature.
        boolean submit = author && report.getStatus().canTransitionTo(PekReportStatus.SUBMITTED)
                && submissionService.hasSignedCurrentOfficialDocument(report);
        ...
        // requireReportAccess already passed for this caller to reach here - see the program-side
        // note in PekProgramService#availableActions.
        actions.put("view", true);
        boolean editable = (author || laboratory) && report.getStatus().isEditable();
        actions.put("edit", editable);
        actions.put("collect", editable);
        actions.put("matchSources", editable);
        // Contract aliases for the frontend's names. Kept alongside the existing keys rather than
        // renamed so current callers keep working.
        actions.put("match", editable);
        // "validate" is running the readiness check and acting on its result: same role set and
        // same editable-status precondition as collect/match - PEK_REPORT_VALIDATE shares
        // PEK_REPORT_CREATE's role union.
        actions.put("validate", editable);
        ...
        // Item 1: a report with no package yet cannot answer this from PackageResponse, because
        // GET .../package 404s until the first one exists - so the flag has to live here too.
        // Computed by the same PekPackagePolicy that PekReportPackageService.dto() reports and
        // generate() enforces, rather than re-deriving it from the role alone.
