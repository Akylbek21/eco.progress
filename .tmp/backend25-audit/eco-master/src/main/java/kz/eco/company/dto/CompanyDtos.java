...
public final class CompanyDtos {
    ...
    /** version is mandatory (P0 module fix item 10: optimistic-lock protection on company update). */
    ...
    public record CompanyObjectDto(
            ...
            String contactPerson,
            String contactPhone,
            SpecialMonitoringType specialMonitoringType,
            CompanyStatus status,
            boolean primary,
            Long version
    ) {
    ...
    /** Partial update - roleCode/status are both optional (null means "leave unchanged"), but
     *  version is mandatory (module fix P0 item 2/3: optimistic-lock protection). */
    public record UpdateCompanyMembershipRequest(
            String roleCode,
            String status,
            Long version
    ) {
    ...
    /** version is mandatory for update (P0 module fix item 10) - ignored (may be null) on create,
     *  where there is nothing yet to check against. */
