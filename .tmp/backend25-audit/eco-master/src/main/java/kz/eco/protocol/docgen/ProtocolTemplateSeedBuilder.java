...
public final class ProtocolTemplateSeedBuilder {
    ...
    private static void addLetterhead(XWPFDocument doc) {
        // The renderer moves {{LAB_LOGO}}..{{LAB_ADDRESS}} into the 3-column letterhead table; the
        // committed templates carry this marker, so a regenerated one must too.
        XWPFParagraph logo = doc.createParagraph();
        run(logo, ProtocolDocxTemplateRenderer.LAB_LOGO_MARKER, false, BODY_SIZE);
    ...
    private static void addFieldLabels(XWPFDocument doc, ProtocolTemplateKey key) {
        ...
        String samplingPlaceLabel = switch (key) {
            case SOIL -> "Место отбора продукции:";
            case AMBIENT_AIR_SZZ -> "Точки контроля на границе СЗЗ (стороны света):";
            case INDUSTRIAL_EMISSIONS -> "Источник выброса (место отбора):";
            default -> "Место отбора:";
        };
        ...
        String conditionsLabel = isPhysicalFactor(key) ? "Условия окружающей среды:"
                : key == ProtocolTemplateKey.AMBIENT_AIR_SZZ ? "Метеорологические условия:"
                : key == ProtocolTemplateKey.INDUSTRIAL_EMISSIONS ? "Условия отбора (режим работы источника):"
