...
public enum ProtocolTemplateKey {
    ...
    AMBIENT_AIR_SZZ("protocol_ambient_air_szz.docx", ResultColumnSet.AMBIENT_AIR_SZZ),
    INDUSTRIAL_EMISSIONS("protocol_industrial_emissions.docx", ResultColumnSet.INDUSTRIAL_EMISSIONS),
    ...
    public enum ResultColumnSet {
        ...
        GENERIC,
        /** Точка контроля / сторона света | показатель, ед. | НД | ПДК | результат | доля ПДК. */
        AMBIENT_AIR_SZZ,
        /** Источник | параметры газового потока | вещество | мг/м³ | г/с | т/год | норматив г/с |
         *  отклонение, % | НД. */
    ...
    public static ProtocolTemplateKey fromTemplateId(String templateId) {
        ...
        return switch (normalized) {
            case "soil" -> SOIL;
            case "ambient_air", "ambient_air_szz", "atmospheric_air" -> AMBIENT_AIR_SZZ;
    ...
    private static ProtocolTemplateKey fromProtocolTemplateCode(ProtocolTemplateCode code) {
        ...
        return switch (code) {
            ...
            case AMBIENT_AIR_SZZ -> ProtocolTemplateKey.AMBIENT_AIR_SZZ;
            case INDUSTRIAL_EMISSIONS -> ProtocolTemplateKey.INDUSTRIAL_EMISSIONS;
