...
public interface NormativeRecordRepository extends JpaRepository<NormativeRecord, Long> {
    ...
    @Query("select n.templateType, count(n) from NormativeRecord n where n.active = true group by n.templateType")
    List<Object[]> countActiveByTemplateType();
    ...
    List<NormativeRecord> findByReplacedRecordIdIn(java.util.Collection<Long> replacedRecordIds);

    @Query("""
            SELECT n FROM NormativeRecord n
            WHERE n.active = true
              AND n.sourceDocumentCode = :sourceDocumentCode
              AND LOWER(TRIM(n.indicatorNameRu)) = LOWER(:indicatorNameRu)
            """)
    List<NormativeRecord> findActiveBySourceAndIndicator(@Param("sourceDocumentCode") String sourceDocumentCode,
                                                         @Param("indicatorNameRu") String indicatorNameRu);
