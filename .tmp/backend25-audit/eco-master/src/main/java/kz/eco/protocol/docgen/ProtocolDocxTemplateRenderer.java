...
public final class ProtocolDocxTemplateRenderer {
    ...
    /** СЗЗ: concentration at a control point against the ПДК, and its share of that ПДК. */
    private static final String[] AMBIENT_AIR_SZZ_HEADERS = {
            "Точка контроля / сторона света",
            "Наименование загрязняющего вещества, ед. измерения",
            "НД на методы испытаний",
            "ПДК",
            "Результат испытаний",
            "Доля ПДК"
    ...
    /** Industrial emissions: one row per source/substance with the gas-flow parameters the mass
     *  emission is derived from, the emission itself and its deviation from the норматив ПДВ. */
    private static final String[] INDUSTRIAL_EMISSIONS_HEADERS = {
            "Источник выброса",
    ...
    public static byte[] render(ProtocolTemplateKey key,
        ...
        return render(key, protocol, results, environment, logoBytes, logoContentType, java.util.Map.of());
    }

    public static byte[] render(ProtocolTemplateKey key, Protocol protocol, List<ProtocolResult> results,
                                ProtocolEnvironmentConditions environment, byte[] logoBytes,
                                String logoContentType,
                                java.util.Map<Long, kz.eco.protocol.ProtocolSamplingPoint> samplingPoints) throws IOException {
        ...
        try (InputStream in = new ByteArrayInputStream(templateBytes);
            ...
            insertResultsTable(doc, key, protocol, results, samplingPoints);
    ...
    private static void insertResultsTable(XWPFDocument doc, ProtocolTemplateKey key, Protocol protocol,
                                            List<ProtocolResult> results,
                                            java.util.Map<Long, kz.eco.protocol.ProtocolSamplingPoint> samplingPoints) {
        XWPFParagraph markerParagraph = findMarkerParagraph(doc);
        String[] headers = switch (key.columnSet()) {
            case SOIL -> SOIL_HEADERS;
            case AMBIENT_AIR_SZZ -> AMBIENT_AIR_SZZ_HEADERS;
            case INDUSTRIAL_EMISSIONS -> INDUSTRIAL_EMISSIONS_HEADERS;
            case GENERIC -> GENERIC_HEADERS;
        };
        ...
        if (results.isEmpty()) {
            ...
            for (int i = 0; i < results.size(); i++) {
                String[] cells = rowCells(key, protocol, results.get(i), samplingPoints);
    ...
    /** One results-table row, in the column order of the key's header set. */
    private static String[] rowCells(ProtocolTemplateKey key, Protocol protocol, ProtocolResult result,
                                     java.util.Map<Long, kz.eco.protocol.ProtocolSamplingPoint> samplingPoints) {
        Map<String, Object> values = ProtocolDocValues.valuesOf(result);
        String method = ProtocolDocValues.resolveTestingMethod(result, values, protocol);
        return switch (key.columnSet()) {
            case AMBIENT_AIR_SZZ -> new String[]{
                    ambientPlace(result, samplingPoints),
                    ProtocolDocValues.resolveIndicatorWithUnit(result),
                    method,
                    ProtocolDocValues.resolveNorm(result, values),
                    ProtocolDocValues.resolveResult(result),
                    ProtocolDocValues.pdkShare(result)};
            case INDUSTRIAL_EMISSIONS -> new String[]{
                    ProtocolDocValues.firstNonBlank(result.getPollutionSourceNumber(), protocol.getSourceNumber(),
                            result.getMeasurementPlace()),
                    ProtocolDocValues.gasFlowParameters(result),
                    ProtocolDocValues.resolveIndicatorWithUnit(result),
                    ProtocolDocValues.formatDecimal(result.getResultMgM3() != null
                            ? result.getResultMgM3() : result.getResultValue()),
                    ProtocolDocValues.formatDecimal(result.getResultGs()),
                    ProtocolDocValues.tonsPerYear(values),
                    ProtocolDocValues.formatDecimal(result.getPdvGs()),
                    ProtocolDocValues.deviationPercent(result.getResultGs(), result.getPdvGs()),
                    method};
            case SOIL, GENERIC -> new String[]{
                    placeCellValue(key, result, samplingPoints),
                    ProtocolDocValues.resolveIndicatorWithUnit(result),
                    method,
                    ProtocolDocValues.resolveNorm(result, values),
                    ProtocolDocValues.resolveResult(result)};
    ...
    /** Control point plus its cardinal direction when both are known ("ТК-1 (север)"). */
    private static String ambientPlace(ProtocolResult result,
                                       java.util.Map<Long, kz.eco.protocol.ProtocolSamplingPoint> samplingPoints) {
        String place = placeCellValue(ProtocolTemplateKey.AMBIENT_AIR_SZZ, result, samplingPoints);
        String direction = result.getDirection();
        if (direction == null || direction.isBlank()) {
            return place;
        }
        return ProtocolDocValues.EMPTY.equals(place) ? direction.trim() : place + " (" + direction.trim() + ")";
    }
    ...
    private static String placeCellValue(ProtocolTemplateKey key, ProtocolResult result,
        ...
        var point = result.getSamplingPointId() == null ? null : samplingPoints.get(result.getSamplingPointId());
        return ProtocolDocValues.firstNonBlank(point == null ? null : point.getName(),
                result.getMeasurementPlace(), result.getSamplingPlace(), result.getRoomOrWorkplace());
    }
