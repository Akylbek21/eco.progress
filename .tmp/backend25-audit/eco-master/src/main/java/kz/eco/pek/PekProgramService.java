...
public class PekProgramService {
    ...
    public PekApiDtos.ProgramResponse create(PekApiDtos.CreateProgramRequest request, Long userId) {
        ...
        applyFacilitySnapshot(program, request.facilitySnapshot(), company);
    ...
    /**
     * Explicit re-template of an editable (DRAFT/RETURNED) program onto the regulation edition
     * currently in force ({@link PekRegulationVersionService#current()}): restamps regulationCode/
     * regulationVersion/templateVersion, bumps THIS program's contentRevision and writes an audit
     * entry. APPROVED/ACTIVE/ARCHIVED/UNDER_REVIEW programs are never restamped - they keep the
     * edition they were authored/approved under. Reports are not touched: each report carries its
     * own immutable regulation/template stamp from creation time.
     */
    @Transactional
    public PekApiDtos.ProgramResponse retemplate(Long id, Long version, Long userId) {
        PekProgram program = getOrThrow(id);
        checkVersion(program, version);
        accessService.requireCompanyEditPermission(userId, actorRole(userId), program.getCompanyId());
        if (!program.getStatus().isEditable()) {
            throw new ConflictException("Актуализировать шаблон можно только у программы в статусе DRAFT или RETURNED"
                    + " (текущий статус: " + program.getStatus() + ")", "PEK_PROGRAM_RETEMPLATE_NOT_ALLOWED");
        }
        PekRegulationVersion edition = regulationVersionService.current();
        if (isOnEdition(program, edition)) {
            throw new ConflictException("Программа уже использует актуальную редакцию и шаблон ("
                    + edition.code() + ", " + edition.programTemplateVersion() + ")", "PEK_PROGRAM_TEMPLATE_CURRENT");
        }
        String before = program.getRegulationCode() + " / " + program.getTemplateVersion();
        stampCurrentRegulation(program);
        program.setContentRevision(program.getContentRevision() + 1);
        program.setUpdatedAt(LocalDateTime.now());
        programRepository.saveAndFlush(program);
        audit(program.getId(), userId, "RETEMPLATE", before,
                program.getRegulationCode() + " / " + program.getTemplateVersion(),
                "Шаблон программы актуализирован, contentRevision=" + program.getContentRevision());
        return toResponse(program);
    }

    private static boolean isOnEdition(PekProgram program, PekRegulationVersion edition) {
        return edition.code().equals(program.getRegulationCode())
                && edition.programTemplateVersion().equals(program.getTemplateVersion());
    }
    ...
    public PekApiDtos.ProgramResponse clone(Long sourceId, PekApiDtos.CloneProgramRequest request, Long userId) {
        ...
        Map<PekProgramControlItem, Long> copiesPointingAtSourcePoint = new java.util.LinkedHashMap<>();
        for (PekProgramControlItem item : controlItemRepository.findByProgramIdOrderBySortOrderAsc(source.getId())) {
            ...
            // The source item's point id names a point of the SOURCE programme; copying it as-is
            // would bind the clone to another programme's point. It is re-pointed at the clone's
            // own copy of that point once points are cloned below.
            if (item.getMonitoringPointId() != null) {
                copiesPointingAtSourcePoint.put(copy, item.getMonitoringPointId());
            }
            copy.setAppliesToAllPoints(item.isAppliesToAllPoints());
        ...
        // Направления and their points are cloned too: a control item's point setting cannot be
        // preserved without them, and without направления the clone produces no protocol
        // requirements at all.
        Map<Long, Long> oldToNewMonitoringId = new java.util.HashMap<>();
        for (PekProgramMonitoring monitoring : monitoringRepository.findByProgramId(source.getId())) {
            PekProgramMonitoring copy = new PekProgramMonitoring();
            copy.setProgramId(clone.getId());
            copy.setMonitoringType(monitoring.getMonitoringType());
            copy.setName(monitoring.getName());
            copy.setMethodology(monitoring.getMethodology());
            copy.setLaboratoryId(monitoring.getLaboratoryId());
            copy.setFrequencyType(monitoring.getFrequencyType());
            copy.setPlannedCount(monitoring.getPlannedCount());
            copy.setActive(monitoring.isActive());
            copy.setControlItemIds(monitoring.getControlItemIds().stream()
                    .map(oldToNewControlItemId::get)
                    .filter(java.util.Objects::nonNull)
                    .collect(Collectors.toCollection(java.util.LinkedHashSet::new)));
            monitoringRepository.save(copy);
            oldToNewMonitoringId.put(monitoring.getId(), copy.getId());
        }
        Map<Long, Long> oldToNewPointId = new java.util.HashMap<>();
        for (PekMonitoringPoint point : monitoringPointRepository.findByProgramIdOrderByIdAsc(source.getId())) {
            Long newMonitoringId = oldToNewMonitoringId.get(point.getMonitoringId());
            if (newMonitoringId == null) {
                continue;
            }
            PekMonitoringPoint copy = new PekMonitoringPoint();
            copy.setProgramId(clone.getId());
            copy.setMonitoringId(newMonitoringId);
            copy.setName(point.getName());
            copy.setCoordinates(point.getCoordinates());
            copy.setDescription(point.getDescription());
            monitoringPointRepository.save(copy);
            oldToNewPointId.put(point.getId(), copy.getId());
        }
        // A source point that no longer exists maps to nothing: the copy is then left without a
        // point and shows up as CONFIGURATION_REQUIRED, rather than inheriting a dangling id.
        copiesPointingAtSourcePoint.forEach((copy, sourcePointId) -> {
            copy.setMonitoringPointId(oldToNewPointId.get(sourcePointId));
            controlItemRepository.save(copy);
        });
    ...
    public PageResponse<PekApiDtos.ProgramResponse> list(Long companyId, Long objectId, String search,
        ...
        org.springframework.data.domain.Page<PekProgram> found =
                programRepository.search(companyId, objectId, statusEnum, responsibleUserId, activeOn, likeSearch, scopeList, pageable);
        java.util.Map<Long, PekApiDtos.ReadinessResponse> readiness = readinessService.evaluateAll(found.getContent());
        return PageResponse.of(found, p -> toResponse(p, null, readiness.get(p.getId())));
    }
    ...
    public List<PekApiDtos.ProgramResponse> listForObject(Long companyId, Long objectId) {
        return toResponses(programRepository.findByCompanyIdAndObjectIdOrderByValidFromDesc(companyId, objectId));
    }
    ...
    private void replaceControlItems(Long programId, List<PekApiDtos.ControlItemDto> dtos) {
        ...
        // Which направления each existing item is already linked to - loaded once for the whole
        // batch instead of per item, and used to reject a point from a different направление.
        Map<Long, Set<Long>> monitoringIdsByItem = new java.util.HashMap<>();
        for (PekProgramMonitoring m : monitoringRepository.findByProgramId(programId)) {
            for (Long itemId : m.getControlItemIds()) {
                monitoringIdsByItem.computeIfAbsent(itemId, k -> new HashSet<>()).add(m.getId());
            }
        }
        ...
        for (PekApiDtos.ControlItemDto dto : dtos) {
            ...
            boolean allPoints = Boolean.TRUE.equals(dto.appliesToAllPoints());
            // Взаимоисключающие настройки: "по всем точкам" и конкретная точка вместе означают
            // противоречивое требование, и молча выбрать одну из них - значит снова сделать за
            // пользователя неочевидный выбор, с которого началось задвоение.
            if (allPoints && dto.monitoringPointId() != null) {
                throw new BadRequestException("Позиция контроля \"" + dto.name()
                        + "\": нельзя одновременно указать конкретную точку мониторинга и"
                        + " \"все точки\" - выберите что-то одно", "PEK_CONTROL_ITEM_POINT_AMBIGUOUS");
            }
            if (dto.monitoringPointId() != null) {
                requireOwnMonitoringPoint(programId, item, dto.monitoringPointId(), dto.name(),
                        monitoringIdsByItem);
            }
    ...
    /** A control item's point is a soft link (plain Long, no FK), so nothing at the DB level stops
     *  it from naming a point that does not exist, belongs to another programme, or sits in a
     *  different направление than the item is monitored under - and any of those would silently
     *  turn into a requirement for the wrong place. The programme check also covers "same company
     *  and object": a point belongs to exactly one programme, and the programme owns both. */
    private void requireOwnMonitoringPoint(Long programId, PekProgramControlItem item, Long pointId,
                                           String itemName, Map<Long, Set<Long>> monitoringIdsByItem) {
        PekMonitoringPoint point = monitoringPointRepository.findByIdAndProgramId(pointId, programId)
                .orElseThrow(() -> new BadRequestException("Позиция контроля \"" + itemName
                        + "\": точка мониторинга " + pointId + " не найдена в этой программе ПЭК",
                        "PEK_MONITORING_POINT_INVALID"));
        Set<Long> linked = item.getId() == null ? Set.of()
                : monitoringIdsByItem.getOrDefault(item.getId(), Set.of());
        if (!linked.isEmpty() && !linked.contains(point.getMonitoringId())) {
            throw new BadRequestException("Позиция контроля \"" + itemName + "\": точка мониторинга \""
                    + point.getName() + "\" относится к другому направлению мониторинга",
                    "PEK_MONITORING_POINT_INVALID");
        }
    }
    ...
    private void replaceMeasures(Long programId, List<PekApiDtos.MeasureDto> dtos) {
        ...
        for (PekApiDtos.MeasureDto dto : dtos) {
            ...
            measure.setWorkVolume(dto.workVolume());
            measure.setEnvironmentalEffect(dto.environmentalEffect());
    ...
    /**
     * Applies the request's facility snapshot, filling {@code binSnapshot} from the company's own
     * master-data BIN when the request leaves it blank (Task 5: "не повторно вводить, если уже
     * есть в master-data"). КАТО/ОКЭД/категория/мощность have no equivalent on Company/CompanyObject
     * to auto-fill from - they stay explicit program input, never guessed.
     *
     * @param company the program's company, for the BIN fallback only - never overrides an
     *                explicitly-provided binSnapshot, and is not itself persisted as a new source
     *                of truth (module spec: "не создавать разные источники истины" - the snapshot
     *                column remains the one place a signed/approved program's БИН is read from).
     */
    private static void applyFacilitySnapshot(PekProgram program, PekApiDtos.FacilitySnapshotDto snap, Company company) {
        if (snap == null) return;
        program.setFacilityInformation(snap.facilityInformation());
        program.setKato(snap.kato());
        program.setBinSnapshot(firstNonBlank(snap.binSnapshot(), company == null ? null : company.getBin()));
        program.setOked(snap.oked());
        program.setEnvironmentalCategory(validateCategory(snap.environmentalCategory()));
        program.setDesignCapacity(snap.designCapacity());
        program.setDesignCapacityUnit(snap.designCapacityUnit());
        ...
        program.setTechnologicalProcess(snap.technologicalProcess());
        program.setMainImpactSources(snap.mainImpactSources());
    }

    /** Task 3: the official PEK program form/document (see PekProgramDocxRenderer,
     *  OfficialPekReportDocxRenderer) only exists for facilities of environmental category I or II
     *  today - normative tables and the official document structure never account for III/IV,
     *  which under Правила №250 mean a lighter-weight or no PEK obligation at all. Rejecting III/IV
     *  here, not at readiness time, so a program is never even created around a category the form
     *  cannot represent. Free text elsewhere (CompanyObject, other modules) is untouched - this
     *  check is local to program creation/edit only. Blank/null stays allowed (category may be
     *  filled in later; readiness, not this validator, enforces "required to go READY"). */
    private static final java.util.Set<String> SUPPORTED_PROGRAM_CATEGORIES = java.util.Set.of("I", "II", "1", "2");

    /** Matches the leading category token of a free-text value like "II категория" or "I" - IV
     *  before III before II before I so the alternation cannot match "I" as a false-positive
     *  prefix of "IV"/"III". */
    private static final java.util.regex.Pattern CATEGORY_TOKEN =
            java.util.regex.Pattern.compile("^(IV|III|II|I|[1-4])\\b", java.util.regex.Pattern.CASE_INSENSITIVE);
    ...
    private static String validateCategory(String category) {
        ...
        java.util.regex.Matcher m = CATEGORY_TOKEN.matcher(trimmed);
        String token = m.find() ? m.group(1).toUpperCase(java.util.Locale.ROOT) : trimmed.toUpperCase(java.util.Locale.ROOT);
        if (!SUPPORTED_PROGRAM_CATEGORIES.contains(token)) {
            throw new BadRequestException(
                    "Программа ПЭК поддерживается только для объектов I и II категории. "
                            + "Указанная категория \"" + category + "\" не поддерживается текущей формой.",
                    "PEK_CATEGORY_NOT_SUPPORTED");
        }
        return trimmed;
    }
    ...
    /** Item 2 of the creation-context contract gap list: DRAFT/RETURNED programs on this object
     *  that are no longer on the current regulation/template edition - i.e. would need
     *  {@link #retemplate} before they could ever become ACTIVE and back a report. Kept separate
     *  from findActiveForObject so creation-context never conflates "usable to create a report
     *  right now" with "needs actualization first". RETEMPLATE is offered only via
     *  availableActions, computed here the same way {@link #availableActions} does it - the
     *  frontend must never infer it on its own. */
    List<PekApiDtos.ProgramRequiringUpdate> findProgramsRequiringUpdate(Long companyId, Long objectId) {
        PekRegulationVersion edition = regulationVersionService.current();
        User user = kz.eco.auth.CurrentUser.getOrNull();
        Long userId = user == null ? null : user.getId();
        boolean canEdit = userId != null && accessService.hasCompanyEditPermission(userId, user.getRole(), companyId);
        return programRepository.findByCompanyIdAndObjectIdOrderByValidFromDesc(companyId, objectId).stream()
                .filter(p -> p.getStatus().isEditable() && !isOnEdition(p, edition))
                .map(p -> new PekApiDtos.ProgramRequiringUpdate(
                        p.getId(), p.getNumber(), p.getName(), p.getStatus().name(),
                        p.getRegulationVersion(), p.getRegulationCode(), p.getTemplateVersion(),
                        "Программа использует устаревшую редакцию НПА/шаблон ("
                                + p.getRegulationCode() + " / " + p.getTemplateVersion() + "), актуальная - "
                                + edition.code() + " / " + edition.programTemplateVersion(),
                        java.util.Map.of("retemplate", canEdit)))
                .toList();
    }
    ...
    PekApiDtos.ProgramResponse toResponse(PekProgram p, kz.eco.pek.dto.PekMonitoringDtos.ListResponse monitoring) {
        return toResponse(p, monitoring, null);
    }

    /** Responses for several programs with readiness evaluated in one batch (no per-program readiness queries). */
    List<PekApiDtos.ProgramResponse> toResponses(List<PekProgram> programs) {
        java.util.Map<Long, PekApiDtos.ReadinessResponse> readiness = readinessService.evaluateAll(programs);
        return programs.stream().map(p -> toResponse(p, null, readiness.get(p.getId()))).toList();
    }

    /** readinessPercent is ALWAYS PekProgramReadinessService#progressPercent - the same number
     *  GET /programs/{id}/readiness returns - never a separate simplified metric.
     *  @param readiness precomputed (batch) readiness, or null to evaluate here */
    PekApiDtos.ProgramResponse toResponse(PekProgram p, kz.eco.pek.dto.PekMonitoringDtos.ListResponse monitoring,
                                          PekApiDtos.ReadinessResponse readiness) {
        PekApiDtos.ReadinessResponse effectiveReadiness = readiness != null ? readiness : readinessService.evaluate(p);
        ...
        PekApiDtos.FacilitySnapshotDto facilitySnapshot = new PekApiDtos.FacilitySnapshotDto(
                ...
                p.getMonitoringScope(), p.getReadinessNotes(), p.getDesignCapacityUnit(),
                p.getTechnologicalProcess(), p.getMainImpactSources());
        ...
        return new PekApiDtos.ProgramResponse(
                ...
                effectiveReadiness.progressPercent(),
                ...
                availableActions(p, effectiveReadiness.ready()), !p.getStatus().isEditable(),
                ...
                effectiveReadiness.blockingIssues().stream().map(PekApiDtos.ReadinessIssue::message).toList(),
                effectiveReadiness.warnings().stream().map(PekApiDtos.ReadinessIssue::message).toList());
    }
    ...
    private java.util.Map<String, Boolean> availableActions(PekProgram program, boolean ready) {
        User user = kz.eco.auth.CurrentUser.getOrNull();
        ...
        // Reaching this method means requireProgramAccess already passed for this caller, so the
        // program is genuinely readable by them - the flag exists because the frontend contract
        // lists it, not because it can ever be false here.
        actions.put("view", true);
        actions.put("edit", canEdit && status.isEditable());
        boolean canSubmit = canEdit && status.canTransitionTo(PekProgramStatus.UNDER_REVIEW) && ready;
        actions.put("submit", canSubmit);
        // Contract alias: the frontend calls this action submitReview (matching the endpoint name
        // POST /programs/{id}/submit-review). Kept alongside "submit" rather than renamed, so
        // existing callers of the older key keep working.
        actions.put("submitReview", canSubmit);
        ...
        // Offered only when it would actually change something - the endpoint answers 409 otherwise.
        actions.put("retemplate", canEdit && status.isEditable()
                && !isOnEdition(program, regulationVersionService.current()));
        return actions;
    }
    ...
    private PekApiDtos.ControlItemDto toControlItemDto(PekProgramControlItem i) {
        return new PekApiDtos.ControlItemDto(
                ...
                i.getEnvironmentComponent(), i.getMonitoringPointId(), i.isAppliesToAllPoints(),
                i.getEmissionSourceId(),
    ...
    private PekApiDtos.MeasureDto toMeasureDto(PekProgramMeasure m) {
        return new PekApiDtos.MeasureDto(
                ...
                m.getWorkVolume(), m.getEnvironmentalEffect());
    }
