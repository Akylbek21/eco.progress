...
public class PekController {
    ...
    /** Official submission: SIGNED -> SUBMITTED together with the filing details (body required).
     *  A body-less call is rejected with PEK_SUBMISSION_DETAILS_REQUIRED - no placeholder details
     *  are ever invented. Returns the report with its saved submission record. */
    @PreAuthorize(PekSecurityExpressions.PEK_REPORT_SUBMIT)
    @PostMapping("/reports/{reportId}/submit")
    public ApiResponse<PekApiDtos.ReportResponse> submitReport(@PathVariable Long reportId,
                                           @RequestHeader("If-Match") Long version,
                                           @RequestBody(required = false) PekApiDtos.SubmitReportRequest request) {
        requireReportAccess(reportId);
        PekReportSubmissionService.OfficialSubmissionDetails details = request == null ? null
                : new PekReportSubmissionService.OfficialSubmissionDetails(
                        parseSubmittedAtOrNull(request.submittedAt()),
                        request.submissionMethod() == null ? null : parseSubmissionMethod(request.submissionMethod()),
                        request.registrationNumber(),
                        request.confirmationFileId(),
                        request.comment());
        submissionService.submit(reportId, version, details, CurrentUser.get().getId());
        return ApiResponse.ok(reportService.get(reportId), "Отчёт сдан");
    }
    ...
    // Distinct from /submit above: that one moves the workflow status, this one records WHERE the
    // signed report was actually filed. Gated on PEK_REPORT_SUBMIT (item 25), not PEK_REPORT_EDIT.

    @PreAuthorize(PekSecurityExpressions.PEK_REPORT_SUBMIT)
    @PostMapping("/reports/{reportId}/submission")
    public ApiResponse<PekApiDtos.ReportResponse> recordSubmission(
            @PathVariable Long reportId,
            @RequestHeader("If-Match") Long version,
            @RequestBody PekApiDtos.RecordSubmissionRequest request) {
        requireReportAccess(reportId);
        PekSubmissionMethod method = parseSubmissionMethod(request.submissionMethod());
        java.time.LocalDateTime submittedAt = parseSubmittedAt(request.submittedAt());
        submissionService.recordOfficialSubmission(reportId, version, method,
                request.registrationNumber(), request.submissionComment(),
                request.confirmationFileId(), submittedAt, CurrentUser.get().getId());
        return ApiResponse.ok(reportService.get(reportId), "Сведения о сдаче сохранены");
    }

    @PreAuthorize(PekSecurityExpressions.PEK_REPORT_SUBMIT)
    @PostMapping(value = "/reports/{reportId}/submission/file",
            consumes = org.springframework.http.MediaType.MULTIPART_FORM_DATA_VALUE)
    public ApiResponse<PekApiDtos.PermitFileUploadResponse> uploadSubmissionConfirmation(
            @PathVariable Long reportId,
            @RequestPart("file") org.springframework.web.multipart.MultipartFile file)
            throws java.io.IOException {
        requireReportAccess(reportId);
        return ApiResponse.ok(
                submissionService.uploadConfirmationFile(reportId, file, CurrentUser.get().getId()),
                "Подтверждающий файл загружен");
    }

    /** Download by REPORT id, never by raw fileId - the stored-file id is read from the report row
     *  itself, so the company-scope check above is the only way in. PEK_VIEW is enough to read a
     *  receipt for a report the caller can already see. */
    @PreAuthorize(PekSecurityExpressions.PEK_VIEW)
    @GetMapping("/reports/{reportId}/submission/file")
    public org.springframework.http.ResponseEntity<org.springframework.core.io.InputStreamResource>
            downloadSubmissionConfirmation(@PathVariable Long reportId) {
        requireReportAccess(reportId);
        kz.eco.storage.StoredFileContent file = submissionService.downloadConfirmationFile(reportId);
        String filename = file.filename() == null || file.filename().isBlank()
                ? "submission-confirmation" : file.filename();
    ...
    private static PekSubmissionMethod parseSubmissionMethod(String raw) {
        if (raw == null || raw.isBlank()) {
            throw new kz.eco.common.exception.BadRequestException(
                    "Укажите способ сдачи (submissionMethod)", "PEK_SUBMISSION_METHOD_REQUIRED");
        }
        try {
            return PekSubmissionMethod.valueOf(raw.trim().toUpperCase());
        } catch (IllegalArgumentException ex) {
            throw new kz.eco.common.exception.BadRequestException(
                    "Недопустимый submissionMethod. Допустимо: EGOV_PORTAL, EMAIL, PAPER, COURIER, OTHER",
                    "PEK_SUBMISSION_METHOD_INVALID");
        }
    }

    /** Accepts ISO local date-time or ISO date-time with offset (converted to server local time). */
    private static java.time.LocalDateTime parseSubmittedAtOrNull(String raw) {
        return parseSubmittedAt(raw);
    }

    private static java.time.LocalDateTime parseSubmittedAt(String raw) {
        if (raw == null || raw.isBlank()) {
            return null;
        }
        try {
            ...
            throw new kz.eco.common.exception.BadRequestException(
                    "Некорректный формат submittedAt, ожидается ISO-8601 (2026-09-10T14:30:00)",
                    "PEK_SUBMISSION_DATE_INVALID");
        }
    }
    ...
    /** Explicit migration of a DRAFT/RETURNED program to the current regulation edition/template. */
    @PreAuthorize(PekSecurityExpressions.PEK_PROGRAM_EDIT)
    @PostMapping("/programs/{id}/retemplate")
    public ApiResponse<PekApiDtos.ProgramResponse> retemplateProgram(
            @PathVariable Long id, @RequestHeader("If-Match") Long version) {
        requireProgramAccess(id);
        return ApiResponse.ok(programService.retemplate(id, version, CurrentUser.get().getId()),
                "Шаблон программы ПЭК актуализирован");
    }
    ...
    public ApiResponse<PekApiDtos.ReportCreationContext> creationContext(
        ...
        requireCompanyAccess(companyId);
        return ApiResponse.ok(reportService.creationContext(companyId, objectId, periodType, year, quarter, programId));
    }
    ...
    /** Module fix: report-level general fields (currently actualCapacity/actualCapacityUnit) -
     *  see PekReport's javadoc for why capacity moved off the program. */
    @PreAuthorize(PekSecurityExpressions.PEK_REPORT_EDIT)
    @PatchMapping("/reports/{id}/general")
    public ApiResponse<PekApiDtos.ReportResponse> updateReportGeneral(
            @PathVariable Long id, @RequestHeader("If-Match") Long version,
            @RequestBody PekApiDtos.UpdateReportGeneralRequest request) {
        requireReportAccess(id);
        return ApiResponse.ok(reportService.updateGeneral(id, request, version), "Отчёт обновлён");
    }
    ...
    /** Official (state-facing) report structure - normative/actual tables built from
     *  {@code PekReportResultRow}, assembled by {@link PekOfficialReportDataService}. */
    @PreAuthorize(PekSecurityExpressions.PEK_VIEW)
    @GetMapping("/reports/{id}/official-data")
    public ApiResponse<PekApiDtos.OfficialReportData> officialData(@PathVariable Long id) {
        requireReportAccess(id);
        return ApiResponse.ok(reportService.getOfficialData(id));
    }
