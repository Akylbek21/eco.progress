...
public class PekReportPackageController {
 ...
 /** If-Match carries the REPORT's version (not the package's documentVersion). It is mandatory:
  *  building a package renders every document from the report's current data, so a caller working
  *  from a stale view must be told rather than handed an archive of something they did not see.
  *  A missing header is VERSION_REQUIRED (400); a stale one is PEK_VERSION_CONFLICT (409). */
 @PreAuthorize(PekSecurityExpressions.PEK_REPORT_EDIT) @PostMapping("/generate") public ApiResponse<PekMonitoringDtos.PackageResponse> generate(@PathVariable Long id,@RequestHeader(value="If-Match",required=false) Long version){check(id);return ApiResponse.ok(service.generate(id,CurrentUser.get().getId(),version),"Комплект ПЭК сформирован");}
 /** What a package built right now would contain: every planned file with READY/MISSING/STALE and
  *  every issue that blocks generation. Read-only; available before the first package exists. */
 @PreAuthorize(PekSecurityExpressions.PEK_VIEW) @GetMapping("/preflight") public ApiResponse<PekMonitoringDtos.PreflightResponse> preflight(@PathVariable Long id){check(id);return ApiResponse.ok(service.preflight(id));}
