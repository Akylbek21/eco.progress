...
public class GlobalExceptionHandler {
    ...
    /** Multipart-запрос без обязательной части (например, импорт без file) - 400, а не 500. */
    @ExceptionHandler(org.springframework.web.multipart.support.MissingServletRequestPartException.class)
    public ResponseEntity<ApiResponse<Void>> handleMissingRequestPart(
            org.springframework.web.multipart.support.MissingServletRequestPartException ex) {
        String message = "Отсутствует обязательная часть запроса: " + ex.getRequestPartName();
        return ResponseEntity.badRequest()
                .body(ApiResponse.error(message, "MISSING_REQUIRED_PARAMETER", List.of(message)));
    }
    ...
    @ExceptionHandler(StructuredConflictException.class)
    public ResponseEntity<ApiResponse<Void>> handleStructuredConflict(StructuredConflictException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(ApiResponse.error(ex.getMessage(), ex.getCode(), ex.getItems()));
    }
    ...
    /** A value longer than the column it is stored in is a server-side schema/width defect, exactly
     *  like the NOT NULL case: the client sent legitimate data, nothing is missing and no reference
     *  is broken, so REFERENCE_CONFLICT ("one of the related records does not exist") is wrong here
     *  too and sends whoever debugs it hunting for a deleted row - see
     *  pek_report_document_versions.regulation_version, which stayed VARCHAR(120) wherever V117's
     *  widening silently did nothing (it was written in H2's ALTER COLUMN syntax, which MySQL
     *  rejects), while the application stamps the 272-character citation of the 2026 edition into
     *  it. Every PEK document generation then failed pointing at the wrong cause; V127 widens the
     *  columns, this branch makes the next occurrence say what it actually is. */
    private static final java.util.regex.Pattern DATA_TOO_LONG_VIOLATION = java.util.regex.Pattern.compile(
            "data too long|value too long|data truncation|exceeds maximum length",
            java.util.regex.Pattern.CASE_INSENSITIVE);

    /** A clash on uk_pek_report_document_versions_report_version means two generations of the same
     *  PEK report raced for the same document version number - nothing is missing and no reference
     *  is broken, so REFERENCE_CONFLICT ("one of the related records does not exist") is actively
     *  wrong here and sends whoever debugs it hunting for a deleted row. The generation paths take
     *  a row lock on the report first (PekReportDocumentGenerationService#lockReport), so reaching
     *  this is the second line of defence and the caller only has to retry. */
    private static final java.util.regex.Pattern PEK_DOCUMENT_VERSION_CONFLICT =
            java.util.regex.Pattern.compile("uk_pek_report_document_versions_report_version",
                    java.util.regex.Pattern.CASE_INSENSITIVE);
    ...
    public ResponseEntity<ApiResponse<Void>> handleDataIntegrityViolation(DataIntegrityViolationException ex) {
        ...
        if (DATA_TOO_LONG_VIOLATION.matcher(rootMessage).find()) {
            log.error("Schema constraint violation (value longer than the column it is stored in) "
                    + "[traceId={}]: rootCause={}", traceId, rootMessage, ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error(
                            "Внутренняя ошибка сервера при сохранении записи. Сообщите администратору код " + traceId,
        ...
        if (PEK_DOCUMENT_VERSION_CONFLICT.matcher(rootMessage).find()) {
            log.warn("Concurrent PEK report document generation raced on the document version "
                    + "sequence [traceId={}]: rootCause={}", traceId, rootMessage, ex);
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(ApiResponse.error(
                            "Документ этого отчёта в данный момент формируется другим запросом - повторите попытку",
                            "PEK_DOCUMENT_VERSION_CONFLICT", Map.<String, String>of(), traceId));
        }
