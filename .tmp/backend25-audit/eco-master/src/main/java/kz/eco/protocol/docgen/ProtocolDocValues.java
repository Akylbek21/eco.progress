...
final class ProtocolDocValues {
    ...
    /** Result divided by the ПДК, e.g. "0,45"; "—" when either side is missing or the ПДК is zero. */
    static String pdkShare(ProtocolResult result) {
        BigDecimal value = firstNonNull(result.getResultValue(), result.getResultMgM3());
        BigDecimal pdk = firstNonNull(result.getPdkMgM3(), result.getNormativeValue());
        if (value == null || pdk == null || pdk.signum() == 0) {
            return EMPTY;
        }
        return formatDecimal(value.divide(pdk, 2, java.math.RoundingMode.HALF_UP));
    }

    /** "t = 45 °C; V = 12,3 м/с; 1,2 нм³/с" - only the parameters actually measured are listed. */
    static String gasFlowParameters(ProtocolResult result) {
        java.util.List<String> parts = new java.util.ArrayList<>();
        if (result.getTemperatureC() != null) parts.add("t = " + formatDecimal(result.getTemperatureC()) + " °C");
        if (result.getFlowSpeedMs() != null) parts.add("V = " + formatDecimal(result.getFlowSpeedMs()) + " м/с");
        if (result.getGasAirVolumeNm3s() != null) parts.add(formatDecimal(result.getGasAirVolumeNm3s()) + " нм³/с");
        if (result.getDuctAreaM2() != null) parts.add("S = " + formatDecimal(result.getDuctAreaM2()) + " м²");
        return parts.isEmpty() ? EMPTY : String.join("; ", parts);
    }

    /** Annual mass emission, when the laboratory recorded one ("resultTonsYear"/"tonsPerYear"). Never
     *  derived from г/с here: that needs the source's operating hours, which a protocol does not carry. */
    static String tonsPerYear(Map<String, Object> values) {
        String v = stringValue(values.get("resultTonsYear"));
        if (v == null) v = stringValue(values.get("tonsPerYear"));
        return v == null ? EMPTY : v;
    }

    /** (actual - normative) / normative in percent, one decimal; "—" without both values. */
    static String deviationPercent(BigDecimal actual, BigDecimal normative) {
        if (actual == null || normative == null || normative.signum() == 0) {
            return EMPTY;
        }
        BigDecimal pct = actual.subtract(normative).multiply(BigDecimal.valueOf(100))
                .divide(normative, 1, java.math.RoundingMode.HALF_UP);
        return (pct.signum() > 0 ? "+" : "") + formatDecimal(pct);
    }
