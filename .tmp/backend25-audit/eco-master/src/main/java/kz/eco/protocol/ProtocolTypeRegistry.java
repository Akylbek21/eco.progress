...
public final class ProtocolTypeRegistry {
    ...
    /** DOCX template codes that were renamed; a client still sending the old code for the same
     *  type is consistent, not a mismatch. */
    private static final Map<String, String> LEGACY_DOCX_TEMPLATE_CODES = Map.of(
            "protocol_ambient_air", "protocol_ambient_air_szz");
    ...
    private static Map<String, ProtocolTypeConfig> build() {
        ...
        // Two separate products, not one: a СЗЗ protocol reports ambient concentrations at control
        // points against ПДК, an industrial-emissions protocol reports a stationary source's gas
        // flow and mass emission (г/с, т/год) against its норматив ПДВ. They used to share one
        // "ambient_air" entry, so an emissions protocol was printed on the СЗЗ form.
        map.put("ambient_air_szz", config(
                "ambient_air_szz", "Атмосферный воздух / СЗЗ", SourceDocumentCode.DSM_70.name(),
                "protocol_ambient_air_szz", "мг/м³", "ambient_air", ProtocolTypeConfig.ResultMode.CHEMICAL));
        map.put("industrial_emissions", config(
                "industrial_emissions", "Промышленные выбросы", SourceDocumentCode.DSM_70.name(),
                "protocol_industrial_emissions", "мг/м³", "industrial_emissions",
                ProtocolTypeConfig.ResultMode.CHEMICAL));
    ...
    private static String canonicalId(String rawTemplateId) {
        ...
        return switch (normalized) {
            // Legacy spellings of the СЗЗ type stay valid input aliases and resolve to the canonical
            // id; industrial_emissions is its own type and never collapses into it.
    ...
    public static void validateConsistency(ProtocolTypeConfig config, String requestedSourceDocumentCode,
        ...
        if (requestedDocxTemplateCode != null && !requestedDocxTemplateCode.isBlank()
                && !equalsIgnoreCaseNormalized(canonicalDocxTemplateCode(requestedDocxTemplateCode),
                        config.docxTemplateCode())) {
            throw new BadRequestException("Шаблон печати не соответствует типу протокола");
        }
    }

    private static String canonicalDocxTemplateCode(String requested) {
        String trimmed = requested.trim();
        return LEGACY_DOCX_TEMPLATE_CODES.getOrDefault(trimmed.toLowerCase(Locale.ROOT), trimmed);
    }
