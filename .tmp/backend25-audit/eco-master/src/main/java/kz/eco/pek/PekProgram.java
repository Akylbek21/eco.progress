...
public class PekProgram {
    ...
    /** Design/installed production capacity of the facility - the value only; the unit is
     *  {@link #designCapacityUnit}. Historically this held a combined free-text string like
     *  "120 т/год"; new writes split value and unit, existing rows keep whatever they already had
     *  in this column (see V122 - no backfill/parsing of historical free text). */
    @Column(name = "design_capacity", length = 255)
    private String designCapacity;

    /** Unit for {@link #designCapacity} (e.g. "т/год", "Гкал/ч"). Added in V122 alongside the
     *  removal of program-level actualCapacity - see that field's former javadoc for why actual
     *  (as-operated) capacity moved to {@link PekReport} instead. */
    @Column(name = "design_capacity_unit", length = 40)
    ...
    /** Описание технологического процесса - section 2 of the explanatory note. */
    @Column(name = "technological_process", columnDefinition = "TEXT")
    private String technologicalProcess;

    /** Основные источники воздействия на окружающую среду - section 3 of the explanatory note. The
     *  structured emission/discharge/waste inventories list them item by item; this is the
     *  author's summary of which of them matter and why. */
    @Column(name = "main_impact_sources", columnDefinition = "TEXT")
    private String mainImpactSources;

    public String getTechnologicalProcess() { return technologicalProcess; }
    public void setTechnologicalProcess(String v) { technologicalProcess = v; }
    public String getMainImpactSources() { return mainImpactSources; }
    public void setMainImpactSources(String v) { mainImpactSources = v; }
    ...
    public String getDesignCapacityUnit() { return designCapacityUnit; }
    public void setDesignCapacityUnit(String v) { designCapacityUnit = v; }
