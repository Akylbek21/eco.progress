...
public enum ProtocolTemplateCode {
    ...
    /**
     * The canonical API id of this template - the same id ProtocolTypeRegistry is keyed by, so
     * GET /templates and GET /{id} agree for the same protocol. СЗЗ is "ambient_air_szz" and
     * industrial emissions "industrial_emissions": two separate types. The old "ambient_air"
     * spelling remains a valid input alias (see fromNormalized), never an output.
     */
    public String toApiId() {
        return switch (this) {
            case AMBIENT_AIR_SZZ -> "ambient_air_szz";
