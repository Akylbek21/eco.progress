...
public final class PekApiDtos {
    ...
    public record MeasureDto(
            ...
            /** Объём работ (free text with its own unit). */
            String workVolume,
            /** Ожидаемый экологический эффект. */
            String environmentalEffect
    ) {
        /** Pre-existing 12-argument shape, for callers that do not send the report-package fields. */
        public MeasureDto(Long id, String code, String name, String description, String plannedStartDate,
                          String plannedEndDate, Long responsibleUserId, java.math.BigDecimal plannedBudget,
                          String currency, String status, Integer completionPercent, String resultDescription) {
            this(id, code, name, description, plannedStartDate, plannedEndDate, responsibleUserId, plannedBudget,
                    currency, status, completionPercent, resultDescription, null, null);
        }
    }
    ...
    public record FacilitySnapshotDto(
            ...
            /** Описание технологического процесса (explanatory note, section 2). */
            String technologicalProcess,
            /** Основные источники воздействия (explanatory note, section 3). */
            String mainImpactSources
    ) {
        public FacilitySnapshotDto(String facilityInformation, String kato, String binSnapshot, String oked,
                                   String environmentalCategory, String designCapacity,
                                   String productionCharacteristics, String monitoringScope,
                                   String readinessNotes, String designCapacityUnit) {
            this(facilityInformation, kato, binSnapshot, oked, environmentalCategory, designCapacity,
                    productionCharacteristics, monitoringScope, readinessNotes, designCapacityUnit, null, null);
        }
    }
    ...
    public record ReportResponse(
            ...
            List<String> blockingReasons,
            /** The period's explanatory-note narrative, edited through PATCH .../general. */
            ExplanatoryNoteFields explanatoryNote
    ) {
    }

    /** Per-period sections of the explanatory note. Null = not written yet; never pre-filled. */
    public record ExplanatoryNoteFields(String performedStudies, String monitoringResultsSummary,
                                        String exceedancesSummary, String measuresTaken, String conclusion) {
    }

    /** PATCH /api/pek/reports/{id}/general - the report-level fields that relate to THIS reporting
     *  period rather than the program (module fix: actual capacity moved off PekProgram - see
     *  {@link PekReport#getActualCapacity()}), plus the explanatory note's per-period sections.
     *  A null field means "leave unchanged", a blank one clears it. */
    public record UpdateReportGeneralRequest(String actualCapacity, String actualCapacityUnit,
                                             String performedStudies, String monitoringResultsSummary,
                                             String exceedancesSummary, String measuresTaken,
                                             String conclusion) {
        public UpdateReportGeneralRequest(String actualCapacity, String actualCapacityUnit) {
            this(actualCapacity, actualCapacityUnit, null, null, null, null, null);
        }
    }

    /** One program measure with this period's execution, GET/PUT .../reports/{id}/measure-executions.
     *  The plan columns are read-only here (they are edited on the program). */
    public record MeasureExecutionDto(Long measureId, String code, String name, String workVolume,
                                      String plannedStartDate, String plannedEndDate,
                                      java.math.BigDecimal plannedAmount, String currency,
                                      java.math.BigDecimal actualAmount, java.math.BigDecimal utilizationPercent,
                                      java.math.BigDecimal completionPercent, String environmentalEffect,
                                      String status, String note, String nonCompletionReason, Long version) {
    }

    public record MeasureExecutionRequest(Long measureId, java.math.BigDecimal actualAmount,
                                          java.math.BigDecimal completionPercent, String status, String note,
                                          String nonCompletionReason) {
    }

    /** Balance columns of the official emissions table for one source/substance - GET/PUT
     *  .../reports/{id}/emission-balances. */
    public record EmissionBalanceDto(Long emissionSourceId, String sourceCode, String sourceName,
                                     String substanceCode, String substanceName,
                                     java.math.BigDecimal withoutTreatmentTons, java.math.BigDecimal capturedTons,
                                     java.math.BigDecimal utilizedTons, String increaseReason,
                                     boolean increaseReasonRequired, Long version) {
    }

    public record EmissionBalanceRequest(Long emissionSourceId, String substanceCode,
                                         java.math.BigDecimal withoutTreatmentTons, java.math.BigDecimal capturedTons,
                                         java.math.BigDecimal utilizedTons, String increaseReason) {
    }
    ...
    public record PekReportDocumentVersionResponse(
            ...
            boolean stale,
            /** EMISSIONS_XLSX versions carry a spreadsheet instead of DOCX/PDF. */
            boolean hasXlsx
    ) {
        public PekReportDocumentVersionResponse(Long id, Long reportId, int version, String documentType,
                                                boolean hasDocx, boolean hasPdf, String sha256, String generatedAt,
                                                Long generatedBy, String generatedByName,
                                                Long sourceContentRevision, Long currentContentRevision,
                                                boolean stale) {
            this(id, reportId, version, documentType, hasDocx, hasPdf, sha256, generatedAt, generatedBy,
                    generatedByName, sourceContentRevision, currentContentRevision, stale, false);
        }
    }
