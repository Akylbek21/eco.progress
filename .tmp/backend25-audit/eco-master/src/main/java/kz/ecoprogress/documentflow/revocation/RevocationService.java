...
public class RevocationService {
    ...
    public RevocationRequest reject(Long requestId, String comment, Long userId) {
        ...
        touch(request);
        repository.save(request);
        // Module spec §19: the document must not be left stuck in REVOCATION_REQUESTED forever -
        // a rejected revocation request means the original signed document stands, so it goes
        // back to SIGNED (an explicitly allowed transition in DocumentStatus.ALLOWED_TRANSITIONS
        // that nothing used to actually trigger). Same transaction as the request status change.
        documentService.transitionStatus(request.getDocumentId(), DocumentStatus.SIGNED,
                "Отзыв отклонён: " + comment);
    ...
    public RevocationRequest cancel(Long requestId, Long userId) {
        ...
        RevocationStatus previousStatus = request.getStatus();
        request.setStatus(RevocationStatus.CANCELLED);
        touch(request);
        repository.save(request);
        // Same stuck-status fix as reject() - but only if the document actually left SIGNED
        // already (send() is what moves it to REVOCATION_REQUESTED; cancelling a still-DRAFT
        // request, which send() was never called on, must not touch a document that never left
        // SIGNED in the first place - transitionStatus would just no-op/throw on an invalid
        // transition otherwise, so this check keeps the intent explicit rather than relying on that).
        if (previousStatus != RevocationStatus.DRAFT) {
            documentService.transitionStatus(request.getDocumentId(), DocumentStatus.SIGNED,
                    "Отзыв отменён");
        }
