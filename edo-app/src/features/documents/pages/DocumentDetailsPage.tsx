...
export const DocumentDetailsPage = ({ initialTab = 0, openSigning = false }: { initialTab?: number; openSigning?: boolean }) => {
  ...
  const sign = async () => {
    ...
    try {
      ...
      const cms = await createDetachedCms(challenge.dataBase64, setSigningState);
      setSigningState('Сервер проверяет подпись…');
      const updated = await signingApi.submit(documentId, challenge, cms);
      client.setQueryData(documentKeys.details(documentId), updated);
      await client.invalidateQueries({ queryKey: documentKeys.dashboard });
      setSigningState('Подпись проверена сервером.');
      if (openSigning) navigate(`/app/documents/${documentId}`, { replace: true });
    } catch (requestError) {
      setError(mapApiError(requestError, 'Не удалось подписать документ.').message);
      setSigningState(undefined);
    }
  };
  const download = async (kind: 'file' | 'signed-package') => {
    const blob = await documentsApi.download(documentId, kind);
    const url = URL.createObjectURL(blob);
    const anchor = window.document.createElement('a');
    anchor.href = url;
    anchor.download = kind === 'file' ? `document-${documentId}` : `signed-package-${documentId}.zip`;
    anchor.click();
      ...
      <Paper variant="outlined" sx={{ p: { xs: 2, md: 3 } }}>
        <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" gap={3}>
          ...
          <Stack direction="row" gap={1} flexWrap="wrap">
            ...
            {can(document.availableActions, 'SEND') && <Button variant="contained" startIcon={<SendIcon />}>Отправить</Button>}
