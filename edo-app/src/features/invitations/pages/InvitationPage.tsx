...
export const InvitationPage = () => {
  ...
  const query = useQuery({ queryKey: ['invitation', token], queryFn: ({ signal }) => invitationsApi.get(token, signal), retry: false });
  const mutation = useMutation({ mutationFn: (action: 'accept' | 'decline') => invitationsApi.respond(token, action), onSuccess: (_, action) => setResult(action === 'accept' ? 'Приглашение принято.' : 'Приглашение отклонено.') });
