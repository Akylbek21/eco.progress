...
class PekSubmissionLifecycleTest {
    ...
    void setUp() {
        ...
        // submit() requires a verified signature on exactly this OFFICIAL version.
        PekReportSignature signature = new PekReportSignature();
        signature.setReportId(reportId);
        signature.setDocumentVersionId(official.getId());
        signature.setSignerUserId(userId);
        signature.setSignedAt(java.time.LocalDateTime.now().minusHours(1));
        signature.setDocumentHash("0".repeat(64));
        signature.setCmsFileId("test-cms-" + reportId);
        signature.setVerified(true);
        signatureRepository.saveAndFlush(signature);
    }

    private PekReport doSubmit(Long version) {
        return submissionService.submit(reportId, version,
                new PekReportSubmissionService.OfficialSubmissionDetails(java.time.LocalDateTime.now().minusMinutes(5),
                        PekSubmissionMethod.EMAIL, null, null, null), userId);
    }
    ...
    void reportResponse_exposesSubmissionLifecycleFields() {
        ...
        PekReport submitted = doSubmit(report.getVersion());
    ...
    void submit_signedToSubmitted_stampsSubmittedAt() {
        ...
        PekReport updated = doSubmit(version);
    ...
    void accept_submittedToAccepted_stampsAcceptedAt() {
        // First submit
        PekReport after = doSubmit(reportRepository.findById(reportId).orElseThrow().getVersion());
    ...
    void submit_withOnlyInternalDocument_isRejected() {
        ...
        BadRequestException ex = assertThrows(BadRequestException.class,
                () -> doSubmit(reportRepository.findById(reportId).orElseThrow().getVersion()));
    ...
    void submit_fromDraft_throwsConflict() {
        ...
        assertThrows(ConflictException.class,
                () -> doSubmit(reloaded.getVersion()));
    }

    @Test
    void accepted_cannotBeSubmittedAgain() {
        PekReport afterSubmit = doSubmit(reportRepository.findById(reportId).orElseThrow().getVersion());
        PekReport afterAccept = submissionService.accept(reportId, afterSubmit.getVersion());

        assertThrows(ConflictException.class,
                () -> doSubmit(afterAccept.getVersion()));
    }
}
