...
import kz.eco.protocol.NormativeSampleSeeder;
...
class NormativeDirectoryServiceTest {
    ...
    @Autowired
    private NormativeReferenceRepository normativeReferenceRepository;
    ...
    void importClassifiedSample() throws IOException {
        ...
        // NormativeSampleSeeder is @Profile({"dev","local"}) - it never runs as a Spring-managed
        // bean under this test's default profile (no @ActiveProfiles anywhere in this test suite),
        // which is exactly why listRecords_all_includesLegacyDemoNormatives/
        // listRecords_includesLegacyNormatives used to fail: the "Пыль"/"Железо"/"Шум"/"E.coli"
        // legacy demo rows they assert on were never inserted. Invoke it directly (not as a bean)
        // so these tests exercise the real legacy-table (normative_references) read path without
        // depending on which profile happens to be active.
        new NormativeSampleSeeder(normativeReferenceRepository).run();
    }
