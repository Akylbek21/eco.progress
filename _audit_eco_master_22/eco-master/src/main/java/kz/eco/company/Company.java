...
public class Company {
    ...
    /** P0 module fix item 12: objectName/objectAddress/activityType/samplingLocation are legacy
     *  CREATE-TIME-ONLY input fields - {@link CompanyService#ensurePrimaryObject} /
     *  {@link CompanyPrimaryObjectMaterializer} consumes them exactly once to seed that company's
     *  primary {@link CompanyObject} row, which is the real, ongoing source of truth for
     *  per-object data from then on (a company can have several objects; these four fields can
     *  only ever describe one). Kept on Company for backward compatibility with existing create
     *  payloads/rows, but callers that want CURRENT object data must read CompanyObject, never
     *  these fields, after creation. directorPosition/knp/contractNumber/contractDate ARE genuine
     *  company-level fields (legal representative role, tax/contract metadata for the company as a
     *  legal entity, not any one object) and stay here. */
    @Column(length = 255)
    ...
    /** Company-level: the legal entity's designated representative for correspondence/contracts -
     *  distinct from any one object's on-site contact. */
    @Column(length = 255)
    ...
    /** P0 module fix item 11: FE-only fields that the backend never stored - added here (rather
     *  than removed from the contract) since they're small, harmless, genuinely useful company
     *  attributes and dropping them would silently discard whatever the frontend already collects
     *  from users. */
    @Column(length = 100)
    ...
    /** P0 module fix item 8: optimistic locking - was entirely absent, so two concurrent PATCHes
     *  silently last-write-wins clobbered each other. */
    @Version
    @Column(nullable = false)
