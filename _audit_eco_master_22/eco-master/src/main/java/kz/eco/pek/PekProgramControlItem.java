...
public class PekProgramControlItem {
    ...
    /** Explicit "this position is performed at every monitoring point of its направление" flag.
     *  Before it existed, a {@code null} monitoringPointId implicitly meant "all points", which is
     *  indistinguishable from "the point was simply not filled in" - and that silently multiplied
     *  a position across every point (a two-point programme produced four requirements instead of
     *  two). The two cases are now separate: this flag means all points on purpose, a set
     *  monitoringPointId means exactly that point, and neither means the position is not
     *  configured yet and is reported as CONFIGURATION_REQUIRED rather than fanned out.
     *  See ProtocolPekCreationService#pointsFor. */
    @Column(name = "applies_to_all_points", nullable = false)
    private boolean appliesToAllPoints = false;
    ...
    public boolean isAppliesToAllPoints() { return appliesToAllPoints; }
    public void setAppliesToAllPoints(boolean appliesToAllPoints) { this.appliesToAllPoints = appliesToAllPoints; }
