...
public class PekProgramDocumentGenerationService {
    ...
    private PekProgramDocValues buildValues(PekProgram program) {
        ...
        var general = new OfficialPekReportDocValues.GeneralInfo(
                ...
                // The PROGRAM document has no actual (as-operated) capacity to show - that figure
                // now belongs to a specific PekReport period, not the multi-year program.
                program.getProductionCharacteristics(),
                program.getDesignCapacityUnit() == null || program.getDesignCapacityUnit().isBlank()
                        ? program.getDesignCapacity()
                        : (program.getDesignCapacity() == null ? "" : program.getDesignCapacity() + " ")
                                + program.getDesignCapacityUnit(),
                null,
