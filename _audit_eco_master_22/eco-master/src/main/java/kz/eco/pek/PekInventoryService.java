...
public class PekInventoryService {
    ...
    public WasteMovementDto upsertWasteMovement(Long reportId, WasteMovementRequest r, Long version) {
        ...
        PekReportWasteMovement persisted;
        try {
            persisted = wasteMovements.saveAndFlush(m);
        } catch (DataIntegrityViolationException e) {
            // Two callers can both find no row for this (report, waste type) and both try to insert
            // one. uk_pek_waste_movement_report_item stops the second from creating a duplicate;
            // without this catch it surfaced as the generic REFERENCE_CONFLICT ("одна из связанных
            // записей не существует"), which describes a different problem entirely and tells the
            // caller nothing about what to do next.
            throw new ConflictException(
                    "Данные по этому виду отхода за период уже введены другим сотрудником - "
                            + "обновите страницу и повторите изменение",
                    "PEK_WASTE_MOVEMENT_DUPLICATE");
        }
        WasteMovementDto saved = dto(persisted, item);
